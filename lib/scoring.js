...
// QUICK scorer prompt (Haiku for feed scans). Scores a batch of listings from
// title / company / location only. Embeds the identical RUBRIC.
function buildQuickPrompt(candidateProfile, recipe, jobsBlock, extraLine, styleRules) {
  return `Score these job search results for the candidate below. This is a quick scan from title, company and location only, so where a factor is not visible apply the missing-information rule.
...
Return ONLY a JSON array, no markdown. Each object: {"i": index, "score": 1-10, "signal": "apply"/"maybe"/"skip", "reason": "one sentence on relevance to THIS candidate", "badge": "Best Match"/"Strong Fit"/"Worth a Look"/"Stretch"/null, "office": "Remote"/"1 day"/"2 days"/"3+ days"/"Unknown"}.
Only include roles scoring 7 or above. Max 3 per company. Be strict: reject junior, pure sales-quota, 3+ mandatory office days, wrong sector, generic aggregator listings.
