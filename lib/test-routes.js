 ...
 * ============================================================================
 *
 * Backs the 6 dev-only sign-in routes (app/freetier, app/protier,
 * app/maxtier, app/trialtier, app/sandbox, app/onboardtier). Each one signs
 * Rob into a dedicated, pre-seeded test account for that tier and drops him
 * into /app, so real tier differences can be experienced firsthand rather
 * than inferred from the code. /onboardtier is the exception: it resets its
 * account to a genuinely blank, never-onboarded state on EVERY hit, THEN
 * signs in and lands on /onboard, not /app -- see reset_onboarding_test_
 * account() (migration 020) and app/onboardtier/route.js.
 *
...
export const TEST_ACCOUNTS = {
  ...
  onboard: {
    email: 'rob.test.onboard@requite-internal.test',
    label: 'Onboarding (Free tier, reset to a genuinely blank state on every visit)',
    // Reset happens on ENTRY (before sign-in), never on exit -- no
    // beforeunload/close detection, which is unreliable and was explicitly
    // ruled out. redirectTo overrides the default '/app' landing so a
    // blank profile lands where a blank profile actually belongs.
    resetOnEntry: true,
    redirectTo: '/onboard',
  },
}
 ...
 *
 * @param {'free'|'pro'|'max'|'trial'|'sandbox'|'onboard'} tierKey
...
export async function signInAsTestAccount(tierKey, request) {
  ...
  // Reset on ENTRY, before the sign-in link is even minted -- so the very
  // first thing this account shows, every single time, is blank. Looked up
  // by email against public.users (which mirrors auth.users.id/email)
  // rather than paging through admin.listUsers, since this runs on every
  // single hit and the id is a plain, fast, indexed lookup.
  if (account.resetOnEntry) {
    const { data: userRow, error: lookupError } = await service
      .from('users')
      .select('id')
      .eq('email', account.email)
      .single()
    if (lookupError || !userRow?.id) {
      console.error(`[test-routes] reset lookup failed for ${tierKey}:`, lookupError?.message || 'no user row')
      return NextResponse.json({ error: 'Test account reset failed: account not found. Has it been seeded yet? See PROGRESS.md Stage 80.' }, { status: 500 })
    }
    const { error: resetError } = await service.rpc('reset_onboarding_test_account', { p_user_id: userRow.id })
    if (resetError) {
      console.error(`[test-routes] reset RPC failed for ${tierKey}:`, resetError.message)
      return NextResponse.json({ error: 'Test account reset failed.' }, { status: 500 })
    }
  }
  ...
  callbackUrl.searchParams.set('next', account.redirectTo || '/app')
