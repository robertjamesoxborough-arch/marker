...
const {
  ...
  buildFullSystem, buildQuickPrompt, computeOverall, roundToScale,
} = require('./scoring')
...
const quickPrompt = buildQuickPrompt(candidate, 'MATCH marketing leadership', '[0] "Head of Marketing" at Acme | London', '', style)
assert(fullPrompt.includes(RUBRIC), 'FULL prompt contains the rubric verbatim')
assert(quickPrompt.includes(RUBRIC), 'QUICK prompt contains the rubric verbatim')
