...
function scoreRoleFit(profile, job) {
  ...
  for (const t of targets) {
    const overlap = jaccardOverlap(jobTitle, t)
    if (overlap > best) { best = overlap; bestTarget = t }
  }

  // Also score against all keywords combined
  const keywordStr = keywords.join(' ')
  if (keywordStr) {
    const kwOverlap = jaccardOverlap(jobTitle, keywordStr) * 0.7
    if (kwOverlap > best) { best = kwOverlap; bestTarget = 'your keywords' }
  }
...
function scoreCompFit(profile, job) {
  ...
  if (!salaryStr) {
    if (!floor) return { score: 5, reason: 'No salary data — set a salary floor in profile for a better match' }
  ...
  if (!floor) {
    return { score: 7, reason: `Salary ~£${midK}k — set a salary floor in profile for a precise match` }
  }
