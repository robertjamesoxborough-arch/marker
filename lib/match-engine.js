 ...
 *   // result.score       — overall 0–10 (1 d.p.), meaningless if result.excluded
 *   // result.excluded    — true if this job must never be shown to this user
 *   // result.excludeReason — plain-English reason, set only when excluded
 *   // result.dimensions  — { roleFit, seniorityFit, locationFit, compFit, freshness, cultureWlb, weeklyFocus }
 *   // each dimension     — { score: 0–10, reason: string }
 *
 * ── GATING PRINCIPLE (Stage 46) ─────────────────────────────────────────
 * Role relevance is a GATE, not just one weighted factor among seven. If a
 * role isn't the kind of work the candidate does, no amount of good salary,
 * location, seniority, or freshness may promote it into view — a Business
 * Analyst role or a Software Engineer role must never appear in a Programme
 * Lead's feed, no matter how well it scores on everything else. The seven
 * weighted dimensions only ever rank jobs that already passed the gate
 * against each other; they never lift a gated-out job back into view.
 * `result.excluded` is the single source of truth for this — every caller
 * (every feed route) MUST filter on `!result.excluded` before ranking or
 * displaying, not on ad-hoc dimension-score checks of its own. This is
 * enforced in exactly one place (this function) specifically so the rule
 * cannot regress by drifting out of sync across the 5 routes that call
 * scoreMatch (feed-cache, feed-web, feed-gov, job-feed, contractor/roles).
 * See ROLE_FIT_GATE_MIN below for why the threshold is set where it is.
 */

const { geoDistance } = require('./uk-geo')
const { stemWord } = require('./role-keywords')
...
// A roleFit score below this is grounds for hard exclusion, never just a
// weighted penalty (see the GATING PRINCIPLE docstring above). Threshold
// chosen deliberately at the boundary between scoreRoleFit's "some skill
// match" bucket (6, jaccard overlap 0.12-0.25 — a real, if partial,
// functional connection) and its "limited overlap" bucket (4, jaccard
// 0.04-0.12). That 4-12% band is, in practice, one or two incidentally
// shared words between otherwise unrelated titles — the same character of
// false signal as the "lead"/"engineering" bug fixed in Stage 45, not a
// genuine partial match. Bucket 2 (<0.04, "doesn't match") and bucket 3
// (family conflict — "shares wording but is a different type of role") are
// unambiguous exclusions. Only scores actually reachable via a real
// comparison are gated: scoreRoleFit's `assessed: false` results (no job
// title to compare, or no target roles/keywords set on the profile at all)
// are a "we don't know" signal, never excluded on that basis alone.
const ROLE_FIT_GATE_MIN = 6
...
// "lead" deliberately included: verified live (Stage 45 self-test) that it
// alone was enough to rank "Tech Lead - Software Engineering" and "Lead IAM
// Technical Architect" above genuinely-relevant roles against a target of
// "Programme Lead" — it's used as a seniority/role modifier across every
// function (Tech Lead, Sales Lead, Marketing Lead...) and carries no
// domain signal on its own.
const LOW_SIGNAL_WORDS = new Set([
  'senior', 'technical', 'digital', 'global', 'regional', 'national',
  'junior', 'principal', 'strategic', 'commercial', 'group', 'international',
  'remote', 'hybrid', 'uk', 'lead',
])
...
// Compared on STEMS, not raw words -- found live (Stage 68 cross-profession
// stress test) that a plain exact-word Jaccard overlap missed genuinely
// identical professions purely on English suffix variation: "Marine
// Pilotage" vs "Harbour Pilot", "Arboricultural Consulting" vs "Consultant
// Arborist". stemWord (lib/role-keywords.js) is a small, generic suffix
// stripper, not a profession-specific fix -- it helps any two words that
// happen to share a root, regardless of field.
function jaccardOverlapSignificant(a, b) {
  const setA = new Set(tokenizeSignificant(a).map(stemWord))
  const setB = new Set(tokenizeSignificant(b).map(stemWord))
  if (setA.size === 0 && setB.size === 0) return 0
  let shared = 0
  setA.forEach(w => { if (setB.has(w)) shared++ })
  const union = setA.size + setB.size - shared
...
const ROLE_FAMILIES = {
  ...
  // "engineer" alone misses "Software Engineering"/"VP of Engineering" —
  // matchesKeyword is word-boundary, so "engineer" never matches inside
  // "engineering" (verified live, Stage 45 self-test). Listed separately.
  engineering: ['engineer', 'engineering', 'developer', 'devops', 'sre', 'architect'],
  ...
  legal: ['legal', 'compliance', 'counsel', 'solicitor', 'paralegal'],
  healthcare: ['nurse', 'nursing', 'clinical', 'doctor', 'physician', 'midwife', 'physiotherapist', 'matron', 'sister', 'healthcare assistant'],
  education: ['teacher', 'teaching', 'lecturer', 'tutor', 'headteacher', 'classroom'],
  trades: ['electrician', 'plumber', 'carpenter', 'bricklayer', 'tradesperson'],
}
...
// Role relevance is a GATE, not just one weighted factor among seven — see
// the ROLE_FIT_GATE_MIN block below scoreMatch for the full principle. This
// scorer additionally marks `assessed: true` whenever it has real data to
// judge against (so the gate can tell "genuinely doesn't match" apart from
// "we don't know yet" — the latter must never be excluded, only the former).
// Note: this scorer previously also matched against a cvKeywords list
// extracted from the user's CV by an AI parse. That extraction was removed
// (Stage 74) because it could surface special-category data, e.g. a health
// condition mentioned anywhere in a CV, into stored profile data and every
// downstream prompt. Role fit is now judged purely on target_roles, which
// onboarding makes mandatory, so every real profile still has a signal here.
function scoreRoleFit(profile, job) {
  const targets = profile?.target_roles || []
  const jobTitle = job?.role_title || job?.roleTitle || ''

  if (!jobTitle) return { score: 5, reason: 'No job title available to compare against target roles', assessed: false }
  if (targets.length === 0) {
    return { score: 5, reason: 'No target roles set in profile; add them for a better match', assessed: false }
  }
  ...
  if (bestConflict) {
    return { score: 3, reason: `"${jobTitle}" shares wording with "${bestTarget}" but is a different type of role; not a real match despite the overlap`, assessed: true }
  }

  if (best >= 0.40) {
    return { score: 10, reason: `"${jobTitle}" is a close match to "${bestTarget}"`, assessed: true }
  } else if (best >= 0.25) {
    return { score: 8, reason: `"${jobTitle}" strongly aligns with your target "${bestTarget}"`, assessed: true }
  } else if (best >= 0.12) {
    return { score: 6, reason: `"${jobTitle}" partially overlaps with "${bestTarget}": some skill match`, assessed: true }
  } else if (best >= 0.04) {
    const shown = targets.slice(0, 2).join(', ') || 'your targets'
    return { score: 4, reason: `"${jobTitle}" has limited overlap with ${shown}`, assessed: true }
  } else {
    const shown = targets.slice(0, 2).join(', ') || 'your targets'
    return { score: 2, reason: `"${jobTitle}" doesn't match ${shown}`, assessed: true }
  }
}
...
function scoreLocationFit(profile, job) {
  ...
  // Geography — only relevant for roles that need SOME physical presence
  // (not fully remote). Resolves the user's postcode and the job's location
  // string against a static UK city table (lib/uk-geo.js, zero API cost) and
  // compares to the user's radiusMiles. Either side failing to resolve (an
  // unrecognised town, a vague location string) falls through to the
  // existing office-days-only logic below unchanged — this only ever ADDS a
  // hard-mismatch path, it never overrides a fit that the office-days logic
  // already found (respects the existing hard-mismatch behaviour).
  if (!isRemote) {
    const radiusMiles = profile?.hard_filters_json?.radiusMiles
    const dist = geoDistance(profile?.postcode, job?.location, radiusMiles)
    if (dist && !dist.withinRadius) {
      return { score: 1, reason: `${job?.location || 'Job location'} is ~${dist.miles} miles from ${profile.postcode}, outside your ${Number.isFinite(radiusMiles) && radiusMiles > 0 ? radiusMiles : 50}-mile radius${isHybrid ? ' (even hybrid roles need occasional office access)' : ''}` }
    }
  }
...
// ── Weekly preference ─────────────────────────────────────────────────────
// Free text the user writes for "what I want this week" (profiles.hard_
// filters_json.weeklyPreference), parsed deterministically -- no model call.
// Explicit "no X" exclusions are a hard veto; "remote only" is a hard
// preference; anything else is a soft keyword nudge. Kept intentionally
// simple: this is a light per-week steer, not a replacement for the profile's
// structured fields (target roles, seniority, salary floor, office days).
const WEEKLY_STOP_WORDS = new Set([
  'this', 'week', 'only', 'please', 'and', 'the', 'for', 'with', 'from',
  'into', 'not', 'no', 'would', 'take', 'pay', 'cut', 'fewer', 'more', 'less',
  'office', 'days', 'really', 'want', 'like', 'prefer', 'looking',
])

function extractExclusions(pref) {
  const matches = [...pref.matchAll(/\b(?:no|not|avoid)\s+([a-z][a-z-]{2,20})\b/g)]
  return matches.map(m => m[1]).filter(w => !WEEKLY_STOP_WORDS.has(w))
}

function scoreWeeklyPreference(profile, job) {
  const pref = (profile?.hard_filters_json?.weeklyPreference || '').toLowerCase().trim()
  if (!pref) return { score: 5, reason: 'No weekly preference set' }

  const jobText = [
    job?.role_title || job?.roleTitle || '',
    job?.company || '',
    job?.location || '',
    JSON.stringify(job?.raw_json || {}).slice(0, 2000),
  ].join(' ').toLowerCase()

  // Explicit exclusion is a hard veto — overrides everything else.
  const excluded = extractExclusions(pref).find(term => jobText.includes(term))
  if (excluded) {
    return { score: 1, reason: `Your note says "no ${excluded}" and this role mentions it` }
  }

  // Hard remote-only preference.
  if (/\bremote\s*only\b|\bonly\s*remote\b/.test(pref)) {
    const isRemote = /\bremote\b/.test(jobText)
    return isRemote
      ? { score: 9, reason: 'Remote: matches your "remote only" note' }
      : { score: 2, reason: 'Not remote; your note says remote only' }
  }

  // Salary-for-office-days trade-off — informational nudge; the actual
  // office-day scoring already lives in locationFit.
  if (/(pay cut|lower salary).{0,25}office|office.{0,25}(pay cut|lower salary)/.test(pref)) {
    return { score: 6, reason: 'Your note says you\'d trade salary for fewer office days, see location fit' }
  }

  // Generic soft nudge — any non-trivial word from the note also in the job.
  const words = pref.replace(/[^a-z0-9\s]/g, ' ').split(/\s+/).filter(w => w.length > 3 && !WEEKLY_STOP_WORDS.has(w))
  const hit = words.find(w => jobText.includes(w))
  if (hit) return { score: 7, reason: `Matches "${hit}" from your weekly note` }

  return { score: 5, reason: 'Weekly note set but no specific signal for this role' }
}
...
function scoreMatch(profile, job) {
  ...
  const weeklyFocus  = scoreWeeklyPreference(profile, job)
  ...
  const weighted =
    ...
    weeklyFocus.score  * WEIGHTS.weeklyFocus

  const score = Math.round(weighted * 10) / 10

  // The gate — see the GATING PRINCIPLE docstring at the top of this file.
  // Structural mismatches (wrong country/commute radius, wildly wrong
  // seniority) already hard-excluded via locationFit/seniorityFit === 1;
  // role relevance now gates the same way, on the same principle: no
  // weighted average is allowed to paper over "this is not the kind of
  // work this person does".
  let excluded = false
  let excludeReason = null
  if (locationFit.score === 1) { excluded = true; excludeReason = locationFit.reason }
  else if (seniorityFit.score === 1) { excluded = true; excludeReason = seniorityFit.reason }
  else if (roleFit.assessed && roleFit.score < ROLE_FIT_GATE_MIN) { excluded = true; excludeReason = roleFit.reason }
