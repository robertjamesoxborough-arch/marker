...
import { TIER_CAPS, SPEND_CEILING_GBP, SPEND_CEILING_MESSAGE, resolveTier } from './allowance-config'

// TIER_CAPS, SPEND_CEILING_GBP, SPEND_CEILING_MESSAGE and resolveTier all
// live in lib/allowance-config.js now (Stage 77/81) -- pure data/logic with
// no dependency on @supabase/supabase-js, split out for the same reason
// usage-window.js was: so lib/allowance.test.js can load them under plain
// `node`, without pulling in this whole file's Supabase client. See that
// file for the full reasoning behind every number, every cap, and the
// Stage 81 trial-tier fix. Re-exported here so every existing
// `import { ... } from '../lib/allowance'` call site keeps working unchanged.
export { TIER_CAPS, SPEND_CEILING_GBP, SPEND_CEILING_MESSAGE, resolveTier }
