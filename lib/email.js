...
// Sender identity. onboarding@resend.dev is Resend's shared sandbox domain:
// it is not our address, it hurts deliverability, and using it on mail to
// real customers misrepresents who is writing. Set EMAIL_FROM once the real
// domain is DNS-verified in Resend; the sandbox stays only as a last-resort
// fallback so local development does not break.
const FROM = process.env.EMAIL_FROM || `${BRAND_NAME} <onboarding@resend.dev>`

// Every email carries the same footer: who sent it, why this person is
// receiving it, and how to stop. Previously none of them did.
const POSTAL = process.env.EMAIL_POSTAL_ADDRESS || 'United Kingdom'
function footer(reason) {
  return `<div class="footer">
    ${BRAND_NAME}, a trading name of Robert Oxborough · ${POSTAL}<br>
    ${reason}<br>
    <a href="mailto:support@upstreaminsights.co.uk?subject=Unsubscribe">Reply or email us to stop these emails</a> ·
    <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://marker-silk.vercel.app'}/privacy">Privacy Policy</a>
  </div>`
}
...
export async function sendWelcome(email, name) {
  ...
  return resend.emails.send({
    ...
    </div>${footer('You are receiving this because you just created a Requite account.')}</div>`),
  })
}
...
export async function sendTrialEnding(email, name, daysLeft) {
  ...
  return resend.emails.send({
    ...
    </div>${footer('You are receiving this because you have an active Requite trial.')}</div>`),
  })
}
...
export async function sendTrialExpired(email, name) {
  ...
  return resend.emails.send({
    ...
    html: base(`<div class="wrap"><div class="top"><div class="logo">${BRAND_NAME.toLowerCase()}</div></div><div class="body">
      ...
      <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://marker-silk.vercel.app'}/settings" class="btn">Choose a plan</a>
    </div>${footer('You are receiving this because your Requite trial has ended.')}</div>`),
  })
}
...
export async function sendIntroRequest(candidateEmail, roleTitle) {
  ...
  return resend.emails.send({
    ...
    </div>${footer('You are receiving this because an employer requested an introduction to you on Requite.')}</div>`),
  })
}
...
export async function sendIntroResponse(employerEmail, roleTitle, action) {
  ...
  return resend.emails.send({
    ...
    </div>${footer('You are receiving this because you registered an employer interest with Requite.')}</div>`),
  })
}
