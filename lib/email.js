...
const FROM = 'Marker <onboarding@resend.dev>'
...
export async function sendWelcome(email, name) {
  ...
  return resend.emails.send({
    ...
    html: base(`<div class="wrap"><div class="top"><div class="logo">marker</div></div><div class="body">
      ...
      <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://marker-silk.vercel.app'}/app" class="btn">Open Marker</a>
    </div><div class="footer">Marker · AI-powered job hunt for senior people who'd quite like their evenings back.</div></div>`),
  })
}
...
export async function sendTrialEnding(email, name, daysLeft) {
  ...
  return resend.emails.send({
    ...
    html: base(`<div class="wrap"><div class="top"><div class="logo">marker</div></div><div class="body">
      ...
      <p>If Marker's been useful, pick a plan before ${daysLeft === 1 ? 'tomorrow' : `${daysLeft} days`}. If not, no worries — your data stays safe for 30 days in case you change your mind.</p>
      <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://marker-silk.vercel.app'}/settings" class="btn">Choose a plan</a>
    </div><div class="footer">Marker</div></div>`),
  })
}
...
export async function sendTrialExpired(email, name) {
  ...
  return resend.emails.send({
    from: FROM,
    to: email,
    subject: 'Your Marker trial has ended',
    html: base(`<div class="wrap"><div class="top"><div class="logo">marker</div></div><div class="body">
      <h1>${first ? `${first}, your` : 'Your'} trial has ended.</h1>
      <p>Your free trial is now over. To keep using Marker — your job feed, pipeline, and CV tools — choose a plan below.</p>
