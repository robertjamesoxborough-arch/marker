...
const { buildAiContext, MAX_CHARS, hasEnoughProfile } = require('./ai-context')
...
const CAREER_HISTORY = [
  { role_title: 'Head of Partnerships',        company: 'TechCorp',    start_date: '2020-01-01', end_date: '2023-12-31', achievements: 'Grew EMEA revenue 40%' },
  ...
  { role_title: 'Business Development Exec',    company: 'OldCo',       start_date: '2012-01-01', end_date: '2014-12-31' },
  { role_title: 'Graduate Analyst',             company: 'GradCo',      start_date: '2010-01-01', end_date: '2011-12-31' },
  { role_title: 'Intern',                       company: 'InternCo',    start_date: '2009-06-01', end_date: '2009-09-30' },
  { role_title: 'Volunteer Role',                company: 'VeryOldCo',  start_date: '2008-01-01', end_date: '2008-12-31' }, // > 6, excluded
]
...
// Regression guard (Stage 74): AI-extracted CV keywords were removed from
// the profile entirely because they could carry special-category data (a
// health condition named anywhere in a CV) into stored data and into every
// downstream prompt. buildAiContext must never reintroduce a "Key skills"
// line sourced from an AI keyword extraction.
assert('No AI-extracted keyword line injected', ctx.includes('Key skills'),   false)
assert('Contains benefit',             ctx.includes('hybrid'),                 true)
assert('Contains recent company',      ctx.includes('TechCorp'),              true)
assert('Contains an achievement',      ctx.includes('Grew EMEA revenue 40%'), true)
assert('Structured history used, not raw CV excerpt', !ctx.includes('AAAA'),  true)
assert('Only 6 career history items',  !ctx.includes('VeryOldCo'),            true)
assert('4th item within cap of 6',     ctx.includes('OldCo'),                 true)
...
// ── hasEnoughProfile (Stage 81 fix) ─────────────────────────────────────────
// Same definition app/app/tabs/TodayDashboard.js already uses client-side --
// cvRaw present, or at least one target role. Every AI route that builds a
// candidate context should agree with TodayDashboard on what "too thin to
// score" means, or a user can pass one screen's check and fail another's.
assert('Empty object profile: not enough', hasEnoughProfile({}), false)
assert('Null profile: not enough', hasEnoughProfile(null), false)
assert('Profile with only a postcode: not enough (matches TodayDashboard)', hasEnoughProfile({ postcode: 'kt6 5pn', track: 'standard' }), false)
assert('Profile with cvRaw only: enough', hasEnoughProfile({ hard_filters_json: { cvRaw: 'Some real CV text' } }), true)
assert('Profile with empty-string cvRaw: not enough', hasEnoughProfile({ hard_filters_json: { cvRaw: '' } }), false)
assert('Profile with a target role: enough', hasEnoughProfile({ target_roles: ['Product Manager'] }), true)
assert('Profile with an empty target_roles array: not enough', hasEnoughProfile({ target_roles: [] }), false)
assert('Profile with both cvRaw and target roles: enough', hasEnoughProfile({ hard_filters_json: { cvRaw: 'CV text' }, target_roles: ['PM'] }), true)
