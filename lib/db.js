...
function jobToRow(job, userId, accountId) {
  ...
  return {
    ...
    job_cache_id: job.jobCacheId || null,
    ...
    score_breakdown_json: {
      ...
      holdingArea: job.holdingArea || false,
      possibleDuplicateOf: job.possibleDuplicateOf || null,
      // Interview prep packs (Stage 67) — cached in full per stage so
      // Live mode's quick-tap cues are a zero-latency read on reload, not
      // a live call. Safe to store here (unlike stage NOTES, which are
      // append-only in their own table): a pack is always regenerated and
      // overwritten whole by its own owner, never incrementally amended by
      // a concurrent write, so a wholesale JSON upsert can't clobber it.
      interviewPrepByStage: job.interviewPrepByStage || bd.interviewPrepByStage || undefined,
    },
...
function rowToJob(row) {
  ...
  return {
    id: row.id,
    jobCacheId: row.job_cache_id || null,
    ...
    archived: bd.archived || false,
    holdingArea: bd.holdingArea || false,
    possibleDuplicateOf: bd.possibleDuplicateOf || null,
    interviewPrepByStage: bd.interviewPrepByStage || null,
