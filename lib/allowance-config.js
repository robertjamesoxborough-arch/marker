...
//
// This was previously inlined, identically, in TWO places: lib/allowance.js
// (checkAllowance) and app/api/profile/tier/route.js. Both wrote:
//   const trialActive = userData?.trial_ends_at && userData.trial_ends_at > now
//   const tier = userData?.tier || (trialActive ? 'trial' : 'free')
// The `users.tier` column has a NOT NULL DEFAULT 'free' constraint (see
// migration 001), so `userData?.tier` is ALWAYS the truthy string 'free'
// for a never-upgraded account -- the `trialActive` branch can never fire,
// full stop. This is not a rare edge case: it is the state of every single
// trialing user, always. The product promises a 7-day Pro-level trial
// (settings page literally renders "Trial · 7 days free"); the code above
// silently served Free-tier limits (cv:1, cover_letter:0, interview_prep:0,
// ...) to every trialing user for as long as this pattern existed. Found
// via the /trialtier dev test route (PROGRESS.md Stage 78/79/81) exercising
// the real checkAllowance() resolution path against a seeded account with a
// genuine future trial_ends_at.
//
// The fix: a STORED paid tier ('pro'/'max') always wins outright -- a real
// paying customer must never be downgraded by a stale or leftover
// trial_ends_at. Only when the stored tier is still the default 'free' does
// an active trial_ends_at promote the resolved tier to 'trial'. Once
// trial_ends_at passes, the same account correctly falls back to 'free'
// with no other change needed.
//
// Pure and dependency-free on purpose (no Date.now() call baked in --
// `now` is passed in, injectable for tests), so both call sites and
// lib/allowance.test.js can all share this exact one definition instead of
// three subtly different copies.
function resolveTier(userData, now) {
  const nowIso = (now instanceof Date ? now : new Date()).toISOString()
  const trialActive = !!(userData?.trial_ends_at && userData.trial_ends_at > nowIso)
  const storedTier = userData?.tier || 'free'
  if (storedTier === 'free' && trialActive) return 'trial'
  return storedTier
}

module.exports = { TIER_CAPS, SPEND_CEILING_GBP, SPEND_CEILING_MESSAGE, resolveTier }
