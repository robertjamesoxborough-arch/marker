...
export async function POST() {
  ...
  const SYSTEM = `You are a specialist recruitment researcher for UK permanent recruitment agencies and headhunters.
...
${STYLE_RULES}`
  ...
  const userMsg = `A ${seniorities} professional in ${field} (targeting permanent roles like: ${roles}) is looking for UK recruitment agencies and headhunters.

Location: ${location}
${wishlistStr}

Identify 10 UK recruitment agencies and search firms that actively place ${seniorities}-level ${field} professionals into permanent roles. Return ONLY a JSON array of 10 objects.`
