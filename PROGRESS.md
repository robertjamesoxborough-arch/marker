...
**Stage:** 76 complete — Session BE, a plain Notes scratchpad. New toolkit tab, no AI anywhere in the path, no allowance gating (confirmed there is genuinely no per-action cost: zero `fetch()` calls, zero AI imports, zero API route — the client talks straight to Supabase's PostgREST). Reachable via "More" (`buildTabs()` in `app/app/shared.js` pushes `'Notes'`, which `app/app/page.js`'s toolkit split picks up automatically since it isn't in `PRIMARY_TAB_IDS`), not a primary tab, matching the calm-front-door rule from Stage 71.

**A genuine architecture departure, done deliberately and verified.** Every other persistent write in this codebase goes through an API route that authenticates server-side and writes with the service-role client (`app/api/aggregator/channel-click/route.js` is the reference pattern). Notes is the first table in the repo queried with true direct client CRUD: `app/app/tabs/NotesTab.js` calls `createBrowserClient` and hits `.from('notes')` directly, with Postgres RLS as the only thing standing between one user and another's notes rather than a server-side check backing it up. New `supabase/migrations/017_notes.sql`: `notes(id, user_id, body, created_at, updated_at)`, RLS enabled with four explicit own-row policies (select/insert/update/delete, each `USING`/`WITH CHECK (user_id = auth.uid())`), and the table GRANT for `authenticated` included in the same migration from the start, per the standing rule this codebase learned the hard way in migration 008 (missing GRANTs silently broke 19 tables) and migration 009 (RLS enabled with no policy silently blocks every write with an unhelpful 42501).

**10-note cap enforced in Postgres, not just in the client.** A `BEFORE INSERT` trigger (`enforce_notes_cap`) counts the user's existing rows and raises `notes_cap_reached` at 10, so the cap holds even if two tabs are open at once or a client-side check is skipped — this is what actually guarantees it, not defence in depth. `NotesTab.js` also checks the count before attempting an insert, purely so the UI can show the friendly message immediately without a round trip; if the trigger fires anyway (the genuine race), its error is caught and shown as the identical message rather than a raw Postgres error surfacing to the user. `updated_at` is bumped by a second trigger on every edit, same convention as other tables.

**Self-tested against the real live database, not just code-reviewed.** Minted a real session for the real production account via the Supabase Admin API magic-link flow (same technique as prior stages): inserted a note, edited it (confirmed `updated_at` bumped), listed, then filled to exactly 10 and confirmed the 11th insert returns `{"code":"P0001","message":"notes_cap_reached"}` — the cap genuinely blocks, doesn't silently fail. Deleted all 10 via the delete policy, confirmed 204s and an empty list after. For RLS isolation, created two throwaway test accounts via the Admin API rather than touching any real third party's account: confirmed a second user's own list is empty and a filtered query for the first user's `user_id` returns nothing (blocked, not just unmatched); confirmed a spoofed insert (`user_id` set to a *different, under-cap* victim, deliberately not the already-capped first account, since that would have been masked by the cap trigger firing first) is rejected with `42501, new row violates row-level security policy`, and that no row landed under the victim's id. Caught mid-test that `service_role` had no table grant on `notes` at all (`SELECT` returned `42501` with Postgres's own hint naming the exact missing grant) — the identical bug class the standing rule exists for, just not caught by the RLS tests since the app never uses service_role for this table; added the grant anyway for consistency and fixed the migration file to match what's now live. Deleted both throwaway auth users afterward (cascades their rows via the FK) and confirmed the table is empty in production. `npm run build` clean, 115 pages; all 257 `lib/*.test.js` cases still pass (nothing in `lib/` changed, run only as a regression check). Deployed via `vercel --prod --yes`, confirmed `readyState: READY`; verified the tab genuinely landed in the compiled `/app` chunk (`grep` for `from("notes")` in `.next/static/chunks`) rather than trusting the client-component blind spot.

**Stage 75 (prior) complete — Session BD, marketplace dormanting: a pre-solicitor alignment change, not a teardown. Requite is provisionally candidate-pays only, with a permanent free tier, and does not operate an employer-side work-finding service. The reason is legal rather than product: the Conduct of Employment Agencies Regulations 2003 reg 26(6) publication exemption that the paid candidate tier may rely on is harder to sustain if we both charge candidates and run an employer work-finding service. Pending confirmation from a solicitor, so everything is dormant and reversible, nothing is deleted.

**Surveyed before touching anything.** Ten live user-facing employer references were found and reported before a single edit: the homepage nav "For employers" link, the whole `/hire` registration page, four separate Trust Panel locations (the G1 body, the "Introductions" row in the AI-transparency table, the candidate-pays card's marketplace paragraph, and both the "For employers" CTA and the footer link), the pricing page's "Register interest" card, Terms clause 2, and the Introductions section on the dashboard. Also confirmed `app/employer/page.js` is effectively unreachable: it redirects to `/hire` without an employer profile and is linked from nowhere in the candidate product.

**One flag, one file, one line to reverse it.** New `lib/flags.js` exports `MARKETPLACE_LIVE = false`, carrying the full reasoning in a comment so the next person in the file knows this was a decision and not neglect. `/hire` now renders a plain "Requite does not offer an employer service" stub; the entire registration form is preserved verbatim as an unreferenced `EmployerInterestForm` component in the same file. `TodayDashboard`'s `/api/candidate/intros` GET and the Introductions panel are both gated on the same flag. Verified in the built chunk that the fetch compiles to `e1.l && fetch("/api/candidate/intros")`, so the dashboard makes no intros request on load at all: structurally dormant, not merely dormant because the table is empty.

**`/hire` collects nothing, deliberately.** The form was still writing real rows to `employer_profiles` and `employer_roles` and promising follow-up email. Collecting personal data for a service we have decided not to pursue is a fresh UK GDPR problem (no purpose, no lawful basis, data with no plan behind it), not just a messaging one, so the form is not rendered at all rather than rendered and disabled. The stub makes no "coming soon" promise and offers no waiting list, because a promise is what would keep the data-collection question alive.

**Copy reframed to honest absence, not deferred launch.** Trust Panel G1 no longer describes a two-sided marketplace or `requite_managed` roles, and now states plainly that every role is a public listing and nothing in the feed was paid for by an employer. The "Introductions" row is gone from the AI-transparency table (there is nothing to be transparent about), the candidate-pays card says there is no employer service rather than that one is coming, and the "For employers" CTA and footer link are removed. Pricing's "We're building an employer side" card is replaced with a "Candidates only" statement. Terms clause 2 now states the absence factually and in detail. One deliberate change in the other direction there: the old clause asserted Requite "is not a recruitment agency or employment service", which is a legal self-characterisation and precisely the question counsel is answering, so it was replaced with a factual description of what does and does not happen.

**Expired-trial banner reworded, since the free tier is now legally load-bearing.** It read "Your 7-day trial has ended. Upgrade to keep using all features." in alarm red, implying the feed stops. It does not. It now says the user has moved to the free plan and that the feed and scored matches carry on at free limits, in the same amber as the expiring-soon state, and the CTA is "Compare plans" rather than an upgrade demand. The pre-expiry message also now says what happens afterwards. Free-tier permanence was re-verified structurally rather than assumed: `buildTabs()` does not gate Discover by plan, `feed-cache` has no tier or allowance check, `PlanGate` is used only for Interview prep and Contractor and keys on search mode rather than payment, and trial expiry produces a dismissible banner rather than a block.

**Left untouched and dormant, as instructed:** all of `app/api/employer/*`, `app/api/candidate/intros`, the network meter, `app/employer/page.js`, the intro email templates, and all five employer-side database tables. No migration was written and no table was dropped.

**Does NOT settle the EAA question.** Dormanting the marketplace narrows reg 26(6) but does not answer whether the paid candidate features themselves (CV tailoring, interview prep, recruiter search) are work-finding services under the Employment Agencies Act 1973. That remains a solicitor question and is recorded as such in `lib/flags.js` so the flag is never mistaken for a compliance conclusion.

**Self-tested:** `npm run build` clean; served every changed page from a real production server and confirmed `/hire` renders zero form inputs, no page anywhere links to `/hire`, and the word "marketplace" appears in no served HTML; verified the client-rendered pricing card and trial banner text directly in the compiled JS chunks, since `/pricing` and the dashboard bail out of SSR; confirmed the removed pricing card text is absent from the same chunk.

**Stage 74 (prior) complete — Session BC, the deep legal fix session: worked the full 110-finding claims register with a standing preference for REMOVING the thing causing the risk over defending it. Four structural removals, the whole consent order rebuilt, the legal pages completed factually, and six genuine judgement calls researched and written up as positions for counsel rather than guessed at. Nothing here self-certifies legal compliance: where the answer is a real legal judgement, the code was made as safe as possible around it and the residual question was written down precisely.

**1. Health-keyword extraction removed at the source (6-06, the standout finding).** The onboarding CV parser's own worked example told the model to return `"keywords": ["ADHD", "pain management"]`, and whatever it returned was shown to the user as chips, stored permanently on `profiles.hard_filters_json.cvKeywords`, re-sent to Anthropic on every later AI call via `buildAiContext()`, and fed into the deterministic match scorer. Special-category data by design, with no Article 9 basis anywhere in the chain. The whole feature is gone: stripped from the prompt, the response shape, the onboarding UI, the profile save path, `lib/ai-context.js`, `lib/scoring.js`, `lib/match-engine.js`, both wishlist routes and the Memory Card's editable field. Role fit now rests purely on `target_roles`, which onboarding already makes mandatory. Verified against five real production profiles and 400 real cached jobs that scoring quality holds: each profile's top matches are still genuinely relevant. Added a regression guard in `ai-context.test.js` so nobody reintroduces a keyword line, and a hard non-negotiable rule in the parse prompt forbidding extraction of health, disability, ethnicity, religion, sexual orientation, political or trade union data even where the CV states it outright. One nuance worth recording: a user typing "ADHD Nurse" as their own target role is fine and stays, because that is an occupation they chose to name, not an inference drawn about them.

**2. Referrals moved entirely client-side (3-09, 3-10, 6-14).** `contacts` held name, employer, past employers, relationship and free-text notes about third parties who never consented and in most cases will never know Requite exists, and `referral/draft` sent all of it to Anthropic in the US. No Art 14 route existed to notify them, and no realistic one could. New `lib/local-contacts.js` keeps the address book in the user's own browser; `ReferralsTab` no longer touches Supabase at all. The draft route was already stateless with respect to contact data (it only ever read the user's own profile), so a single contact's details are now sent transiently to write a message and never persisted anywhere. `supabase/migrations/016_retire_server_side_contacts.sql` drops both tables and is deliberately NOT applied: it is a destructive production change for Rob to run, though it is zero-data-loss since both were confirmed empty. The legal exposure is already closed by the application change alone. The tab's old assurance line, which claimed Requite never "sees or stores your contacts' own data beyond what you enter" while doing exactly that, is now true as written, and a notice explaining it sits BEFORE the contact form rather than in a footer underneath it. Trade-off accepted knowingly and documented in the file: the address book no longer syncs across devices and is lost if site data is cleared.

**3. Glassdoor and Working Families republishing removed (3-04, 3-05, 3-06, 3-07).** Drew the line where it actually falls: reproducing a third party's ratings for named companies in a paid product is their content and their database right; telling a user to go and look at Glassdoor themselves is ordinary editorial referencing and was kept. Removed: the hardcoded WLB rating and review count for 16 named employers in `shared.js`, the same on the homepage, the separate table in the WLB guide, the "Working Families Top Employer 2024" badges asserted on six companies' behalf, the derived composite score built partly on those ratings, and every rating badge on feed, contractor, wishlist and pipeline cards. Also removed the `analyse` prompt's instruction to recall Glassdoor sentiment and grade named employers on it, which was generating unverifiable and derogatory-capable output about real firms on every score, and the equivalent in the contractor-companies and wishlist prompts. Per Rob's decision the WLB tab survives on employer-published facts only (stated parental leave, stated office expectation, link to the employer's own policy page) with a visible "as at" date, and the `companyCulture` factor survives as 8-of-8 but scored strictly from what the advert itself says, returning an honest `found:false` where the advert says nothing rather than guessing.

**4. The consent order rebuilt so notice precedes processing (journey finding 6-18).** Previously nothing in the journey had notice before the processing it covered. New `components/ConsentGate.js` owns the decision and is the only thing that mounts anything non-essential: Analytics, Speed Insights and referral attribution now genuinely do not load until an explicit accept, and the pre-decision state deliberately behaves as a refusal. `CookieBanner` is presentation only now, with both buttons wired to the gate, so "Necessary only" actually refuses instead of writing a flag and hiding. Added `components/CookieSettings.js` on the Cookie Policy so the choice can be withdrawn as easily as it was given, which it previously could not be at all. Terms and Privacy acceptance now appears on the auth form, before submitting creates the account and the `handle_new_user` trigger provisions the profile. An AI-processing notice now sits above the CV textarea in onboarding, before anything is pasted or sent, naming Anthropic and the US, saying the step is optional, and asking the user to leave out sensitive detail.

**5. Everything else fixable in the register.** Adzuna attribution made compliant with the actual published requirement: their terms require the "Jobs by Adzuna" label at 116x23px (which it already was) AND the word "Jobs" hyperlinked to adzuna.co.uk (which it was not, it was a plain div). Homepage Pro/Max CTAs now route through `/pricing?checkout=...` instead of hardcoding `/auth`, so a signed-in user reaches checkout instead of a login page (2-16). Terms: the false live-marketplace description replaced with what the product actually is, the annual-billing claim removed since no annual path exists in the checkout route, an acceptable-use line added about third-party contact detail, and clause 7 now points at the real processor list. Privacy rewritten end to end as a factual description of the audited data flows: full processor list (Supabase, Vercel, Anthropic, Stripe, Resend), the US transfer disclosed as a fact, the client-side contacts position explained, the ICO complaint route and helpline added, self-serve export and deletion named. Email: sender address made configurable via `EMAIL_FROM` so the Resend sandbox domain can be replaced in one env change once DNS is verified, and every template now carries a real footer with sender identity, why this person is receiving it, an opt-out route and a privacy link, where previously none of the five had any. Cookie Policy rewritten with the complete storage table, split into strictly necessary, functional and optional, which previously listed two cookies and omitted every localStorage key.

**Deliberately NOT fixed, because they are legal judgements and not facts.** Six questions were researched against primary sources and written up as positions rather than guessed at: whether candidate-pays is lawful under the Employment Agencies Act 1973 (the research materially changed the picture, see the brief), whether Adzuna commercial use is defensible pending their written consent, the Consumer Contracts Regulations 14-day right against the stated 7-day refund, the VAT statement, the liability cap's fairness under the Consumer Rights Act 2015, and whether a privacy link is sufficient notice before CV upload. In each case the code was made as safe as it could be around the question and the residual was written down precisely. Two further items are Rob's own actions, not code: ICO registration, and confirming the Supabase project region in the dashboard, since the cached CLI token would not authenticate and a data-location claim should not be published on assumption.

**Self-tested:** all 257 `lib/*.test.js` cases pass including the new keyword regression guard; `npm run build` clean at 115 pages; scoring quality re-verified against real production profiles and real cached jobs after the keyword removal; the previously-open `/api/onboard/parse-cv` re-confirmed returning 401 unauthenticated; deployed and confirmed READY.

**Stage 73 (prior) complete — Session BB, urgent single fix: closed an open, unauthenticated, unmetered Anthropic proxy at `/api/onboard/parse-cv`, found in the journey-audit that followed Stage 1. `middleware.js`'s `protectedPrefixes` list only matches page routes (`/onboard`, not `/api/onboard/*`), and this specific route was the one AI-calling route in the whole app that read no user id and checked no allowance — a live, free, uncapped, unattributed billed Haiku call for anyone who found the URL, logged in or not.**

**Fix, not a middleware change.** The instruction as given suggested adding the route to middleware's protected-prefix list. Checked that first and deliberately didn't do it: middleware's redirect-on-no-session behaviour is designed for page navigation, and `middleware.js`'s own `config.matcher` (`/((?!_next/static|_next/image|favicon.ico).*)`) already covers every `/api/*` path, so adding `/api/onboard` there would have turned a JSON-API 401 into an HTTP 302 redirect to `/auth` for a `fetch()` caller expecting JSON — breaking the legitimate authenticated flow in `app/onboard/page.js` in the process of fixing the unauthenticated one. Instead mirrored the exact pattern every other AI-calling route in this codebase already uses (confirmed against `app/api/career-history/parse/route.js`, a working reference): an inline `getUser()` check via `createServerClient`/`cookies()` returning a real `401 {"error":"Sign in required"}` on no session, `checkAllowance(user.id, 'parse_career_history')` before the Anthropic call (429 with a clear message at cap, reusing the existing bucket that already exists specifically for "Haiku CV-to-structured-history parse (onboarding + re-parse)" — no new allowance category invented), and `trackAiUsage` logged via `after()` once the call succeeds. The parsing logic itself, the prompt, and the keyword-extraction behaviour were untouched — this was scoped as a plain security/cost bug fix only, not the separate special-category-data question the same route also raised in the journey audit (still open, tracked as its own item).

**Self-tested live against production, both directions, not just code-reviewed.** Deployed first, then: an unauthenticated `curl POST` to the live endpoint returned `401 {"error":"Sign in required"}` — confirmed the open proxy is closed. Minted a real session for a real production account (`robertjamesoxborough@gmail.com`, via the Supabase Admin API magic-link + verify flow, reconstructing the real `@supabase/ssr` cookie format — the same technique used for auth-gated route self-tests in earlier sessions) and POSTed a real CV excerpt: got back `200` with a genuine parsed result (role families, seniority, industries, salary hint, all evidenced from the pasted text). Queried `ai_usage` directly before and after: a new row landed at the exact moment of the call, `action:'parse_career_history'`, `model:'claude-haiku-4-5-20251001'`, correctly attributed to the real user id — proving the allowance check passed (the row could only land if `checkAllowance` returned `allowed:true` first) and the usage is now genuinely tracked where before it was invisible.

**Not separately re-verified: the cap-block path itself.** Confirmed by code inspection that the block logic is byte-identical to `career-history/parse`'s already-proven-working 429 path, and didn't burn through a real account's monthly quota just to watch it flip to blocked — five parse_career_history calls is the whole free-tier allowance, not worth spending on a test that inspection already confirms.

**Self-test infra:** all 257 `lib/*.test.js` cases pass (this route has no dedicated test file, and the fix touches no `lib/` code); `npm run build` clean, 115 pages; deployed via `vercel --prod --yes`, confirmed `readyState: READY`, aliased to `marker-silk.vercel.app`; committed (`4158bae`) and pushed to `origin/main`.

**Not done — the other half of the same finding, deliberately out of scope for this urgent pass:** the route's prompt still primes the model to extract health/disability-adjacent terms (its own worked example is `"keywords": ["ADHD", "pain management"]`), and whatever it returns is still shown on screen, stored permanently, and propagated into every later AI call with no Article 9 safeguard. That is a product/legal decision (remove the keyword-extraction feature entirely versus add a lawful basis and a post-filter), not a five-line bug fix, and stays exactly as flagged in the journey-audit register.

**Stage 72 (prior) complete — Session BA, copy-accuracy fix pass: every user-facing claim now matches the code and the live database, closing out roughly two-thirds of the Stage 1 claims-accuracy audit (92 findings). Scope was drawn tight and held to: copy only, on Rob's explicit item-by-item list. Stripe price IDs, refund/VAT/liability wording, the cookie banner's button logic, and the Glassdoor scoring prompt were all explicitly excluded and are tracked separately for a lawyer, a pricing decision, or a follow-up code session.**

**Pricing copy, four locations, one false claim.** "Unlimited AI" appeared on `app/pricing/page.js` (the plan card and the FAQ), `app/PricingSection.js` (the homepage pricing grid), and `app/trust/page.js` (the Honest Limits card) — `lib/allowance.js`'s real caps are Pro 1,000/analyse/month, Max 3,000. All four replaced with the real numbers. Max's own card said "Everything, uncapped." two lines above "3× higher limits" — a self-contradiction on the same card; retitled to "Higher limits, more headroom." Pricing's FAQ claimed "no trial period" while `lib/db.js:116` grants every new signup a real 7-day trial at Pro-level caps; fixed to disclose it. "Priority freshness: daily feed refresh" implied a tier-gated ingest that doesn't exist (the nightly crons are identical for every tier); corrected to what Pro actually buys, the 3-per-day on-demand Fresh Scan. Two claims backed by no code at all — "Advanced filters and profile controls" (nothing is tier-gated) and "First access to new features" (no staged-rollout mechanism exists) — were removed rather than reworded, since neither could be made true by rewording. Added the limits users were most likely to hit with zero warning: recruiter_search (0/5/20), the daily fresh-scan cap, and the shared `web_search` pool `lib/allowance.js` itself documents as "the real binding ceiling in practice, invisibly" — now disclosed on the pricing page as a plain-English note.

**Trust Panel — the page whose entire premise is "every claim is backed by a coded invariant" was, on inspection, the least accurate page in the app.** G1 described a live two-sided marketplace in the present tense ("you can request a real introduction") against a database holding zero `employer_profiles`, zero `employer_roles`, zero `intro_requests` rows — rewritten to the same honest pre-launch framing already standing on `/hire` ("built but not live yet... no requite_managed roles on the platform today"). "Intro receipts: an immutable, timestamped record" was checked against the actual delete chain: `intro_receipts` cascades from `candidate_employer_matches`, which cascades from `auth.users`, and `account/delete/route.js` calls `auth.admin.deleteUser` — so the record is not immutable, it is deleted with the account. Reworded to "kept for as long as your account exists." "Your pipeline is the default landing screen" was checked against `app/app/page.js:67` (`useState('Today')`) and found false — Today is the default, Pipeline is one tab over; both the claim and its own "Built in:" evidence line were corrected. The G2 cache-cycle claim ("a larger cache cycles through in full over a few nights") was checked against real numbers — 40 links per nightly batch against 6,232 cached rows is roughly 156 nights, not "a few" — and the specific, indefensible timeframe was removed rather than replaced with another number that would just as quickly go stale. The AI-transparency table said CV generation runs on Haiku; `cv/generate/route.js` actually runs Sonnet for standard/deep and Haiku only for cover letters and the quick mode — corrected. It referenced a non-existent "Analyse tab" (scoring lives in Today); corrected. The closing superlative, "The only AI recruitment platform honest enough to show you how it works," was flagged in the audit as unsubstantiable and self-contradicting Terms' own "not a recruitment agency" line — per instruction this was removed outright rather than rewritten, replaced with "Every claim above is backed by code, not marketing." The 24-hour support SLA was softened to an honest best-effort, since it is one sole trader, not a team with a ticket system.

**Homepage and in-app copy.** Removed a referral-reward promise ("there's a thank-you in it for both of you") that directly contradicted `/hire`'s own "No referral reward exists yet" — no reward mechanism exists anywhere in the code, `referrals` holds zero rows. Softened an absolute freshness guarantee ("you're only ever spending effort on roles that are actually open") to describe the real, bounded nightly-batch mechanism instead of a guarantee the 40-link cron can't back. De-narrowed the front door: the hero kicker ("For senior professionals...") and the "Who it's for" section header both predated the product going universal (Stage 68's profession-agnostic matching, Stage 70's full Adzuna category sweep) and were softened to drop the senior-only framing; same fix applied to `RecruiterPanel.js`'s "senior permanent roles" line. Removed the single highest-risk sentence found in the whole audit: a daily-tip claiming roles that score well "have a much higher interview rate than average" — a performance claim about Requite's own product with zero outcome data behind it (no hire tracking, no interview tracking anywhere in the schema). Removed four more unsourced statistical claims from the same daily-tips array and rewrote the CV/ATS tip to match `lib/cv-lint.js`'s own stated framing (the file's header explicitly records "the 75% auto-rejected by ATS claim is a myth" and instructs UI copy to stay consistent with that — the old tip leaned the other way). Renamed "ATS check" to "Format & keyword check" in both CV panels, since the mechanism is a structural-parsing + keyword-overlap check that tests against no real applicant tracking system, and the audit's spot-check confirmed the myth-claim itself appears nowhere else in the app. Fixed a fabricated data source on the WLB tab footer ("employer surveys" — no survey mechanism exists anywhere in the codebase; the real sources are Glassdoor and published company disclosures, which is what the line now says).

**`lib/articles.js` — bigger than the audit estimated.** The audit flagged "~9 statistics attributed to named orgs, no citation anywhere" and "~50 sentences with corrupted punctuation." Both were re-verified from scratch rather than trusted at face value. A script parsing every `text`/`detail`/`label`/`subtitle`/`excerpt` string literal for paren-balance found 60 genuinely unbalanced instances (not ~50), confirming a past em-dash-purge script had replaced em dashes with a stray, unmatched `(` or `)` depending on which side of the dash it fell — e.g. `"competitive salary" (a signal that..."` where the original almost certainly read `"competitive salary" — a signal that...`. All 60 were read in full context (not pattern-matched blindly, since some parens are legitimately matched asides and touching those would have been a real regression) and hand-fixed to real punctuation (colon, comma, or a clean sentence break), verified by re-running the paren-balance scan against the edited file until it returned zero. Separately, seven statistics carrying a named, real organisation (Timewise, LinkedIn, McKinsey, Ladders Inc, the Work Foundation at Lancaster University, Stanford, Matthew Walker) with no citation anywhere in the repository were removed as whole array entries, including one — Matthew Walker's UC Berkeley sleep research — that was cited a second time in body prose, not just its stat card, and was reworded rather than deleted outright since the surrounding paragraph still made a defensible point once the specific researcher/institution claim was removed. Four more precise-sounding but unnamed statistics (15 weeks unpaid overtime, 14 weeks Director search time, 72% of career opportunities from past colleagues, and Korn Ferry's "1 in 3") were judgement calls: Korn Ferry is a named real org and was removed on the same principle as the other seven; the other three carry no named source at all and were left in place (punctuation-only fixed) since they were outside the audit's explicit scope — flagged for Rob's own call rather than silently removed. Verified after: `node -e` syntax check confirms valid JS and all 20 articles intact, zero remaining em dashes, zero remaining named-org citations, zero remaining unbalanced parens, and `npm run build` compiled all 20 `/notes/[slug]` pages clean.

**Doc consistency — Cookie Policy directly contradicted Privacy and the banner.** `app/cookies/page.js` claimed "Requite uses only essential cookies... no analytics cookies" while both `app/privacy/page.js` (usage data "collected via Vercel Analytics") and `components/CookieBanner.js` ("We use performance analytics (Vercel Analytics)...") correctly say analytics is used. Per instruction, only the Cookie Policy's copy was changed, not the banner's button logic (both buttons still write the same consent flag and hide the banner — that mismatch between the banner's copy and its actual behaviour is Rob's to fix in a dedicated session). Cookie Policy rewritten to state plainly that Vercel Analytics and Speed Insights are used, that they don't set cookies (genuinely true — Vercel Analytics is cookieless), and why that's a materially different claim from "no analytics." Privacy and Cookies dates bumped to September 2026 (previously May and July respectively, and Privacy hadn't been touched in roughly 20 stages of shipped features). No legal content was rewritten in Privacy — the real gaps found while reading it (no lawful basis stated per Art 13(1)(c), no international-transfer mechanism named for Anthropic, no mention anywhere that the referrals feature makes Requite a controller of third-party contacts' data, no processor list, no geographic address) were left exactly as flagged in the audit register, for a solicitor.

**Also fixed, in the same files while editing them:** the brand's standing zero-em-dash rule, which had drifted in roughly 25 more live, rendered strings beyond the ones the audit specifically named (tooltips, button labels, score-reason strings in `lib/match-engine.js`, duplicate-detection labels) — swept and fixed to comma/colon/middot per the existing separator convention used elsewhere in the app, code comments and AI-prompt-internal text left untouched since neither is user-facing.

**Self-tested:** all 257 cases across 13 `lib/*.test.js` suites pass, including `match-engine.test.js` after the three reason-string edits (confirmed no test asserts exact reason-string wording before touching them). `npm run build` clean, 115 pages. Deployed via `vercel --prod --yes`, confirmed `readyState: READY`, aliased to `marker-silk.vercel.app`. Because `/pricing` is a client component that bails out of SSR, its text isn't in the raw server HTML — verified live by fetching the compiled JS chunk directly and confirming "AI job scoring: 1,000 per month" and "Recruiter search: 5 per month" are present while "Unlimited AI job scoring" and "no trial period" are absent. Homepage and Trust Panel changes verified the same way via direct HTML grep (both are server-rendered). Committed (`42a0905`) and pushed to `origin/main` — this session's standing instruction, after the prior session found the repo could silently drift stages behind without an explicit push.

**Not done — carried forward, exactly as scoped out by Rob, not silently dropped:**
- Stripe price IDs (`lib/stripe.js` still has a literal `price_TODO_max_monthly` and a Pro price ID confirmed invalid against the live Stripe account in a past session) — checkout cannot currently complete for either paid plan. Code fix, not copy.
- Refund window (7 days vs the Consumer Contracts Regulations 2013's 14-day right), VAT statement, liability-cap fairness under the Consumer Rights Act 2015, and the missing geographic address — all flagged CRITICAL/HIGH for a solicitor in the audit register, none touched.
- The cookie banner's button logic (`components/CookieBanner.js`) — both buttons still do the same thing regardless of choice; only the Cookie Policy's description of reality was fixed to stop contradicting itself, not the mechanism.
- Adzuna licence compliance (83% of `jobs_cache` by volume, outside commercial terms per Rob's own prior confirmation), the referrals feature's GDPR exposure (currently zero rows, so zero live exposure, but no code or Terms change was made to prevent it going forward), Glassdoor's terms/database-right question, and ICO registration — all lawyer-flagged in the audit register, none attempted here since this session was copy-only.
- Three unnamed-source statistics in `lib/articles.js` (15 weeks unpaid overtime, 14 weeks average Director search time, 72% career-opportunities-from-past-colleagues) left in place, punctuation-fixed only, since they carry no named organisation and were outside the audit's explicit removal list — flagged for Rob's own read rather than unilaterally removed.

**Stage 71 (prior) complete — Session AZ, UX plan step 5: the collapsed nav is now the default — Focus Mode's opt-in 3-tab shape retired in favour of making the STANDARD nav calm for everyone. The last piece of the calm-front-door rework: primary tabs (Today, Pipeline, Discover) always visible, everything else (Aggregator, WLB, CV, Interview, Referrals, Contractor, Profile) behind a "More" toolkit reveal — nothing removed, nothing hidden permanently, every tab still one tap away.**

**What existed before.** Focus Mode (opt-in via a header toggle + `localStorage['mkr_focus']`) already proved a 3-tab calm shape works: Score/Track/Find, rendering stripped-down composite views (`<EngineTab stripped />`, `FocusPipelineView`, `<FeedTab defaultSubTab="find" />`) instead of the real Pipeline/Today/Discover tabs. Standard mode, meanwhile, still showed the full flat bar of up to 10 tabs (`buildTabs()`: Today, Pipeline, Discover, Aggregator, WLB, CV, Interview, Referrals, Contractor, Profile, depending on plan/searchMode) to everyone who hadn't discovered and switched on Focus Mode — almost certainly most users, since it was opt-in.

**The split, and why it maps to Focus Mode's own shape.** Primary = `['Today', 'Pipeline', 'Discover']`, computed once and never hardcoded a second time — toolkit = `TABS.filter(t => !primary.includes(t))`, so a tab added to `buildTabs()` in the future automatically lands in the toolkit rather than silently disappearing from the nav. This is deliberately the SAME three legs as Focus Mode's old Score/Track/Find (the daily driver + track progress + find new roles — the core loop), which is both the literal instruction ("Focus Mode's existing collapse mechanism already proves this works technically — build on it") and independently confirmable from `FIRST_RUN_STEPS` (`shared.js`), which only ever references `Today` and `Discover` across all three onboarding steps — the product's own first-run tour already treats these as the load-bearing tabs.

**Critical distinction from Focus Mode: primary tabs render the REAL components, not stripped ones.** Focus Mode's Score/Track/Find used reduced composite views (`stripped` EngineTab losing pipeline-context sections, `FocusPipelineView` losing kanban columns/bulk actions/search/tidy-up, in place of the real Pipeline tab). Making THAT the default would have been a functional regression for every standard-mode user. Instead, the primary row now renders exactly the same Pipeline/Today/Discover tab content that was already there — the nav bar's shape changed, not what any tab does. `FocusPipelineView`/`FOCUS_STAGES` are retired unused in `page.js` but left completely untouched and still exported from `shared.js`, same discipline as `JourneyBar`'s Stage 61 retirement — reversible by restoring one render branch and the toggle button.

**A real coupling found and protected before it could break.** Grepped every cross-tab navigation call in the codebase (`onTabSwitch`/`setTab` — the standard, nav-shape-agnostic pattern, used everywhere else) and found one exception: `FeedTab.js`'s "WLB guide →" nudge switches tabs via `document.querySelector('[data-tab="WLB"]')?.click()`, a direct DOM lookup, not a prop callback. Collapsing WLB into a conditionally-UNMOUNTED toolkit panel would have silently broken this nudge the moment "More" was collapsed by default — the query would find nothing, and the button would become a dead click with no visible error. Fixed at the nav-composition layer only, zero changes to `FeedTab.js`: the toolkit reveal panel is rendered in the DOM at all times (every toolkit button, including WLB, keeps its `data-tab` attribute permanently) and toggled with `display: moreOpen ? 'grid' : 'none'`, never `{moreOpen && (...)}` conditional unmounting. A hidden element still fires its click handler when `.click()` is called on it programmatically, so this preserves the nudge exactly, without FeedTab ever needing to know the nav reorganised around it.

**Built:**
1. **Header tab row** (`app/app/page.js`) — `primaryTabs.map(...)` renders Today/Pipeline/Discover exactly as before (same styling, same ⓘ tooltip affordance), followed by a `More ▾` button. When the active tab is a toolkit tab, the button shows that tab's own name instead of the bare word "More" (e.g. "CV ▾"), so nobody loses track of where they are — and picks up the same rainbow-underline active-tab styling (`s.tabActive`) primary tabs already use.
2. **Toolkit reveal panel** — a responsive grid (`repeat(auto-fill, minmax(150px, 1fr))`) of toolkit tabs, each showing its name and the existing one-line `TAB_TOOLTIPS` description (reused directly, no new copy needed except one addition — `Profile` had no entry before, added: "Everything Requite knows about you — your CV, target roles, and preferences. Edit anything."). Clicking a card sets the tab and closes the panel in one action.
3. **Bottom mobile nav** (`s.bottomNav`, <768px) — collapsed the same way: primary tabs (Today/Pipe/Find) + a `More` button sharing the identical `moreOpen` state with the header (never a second, duplicate toolkit panel). Tapping it also scrolls to top, since the header — where the panel actually renders — is `position: sticky; top: 0` and therefore always reachable there regardless of scroll position.
4. **Focus Mode retirement** — the header toggle button, the `focusMode`/`focusTab`/`toggleFocusMode` state, and the two content branches (`{focusMode && (...)}` / `{!focusMode && (<>...</>)}`) all removed from `page.js`. `FOCUS_STAGES`/`FocusPipelineView` untouched in `shared.js`; `mkr_focus` localStorage key simply stops being read (existing values are harmless, inert data).

**Not built this session, correctly out of scope per the explicit instruction:** contextual/progressive reveal (CV surfacing automatically once a role reaches Applied, Interview at Interviewing) — flagged as a genuine follow-up, not attempted. The toolkit reveal built here is a flat, always-the-same-order list; making it context-aware is a distinct, separable piece of work.

**A real, build-time-invisible bug caught before it could ship — the exact class this codebase has been burned by before.** After the first pass of edits, `npm run build` reported success even though `focusMode`/`focusTab` were still referenced in two JSX blocks whose state declarations had just been deleted — a plain `ReferenceError` waiting to fire the moment a real browser rendered the page, invisible to Next.js's build step because `page.js` is a client component (`'use client'`), and undefined identifiers inside JSX for a client component only throw at runtime, never at bundle time. Caught by grepping the file for the retired identifiers immediately after the first build "succeeded" rather than trusting the green build alone; found two live JSX blocks still referencing them, removed both, and confirmed programmatically (a small Node script counting occurrences of every retired identifier across the whole file) that all five (`focusMode`, `focusTab`, `toggleFocusMode`, plus the two now-orphaned import names) hit exactly zero real-code occurrences before rebuilding and calling it done.

**Self-tested, code-level (no login held — same standing constraint since Stage 60):** full `npm run build` clean; all 13 `lib/*.test.js` suites pass (nothing in `lib/` touched, this is pure `page.js`/`shared.js` nav composition); traced `buildTabs()`'s output through all three plan branches (`perm`/`contractor`/`both`) by hand and confirmed the primary/toolkit split lands correctly and CV/Interview/Contractor's existing plan-gating is completely undisturbed (`resolveSearchMode`/`buildTabs` themselves were never edited); confirmed every retired identifier (`focusMode`/`focusTab`/`toggleFocusMode`) has zero remaining code references and `FOCUS_STAGES`/`FocusPipelineView` each have exactly one reference (the retirement comment, not code); confirmed `data-tab="WLB"` remains present in the DOM tree unconditionally, protecting `FeedTab.js`'s existing nudge with zero changes to that file; full manual structural read-through of the entire updated `page.js` (JSX fragment boundaries, div nesting) end to end.

**Not self-tested — same standing constraint carried since Stage 60:** an actual live browser click-through. Full checklist given to Rob directly (see chat).

**Stage 70 (prior) complete — Session AY, UX plan step 6: universal feed coverage. Stage 68 already made the MATCHING mechanism universal (the crons widen from real user `target_roles`); this stage found and fixed a genuinely separate problem — the SOURCE query list itself is still finite and hand-picked, so a real profession nobody had typed yet got zero nightly coverage regardless of how good the matching mechanism was. Diagnosed with real API calls against real sources first, fixed generically (Adzuna's own category taxonomy, not a hand-picked list of more professions), and reported honestly rather than declared solved.

**DIAGNOSIS — real queries against real sources, not assumed.** Mapped every feed source: `cron/adzuna` (the main Discover feed — role-text queries via `lib/aggregate-role-queries.js`'s floor + Stage-68 widening from real `target_roles`), `cron/gov` (public-sector only, opt-in via `wantsGov`, same widening pattern), `cron/contract` (contractor-mode only, same pattern), and the 4-ATS-board pull (`lib/ats.js`, 43 named companies via Greenhouse/Lever/Ashby/SmartRecruiters — inherently company-specific, used for wishlist-company tracking, never intended as a broad discovery source; out of scope for "universal coverage" for that reason, not neglected).

First checked the real, live `jobs_cache` table for 8 deliberately ordinary/awkward professions (management accountant, HR business partner, chef, plumber, care worker, primary teacher, warehouse manager, farm manager) plus a `product manager` control. Result: chef and warehouse manager had reasonable real coverage; management accountant/HR business partner had adjacent-but-not-exact coverage; **plumber and farm manager had zero cached rows at all**; **care worker's only "matches" were false positives from a crude substring search** (a teaching-assistant listing, an unrelated "Director of Brand" role) — genuinely zero real care-worker coverage; **primary teacher had coverage only at Headteacher/Deputy Head level**, zero ordinary classroom-teacher listings.

Then tested whether this was a SOURCE gap or a QUERY gap, using real live Adzuna API calls (a temporary, clearly-labelled diagnostic route deployed to production for the duration of this investigation only, since local `.env.local`'s `ADZUNA_API_KEY` had gone stale — identical to `ADZUNA_APP_ID`, a genuine local-env drift unrelated to production, which has its own correct key; Vercel Sensitive vars can't be read back to confirm/compare, so this ran server-side on the real deployment instead of guessing — deleted immediately after use, confirmed gone via a live 404 check post-redeploy, never part of the shipped code). Real result, every single profession: **Adzuna itself has enormous genuine UK volume** — 27,178 "management accountant" listings, 86 "HR business partner", 22,257 "chef", 8,173 "plumber", 27,583 "care worker", 24,003 "primary teacher", 2,978 "warehouse manager", 277 "farm manager" — all with genuinely on-topic sample titles (real "Management Accountant", "Primary School Teacher", "Farm Manager" postings, not noise). **Conclusion: the source was never thin. The nightly batch simply never asked Adzuna for these professions**, because they weren't in the hand-picked floor and no real user had yet typed them as a `target_role` — the exact chicken-and-egg gap Stage 68's own widening mechanism cannot close on its own for a profession's very first user.

**THE FIX — generic, not another hand-picked list.** Confirmed live that Adzuna exposes its own category taxonomy (`GET /v1/api/jobs/gb/categories`): 29 real, Adzuna-maintained sectors (accounting & finance, IT, sales, customer services, engineering, HR & recruitment, healthcare & nursing, hospitality & catering, PR/advertising/marketing, logistics & warehouse, teaching, trade & construction, admin, legal, creative & design, graduate, retail, consultancy, manufacturing, scientific & QA, social work, travel, energy/oil/gas, property, charity & voluntary, domestic help & cleaning, maintenance, part-time, other/general — `unknown` excluded as literal uncategorised junk). Confirmed live that querying by `category` alone, with **no "what" text filter at all**, returns genuinely relevant, high-volume, real listings (e.g. `trade-construction-jobs` → 63,718 total, `domestic-help-cleaning-jobs` → 40,883, `social-work-jobs` → 24,444, real named employers throughout). This is the qualifying difference from "add more sector boards": it is Adzuna's own complete, Adzuna-maintained list queried in full, every category, every night — nobody at Requite has to name, anticipate, or maintain a profession list for its sector to be covered, and no profession is structurally excluded by construction the way a hand-picked list always eventually excludes someone.

Implemented in `cron/adzuna` only (the main Discover feed — the one surface actually meant to be a universal primary source; `cron/gov`/`cron/contract` stay deliberately narrow-purpose and opt-in, unchanged): new `ADZUNA_CATEGORIES` constant (`lib/aggregate-role-queries.js`), `fetchAdzuna()` generalised to accept `category` as an alternative to `what`, and a new category-sweep loop runs after the existing role-query loop every night, sharing the same `adzuna-${job.id}` external_id scheme (a listing already caught by a role-text query simply de-dupes on upsert, same reasoning `cron/contract` already uses for its own shared IDs) and the same downstream pipeline — no new seniority/quality filter added at ingest, exactly consistent with how the existing role-query rows already work (candidate-agnostic quality and per-user relevance are `scoreJobsBatch`/`scoreMatch`'s job downstream, not the ingest cron's).

**Cost guardrails**: reserves `ROLE_QUERIES.length + 29` calls up front via the existing `reserveAdzuna({kind:'cron'})` before firing a single request — worst case ~73 calls for `cron/adzuna` alone (24 floor + up to 20 widened + 29 categories), plus `cron/gov`/`cron/contract`'s existing worst-case ~33/~34, totals to ~140 calls/night, comfortably inside the 220/day cron ceiling (`lib/adzuna-budget.js`, unchanged) with real headroom to spare. Category pages capped at 30 results (vs 50 for role-text queries) since the goal here is universal breadth, not depth per sector; still one shared nightly batch, zero per-user live spend, same 3-day prune as every other Adzuna row.

**Honest limitations, not claimed away**: (1) the actual `jobs_cache` "after" state depends on tonight's real scheduled run (`vercel.json`: `0 3 * * *`, 03:00 UTC) — the cron endpoint is correctly `CRON_SECRET`-protected (confirmed live: unauthenticated request returns 401) and this session does not hold that secret, so no attempt was made to force an immediate live run by working around the auth boundary; the diagnosis above is real live-API evidence that the fix is sound, but the literal before/after `jobs_cache` comparison completes on the next natural cron run, not inside this session. Rob can confirm tomorrow with the same `jobs_cache` query used in the diagnosis above. (2) Whether a "Primary School Teacher"-level (non-senior) listing that now enters the cache via the `teaching-jobs` category sweep actually SURFACES prominently to a given user still depends on `scoreJobsBatch`'s candidate-agnostic quality scoring, which this product's "for senior professionals" positioning may reasonably rank lower than a Head of Department role — that is a ranking/positioning question, a different axis from the coverage question this stage was scoped to fix (does the row exist in the cache at all), and it is not touched here. (3) ATS boards remain company-specific by nature (43 named companies) — this was never the universal-coverage source and stays exactly as it is, unrelated to this fix.

**Not fully self-tested — same honest limit as above**: the live jobs_cache before/after for these exact professions after a real category-sweep run. What WAS verified live: the diagnosis (real Adzuna volume per profession), the fix mechanism (real category-only queries return relevant results), the code (clean build, no test regressions — nothing in `lib/*.test.js` touches this path), and the deploy (confirmed READY, confirmed the temporary diagnostic route is gone from production via a live 404 check).

**Stage 69 (prior) complete — Session AX, the referrals engine: candidate-side network outreach. Targets the channel that actually produces 30-50% of hires, not the ~3% cold-application channel most tools optimise for. New "Referrals" tab plus reveal points from a role's own detail (Pipeline card / Today's Best Opportunity).

**Hard guardrails held, explicitly**: no paid people-search or hiring-manager-by-name lookup was built (deliberately out of scope, not attempted then removed) — this stage is entirely candidate-side, working the user's OWN entered contacts; no employer/marketplace mechanic introduced anywhere; nothing hardcoded from any founder profile — contacts, companies, and career context all derive from the user's own `contacts` rows, `career_history`, and pipeline, with zero seed data shipped.

**New tables (migration `015_referrals_engine.sql`, GRANTs from the start)**: `contacts` (name, company, past_companies, relationship, last_contacted_at, notes — a simple personal address book, RLS `user_id = auth.uid()`, direct client CRUD via the browser Supabase client, no API route needed for basic reads/writes) and `referral_requests` (contact_id, job_id nullable — null means speculative, message_type, drafted_message, status, plus timestamp columns per stage). Both GRANT SELECT/INSERT/UPDATE/DELETE to `service_role, authenticated` alongside their RLS policy, matching migration 001's exact proven per-user-table pattern (not the append-only pattern used for `interview_stage_notes` — a referral record legitimately gets edited/advanced by its own owner, so full CRUD is correct here, unlike a historical interview note). Explicitly NOT the same table as the pre-existing `public.referrals` (Requite's own refer-a-friend growth programme) — named `contacts`/`referral_requests` to avoid any confusion between the candidate's professional network and Requite's own affiliate mechanic.

**Speculative vs role-specific, kept genuinely separate**: `referral_requests.job_id` is nullable specifically so "building a relationship at a dream company" (no live role) and "reaching the right person for a role already in the pipeline" are different data shapes, not one flow awkwardly forcing a fake job. The UI (`ReferralsTab.js`) surfaces this as two distinct entry points per contact: a company-matched role chip (role-specific) or a standalone "Speculative outreach" link (no role) — never conflated.

**Company matching**: reused `lib/job-match.js`'s existing `normaliseCompany()` verbatim rather than writing a second normaliser — exactly the suffix-stripping, exact-post-normalisation-equality approach already proven for pipeline dedup, so "Monzo" correctly matches "Monzo Bank Ltd" while "King" correctly does NOT match "Kingfisher" (verified live against real strings, see self-test below).

**AI-drafted messages, 5 genuinely different briefs**, one Haiku call per draft (`app/api/referral/draft/route.js`), gated by a new `referral_draft` allowance (Free 0 / Trial+Pro 20 / Max 50 — deliberately self-limiting anyway, since a real request only ever touches a handful of real contacts, not hundreds):
- `warm_referral` / `reconnect_ask` — role-specific, only valid BEFORE a role reaches Applied.
- `speculative_outreach` — no role attached, explicitly instructed to ask for a conversation, never a job.
- `nudge` / `intel_request` — the substitute once a role IS Applied/Interviewing/Offer: a low-pressure heads-up or a request for process intel, never a referral ask.

**The forbid rule is enforced server-side, not just hidden in the UI** (`MESSAGE_TYPES[type].allowedWhen(job.status)` in the route, re-validated on every request regardless of what the client sent): `warm_referral`/`reconnect_ask` reject with a clear explanation once `job.status` is in `['applied','interviewing','offer']`; `nudge`/`intel_request` reject in the reverse direction (job not yet applied); `speculative_outreach` rejects if a job is attached at all. "Match whatever effort pattern the CV flow now uses" was interpreted as: a single-shot message-type selector (the natural analogue of CV's role-type selector) with one Generate call per type, deliberately WITHOUT a depth/effort axis — a 60-120 word message doesn't have a meaningful "quick vs deep" distinction the way a CV document does; flagging this interpretation in case a different pattern was actually meant.

**Deterministic message lint** (`lib/message-lint.js`, new, same verification-not-trust philosophy as the Stage 65 CV lint): checks for an em dash character and a defined list of common AI-tell phrases (delve, moreover, leverage, seamless, robust, cutting-edge, etc.) — flag-only, always runs, zero AI cost, shown to the user as a calm "✓ Clean" or a specific flagged-phrase list, never silently trusted from the prompt instruction alone.

**Status tracking**: `sent → responded → agreed → referred`, plus `declined`/`no_response` as terminal exits from any point — the `agreed` state is a genuine, distinct stage in the UI (not collapsed into "responded" or "referred"), specifically closing the gap the brief named between "they said yes" and "the referral actually landed."

**Integration**: `PipelineCard` gets a new "Ask for a referral" button (`onAskForReferral` prop, same call pattern as the existing "Tailor CV" button); Today's "Best opportunity today" card gets the same action alongside "Apply now"/"View in pipeline"; both thread a `referralPrefill` (mirrors the existing `cvPrefill`/`prepPrefill` pattern in `page.js` exactly) into the Referrals tab, which auto-opens the draft panel for the best company-matched contact if one exists. The tab itself (`buildTabs` in `shared.js`) is available on every plan/search-mode combination — referrals apply equally to permanent and contractor search, so it was added unconditionally rather than gated like CV/Interview.

**Self-tested live, real data, zero mocks**: inserted a real contact ("Priya Sharma", company "NatWest") against Rob's real account, matched against his real pipeline row for "NatWest Group" — confirmed `normaliseCompany('NatWest') === normaliseCompany('NatWest Group')` is `true` via the actual imported function, and confirmed the King/Kingfisher negative control stays `false`. Ran a real Haiku call through the exact live prompt logic using Rob's real `career_history` (Meta, Sony/PlayStation, NatWest, King, Google) as candidate background: the drafted warm-referral message genuinely read as a real, casual message ("Hi Priya, Hope you're doing well! It's been a while since we worked together on the digital transformation stuff at NatWest back in 2016-2018... No pressure at all if the timing's off your end. Cheers, Rob") — referenced the real relationship years, the real target role, and naturally worked in the real subsequent Meta/Sony career without inventing anything; passed the message lint clean. Ran the exact forbid-rule validation function through all 6 real branch combinations: warm_referral correctly passes pre-application and correctly fails once `status:'applied'` with a clear redirect to nudge/intel; nudge/intel correctly pass post-application and correctly fail pre-application; speculative_outreach correctly fails with a job attached and correctly passes without one. Cleaned up the test contact afterward (this table supports normal DELETE, unlike Stage 67's append-only notes table) and confirmed the live `contacts` table is empty as found. Clean production build, `/app` bundle +3 kB (438 kB). All 13 `lib/*.test.js` suites pass (nothing in `lib/` changed except the new, additive `normaliseCompany` reuse and the new `lib/message-lint.js`, neither touching existing tested behaviour). Deployed via `vercel --prod --yes`, confirmed READY and the newest production deployment, live at `marker-silk.vercel.app`.

**Not self-tested — same standing constraint carried since Stage 60**: an actual live browser click-through. Layman spot-check for Rob: (1) Referrals tab → add a real contact at a company you're tracking → confirm a "Matches: [role]" chip appears if you have a live pipeline role at that company; (2) click a match chip → pick "Warm referral ask" → Draft message → confirm it reads like something you'd actually send and reflects the relationship details you entered; (3) move that same pipeline role to Applied, then try drafting for the same contact → confirm only "Post-application nudge"/"Ask for intel" are offered, not the referral ask; (4) from a Pipeline card, click "Ask for a referral" → confirm it jumps to the Referrals tab with the right contact/role pre-selected if a match exists; (5) after drafting, click "Mark as sent" → confirm it appears in "Tracked asks" and the status buttons walk through Responded → Agreed → Referred correctly.

**Not touched, as instructed**: no employer-side code, no paid lookup service, no seed/founder contact data anywhere in the codebase.

**Stage 68 (prior) complete — Session AW, the "built for anyone with a CV, not one favoured field" audit. Two parts: a full copy sweep of every profession-example in the product, and an honest, code-level verification of whether tailoring is genuinely open-vocabulary or secretly depends on a profession being on a list somewhere. Real findings in both halves — this stage found and fixed things, it did not just confirm the product was already fine.

**PART 1 — COPY SWEEP.** Every profession-example instance found across the codebase, before/after:
- `app/onboard/page.js` (no-CV job-title placeholder): was fixed `"e.g. Ward Sister, Secondary School Teacher, Qualified Electrician"` → now rotates daily across 7 sets of 3 (`JOB_TITLE_EXAMPLE_SETS` in `lib/onboarding-options.js`), spanning clinical/education/trades/legal/finance/hospitality/logistics/retail.
- `app/onboard/page.js` (career-summary placeholder): was fixed `"e.g. 12 years in acute nursing..."` → now rotates daily across 7 full example sentences (`CAREER_SUMMARY_EXAMPLES`), one each for retail, clinical, trades, legal, education, hospitality, logistics.
- `app/settings/page.js` (career-summary placeholder): was fixed `"e.g. 10 years in digital marketing... Series B startup..."` → now uses the same `CAREER_SUMMARY_EXAMPLES` rotation as onboarding, closing a drift the two pages already had a documented history of (see `lib/onboarding-options.js`'s own header comment on why it's the single source of truth).
- `app/app/shared.js`'s `GettingStartedPanel` quick-profile-build mini-form (3 fields: title, skills, highlight): was fixed, all three nursing-themed (`"Senior Sister at an NHS trust"` / `"clinical leadership, triage, staff training"` / `"Cut ward incident rate by a third"`) → now rotates daily across 7 coherent single-person examples (`QUICK_PROFILE_EXAMPLES`) spanning nursing, trades, hospitality, legal, logistics, marketing, education — each rotation shows one internally-consistent persona, not three unrelated fields at once.
- Homepage hero's floating "score card" (`app/page.js`): was a permanently fixed single example, `"Staff Product Manager at Monzo, 9.2"` — the single most prominent piece of copy on the entire site, always the same tech/product example for every visitor. Extracted into a new client component (`components/RotatingScoreCard.js`, same rotation pattern as the existing `RotatingLifestyle.js`) that now rotates daily across 7 genuinely different sectors: tech/product (Monzo), clinical (Guy's and St Thomas', Senior Sister), construction trades (Balfour Beatty, Site Electrical Supervisor), corporate law (Clifford Chance, Senior Associate), logistics (DPD, Regional Logistics Manager), hospitality/retail ops (Pret A Manger, Regional Operations Manager), and education (Ark Schools, Head of Science) — each with its own plausible factor-row values, not the same generic numbers reused.
- `app/app/tabs/BalancedTab.js`'s "Best titles for senior + balanced" list: all 6 entries were corporate/office titles (Programme Manager, Partnerships Lead, Digital Strategy Manager, etc.) with no non-office equivalent. Judgement call, stated plainly rather than silently made: the underlying advice ("avoid 'Growth' in the title", "structured delivery, rarely on-call") is genuinely about corporate title semantics that don't transfer to clinical/trades/education, where WLB signals come from shift patterns and staffing ratios instead — forcing a fabricated "best titles" tip for those fields would be worse than an honest scope caveat. Added one: "Title semantics below are specific to office/corporate roles — in clinical, trades, and education, WLB signals come from shift patterns, staffing ratios and term structure rather than the title itself." Flagging this choice for Rob to override if a genuine trades/clinical equivalent list is wanted instead.
- Swept and found clean (no changes needed): `lib/score-jobs-batch.js`'s sector-calibration prompt (already covers tech, financial services, retail/hospitality, public sector, manufacturing, media, healthcare, education, professional services, energy, charity, insurance, telecoms, PR, housing, logistics, aviation — genuinely broad, not marketing/healthcare-skewed, with an explicit own "not a personalised judgement" anti-bias reminder); `lib/articles.js`/guides pages; `app/employer/page.js`/`app/pricing/page.js`/`app/trust/page.js`; all other tab/modal placeholder text.

**PART 2(a) — VERIFYING THE MECHANISM, NOT JUST OUTCOMES.** Read the actual role-suggestion, target-company, and feed-query code, not just tested outputs. Genuinely open-vocabulary, confirmed by reading the code: `app/api/onboard/parse-cv/route.js` (role families/industries are AI-derived from the actual CV text; the example lists in the prompt are explicitly documented and code-verified as shape-only, never filtered against); `app/api/wishlist/generate/route.js` and `app/api/contractor/companies/route.js` (target companies are AI-derived directly from real CV/profile text, explicitly instructed to reflect the person's OWN sector, no company list anywhere in the code); `lib/match-engine.js`'s `scoreRoleFit` (the primary feed-relevance scorer — pure word-overlap on the user's own free-text target_roles vs job titles, works for literally any string); `lib/role-keywords.js` (already explicitly documented "deliberately sector-agnostic... rather than assuming a fixed profession list", used by `feed-gov` and `wishlist/check`); the main Discover feed's on-demand fresh scan (`feed-web`'s `runFreshScan` literally uses the user's own `target_roles` text as the live Adzuna query); the public-sector fresh scan (`feed-gov`'s `buildGovQueries`, same principle). `lib/onboarding-options.js`'s field/role dropdowns are convenience suggestions only — confirmed a free-text `customRole` field exists in onboarding, so a user is never limited to the dropdown.

**Two real, previously-undiscovered gaps found and fixed, not just outcomes reassured**:
1. `app/api/cron/contract/route.js` — the nightly contract/interim-role feed was a FIXED, closed 14-item list of white-collar titles (Interim Finance Director, CFO, Programme Manager, Marketing Director, etc.) with **zero** widening from real user data, unlike the main `cron/adzuna` feed which already pulls real `target_roles` from profiles (a Stage-unknown-prior fix). A contractor in any field not on that list — an interim ward sister, a locum anything, a self-employed tradesperson going interim — got literally zero contract-feed coverage regardless of what they'd actually typed, purely because this file never read a profile. Fixed: `lib/aggregate-role-queries.js` refactored to a shared `widenRoleQueries` helper; new `buildContractRoleQueries()` pulls real target_roles from contractor-mode profiles (`searchMode`/`openToContract` derivation, same logic used everywhere else in the codebase), deduped against the floor, capped at 20/night — mirrors the main feed's already-fixed pattern exactly.
2. `app/api/cron/gov/route.js` — same class of gap: the nightly public-sector feed's 18-item query list had zero widening, unlike its own sibling live fresh-scan (`feed-gov`) which already personalises. Fixed: new `widenGovQueries()` pulls real target_roles from profiles that have opted into public-sector roles (`wantsGov === true`), capped at 15/night. **Verified against real production data, not synthetic**: one of the 5 real profiles with `target_roles` set is a nurse (`"Advanced nurse practicioner"`, `"nurse"`, `"ADHD Nurse"`) with `wantsGov: true` — before this fix, her own words were completely invisible to this cron; the widening now genuinely includes exactly those three real, lowercased phrases as live Adzuna queries.

**A third, more fundamental issue found only by stress-testing the mechanism itself (Part 2b) — this is the more important finding of the two**: a real linguistic/word-form gap in the deterministic matching layer itself (`lib/match-engine.js`'s `jaccardOverlapSignificant`, `lib/role-keywords.js`'s `titleMatchesRoleKeywords`) — both do exact-word overlap with no stemming, so an AI-suggested role family in one grammatical form ("Marine Pilotage", "Arboricultural Consulting", "Bookkeeping") failed to match the real job-title form of the exact same profession ("Harbour Pilot", "Consultant Arborist", "Bookkeeper"). This is NOT a profession-rarity bias — it hit a portfolio worker's mainstream bookkeeping role exactly as it hit two "niche" cases — it is a general limitation that could degrade matching for genuinely any user whenever a suggested label and a real listing happen to use different suffix forms of the same word. Found live via the stress test below, not theorised. Fixed: added `stemWord()` (`lib/role-keywords.js`, a small dependency-free suffix stripper, not a full stemmer) and wired it into both `titleMatchesRoleKeywords` and `jaccardOverlapSignificant` as an additional fallback (exact match still tried first). New `lib/role-keywords.js` also converted from ES `export` to `module.exports` (the rest of `lib/` already uses this; needed so `match-engine.js`, a plain-CJS file used directly by the Node test runner, could `require()` it). New test file `lib/role-keywords.test.js` (11 cases) added, including negative controls confirming genuinely unrelated professions (a gardener vs a software engineer) still correctly do NOT match — the fix adds recall without adding false positives. Full test suite now 13 files, all pass.

**PART 2(b) — STRESS TEST, real professions chosen to be awkward, not the obvious ones (explicitly not nurse/teacher/electrician)**: ran 8 deliberately diverse, difficult real-shaped CVs through the REAL onboarding CV-parse prompt and REAL target-company prompt (exact prompt text copied from the live routes, real Haiku calls, no mocking), plus the real deterministic feed-matcher against a plausible real job title for each:

| Case | Role families derived | Target companies (sample) | Deterministic feed match |
|---|---|---|---|
| Self-employed sole trader (garden designer, no employer) | Garden Design / Landscape Management / Horticulture Business Owner | RHS, Crocus.co.uk, Landscape Institute, regional landscape contractors | ✓ correct |
| Portfolio worker (bookkeeper + yoga instructor + proofreader) | Bookkeeping / Small Business Finance / Wellness Operations — correctly captured all 3 threads, not just one | RIBA Enterprises, wellness chains, independent publishers | ✗ then ✓ after the stemWord fix above |
| Mid-career changer (10yr touring cellist → data analyst) | Data Analytics / Business Intelligence / Retail Analytics — correctly prioritised the current direction, didn't get anchored on the unrelated music history | John Lewis, Sainsbury's, Unilever, Sky, Tesco | ✓ correct |
| Niche: Court Stenographer | Court Reporting / Legal Operations / Case Management | TypeOut, Voscribe (real UK court-reporting-tech firms), several law firms | ✓ correct |
| Niche: Arborist (consultant, BS5837 surveys) | Arboricultural Consulting / Tree Surveying / Local Authority Operations | Barton Willmore, LUC, Capita Property and Infrastructure, named local authority | ✗ then ✓ after the stemWord fix above |
| Niche: Marine Pilot (Port of Southampton) | Marine Pilotage / Harbour Operations Management / Maritime Safety / Port Operations | ABP, Peel Ports, Port of London Authority, **Trinity House** (genuinely correct, specific UK deep-sea pilotage authority) | ✗ then ✓ after the stemWord fix above |
| Unusual channel: Offshore Wind Turbine Technician | Wind Turbine Technician / Site Operations Manager / Technical Training Specialist | Ørsted, SSE Renewables, EDF Renewables, Siemens Gamesa | ✓ correct |
| Military-to-civilian (Royal Engineers Major → PM) | Project Management / Programme Management / Infrastructure Delivery / Construction Management | Balfour Beatty, Atkins, Network Rail, Costain | ✓ correct |

None of these 8 role-family sets or company lists came from a fixed list anywhere in the codebase — verified by reading the code (Part 2a), not assumed from the outputs looking plausible. Company suggestions were specific and genuinely sector-accurate throughout (Trinity House and TypeOut/Voscribe in particular are correct, non-obvious real answers a generic/lazy model would not produce), never defaulted to generic tech names for any of the 6 non-office cases.

**Honest summary, as instructed — not reassurance**: the CORE mechanism (AI role/industry/company derivation, and the primary word-overlap feed scorer) is genuinely open-vocabulary by construction and was already mostly built that way in earlier, unlogged work. This stage found three real, concrete exceptions rather than confirming a clean bill of health: two closed-list gaps in nightly cron feeds (contract, gov) with zero widening, and one genuine word-form blind spot in the matching layer itself that a fixed keyword list could never have caught because it isn't about which professions are named, it's about English suffix grammar. All three are now fixed and self-tested against real production data or real API calls, not synthetic fixtures.

**Self-tested**: PROGRESS.md build clean, all 13 `lib/*.test.js` suites pass (12 pre-existing unaffected + 1 new). Real production profile data (5 rows) fetched directly and run through the actual widening algorithms to confirm the contract/gov fixes activate on genuine user data, not hypothetical rows. Real 8-profession stress test via real Haiku calls against the exact prompts the live routes use. Real fix verification: ran the actual (not simulated) `titleMatchesRoleKeywords`/`scoreMatch` functions post-fix against all 3 failing pairs, confirmed all now match with sensible scores (8/10, "strongly aligns"). Clean production build. Deployed via `vercel --prod --yes`, confirmed READY and the newest production deployment, live at `marker-silk.vercel.app`.

**Not fixed, flagged instead**: `BalancedTab.js`'s "best titles" list scope caveat (see Part 1) — a judgement call to add an honest caveat rather than fabricate advice for fields whose title semantics don't transfer; open to being overridden. The nightly cron widening caps (20/night for contract, 15/night for gov, 20/night pre-existing for the main feed) are a real, honest scaling limitation worth naming: at current real user volumes this is a non-issue, but if genuinely new distinct roles ever exceeded the cap in one night, some would not get a query slot until a later run — not a profession bias, a batch-size ceiling that exists for cost reasons (Cost Guardrails Rule 1).

**Stage 67 (prior) complete — Session AV, the interview prep overhaul. The biggest feature in the sequence: four features on the existing `/api/interview-prep` route and `PrepTab.js`, upgraded not forked, plus a Today reveal so an interview no longer has to be hunted for in a tab.

**FEATURE 1 — Scripted "Say:" answers.** Every likely question now leads with a ready-to-speak script (`say`), not a themes list: first person, spoken rhythm, 100-200 words (45-90 seconds aloud), grounded only in the candidate's real CV/career history and the pack's own STAR stories — never a fabricated fact, exactly the same never-invent discipline this project already holds CV generation to. The underlying STAR skeleton survives as a `framework` field behind a per-question "Vary it" toggle in the UI, for anyone who'd rather build their own answer than use the script.

**FEATURE 2 — Stage memory.** New `interview_stage_notes` table (migration `014_interview_stage_notes.sql`, GRANT included from the start — SELECT + INSERT only, deliberately no UPDATE/DELETE, enforced at the database level so a note can never be edited or removed by any path through the app, not just by route discipline). Genuinely append-only, unlike `pipeline_items.score_breakdown_json` which gets wholesale-upserted on every job save (`lib/db.js`'s `jobToRow`) and would silently clobber incrementally-added notes — the exact lesson this instruction named from the personal tracker. New `app/api/interview-prep/stage-notes/route.js` (GET list / POST append) using the established service-role-write pattern (same as `channel_checks`). The main route now loads all prior notes for the job before building a pack and folds them into the prompt with an explicit instruction that they **outrank the JD** ("a JD is what they wrote before they met you"); when notes exist, `tldr.whatsChanged` carries a genuine "what's changed" line. The forwards-only pipeline move is handled without any AI guessing: a deterministic banner appears whenever prep is being built for a role still marked `considering`/`to_apply`/`applied`, offering an explicit "Move to Interviewing →" button — never silent, never backward.

**FEATURE 3 — Two-mode assistant (Prep / Live).** Prep mode is the existing open-ended Sonnet pack build. Live mode reads `pack.liveCues` — 8 to 12 quick-tap cue cards generated in the SAME pack-build call and cached in the pack itself, so tapping one is a pure client-state read, zero network call, zero marginal cost, by construction. A free-text fallback covers the rare question the cues don't: a new, small Haiku-only call (`mode: 'live'` on the same route), no web_search, capped to 250 output tokens, gated by a brand new `interview_prep_live` allowance (Free 0 / Trial+Pro 40 / Max 100 — deliberately far more generous than `interview_prep`'s own 8/month cap, since one real interview can mean many quick taps in one sitting) so it can never compete with or exhaust the pack-build cap. Live mode's cue panel resets to expanded every time the Prep/Live toggle is switched to Live, so it can never be left accidentally tucked away mid-interview.

**FEATURE 4 — Polish.** A TLDR hero (black card, always visible, never scrolls away) leads every pack: `roleSummary` (what the role actually is), `decidingFactors` (the 3-4 things that actually decide the hire, as lime chips), `landThis` (the one thing to land above all, highlighted), and `whatsChanged` when stage notes exist. Below it, a sticky section nav (Essentials / Company / Alignment / Questions / Stories / Ask them / Watch outs / Checklist) jumps instantly within the scrollable pack — no more one long scroll to hunt through. "Streaming first" was interpreted as a structural/display-order priority (TLDR is the first thing built into the JSON and the first thing rendered) rather than literal token-level SSE, since this codebase has zero prior streaming infrastructure anywhere and introducing it for one feature was judged a disproportionate lift versus a second AI call or a display-order fix — flagged here explicitly in case true streaming was actually wanted.

**INTEGRATION — reveals from Today.** `TodayDashboard.js` gets a new, dedicated, unmissable "Interview coming up" section (its own black card, "Interview coming up for [role] — prep now →") the moment any pipeline role hits `interviewing` status — promoted out of the old single-slot "Next action" nudge system, which could bury it behind five other lower-stakes conditions. Clicking "Prep now" threads a `prepPrefill` (mirroring the existing `cvPrefill` pattern in `page.js`) straight into `PrepTab`, pre-selecting that role. The tab itself is unchanged as a standalone destination — still fully reachable as a tool, not only via the reveal.

**COST GUARDRAILS.** The pack-build Sonnet call keeps its existing two gates untouched: `checkAllowance(userId, 'interview_prep')` (feature cap) and `checkAllowance(userId, 'web_search')` (the Stage 64 shared pool — no new bucket created, exactly as instructed, since the pool exists precisely so a feature like this doesn't need one). The Live free-text Haiku call is gated by the new `interview_prep_live` allowance and logged under that same action name. Live mode's cached cues cost nothing per tap by construction (client-state read, no route call). The pack-build's static system prompt (`SYSTEM_STABLE`, ~1330 estimated tokens, candidate-agnostic) is cache-controlled exactly as it always was; self-tested live and confirmed both `cache_creation_input_tokens` and, on a repeat call, `cache_read_input_tokens` firing for real — comfortably clears Sonnet's real ~1024-token floor (the ~4096 figure specifically measured for Haiku in this project doesn't apply here, since this call is Sonnet).

**A genuine live bug found and fixed during self-testing, not a hypothetical**: Sonnet 5 auto-invokes extended thinking on a prompt this complex even when not asked (the same behaviour already noted in `cv/generate` and `onboard/parse-cv`), and on the first real test run it consumed 6,096 of a 6,000-token budget, truncating the strict-JSON response before it could close — a hard failure for JSON in a way markdown output tolerates by degrading gracefully. Fixed by explicitly passing `thinking: { type: 'disabled' }` (a real, documented Anthropic API parameter this codebase hadn't used before) rather than just budgeting around it, and separately sizing `max_tokens` to 14,000 after a second live run showed the genuine JSON payload (9 scripted questions + 4 stories + 12 live cues + 3 prose sections) runs to ~9,700 output tokens on its own even with thinking off.

**Self-tested live, full flow, real data, zero mocks**: inserted a real stage note ("[screening] Recruiter flagged no direct regulated financial services experience...") against one of Rob's own real `pipeline_items` rows via the actual table; read it back exactly as the GET route would; ran the real pack-build prompt construction with Rob's real profile/CV/career history (12,652-char real CV, 6 real career-history rows) for a Senior Partnerships Manager role at Monzo, with the note present. Result: `tldr.decidingFactors` and `tldr.whatsChanged` both directly reflected the planted note's exact concern ("the lack of direct regulated financial services partnerships experience... will almost certainly probe that gap hard") — proving prior-stage notes genuinely reach and reshape a later-stage generation. The first scripted `say` answer was first-person, spoken rhythm, ~180 words, built from Rob's real Meta/Facebook Play EMEA experience, and honestly flagged the real gap rather than papering over it ("I'll be honest, I haven't negotiated a regulated banking-as-a-service contract specifically..."). 12 `liveCues` generated (within the 8-12 spec), one directly addressing the planted gap. A real Live free-text Haiku call answered a follow-up question grounded in the same real background, 107 output tokens, well under the 250 cap. Cleaned up the test note afterward via the Supabase Management API's direct SQL (a deliberate superuser bypass of the table's own append-only GRANT, used only for test cleanup — the application itself never has this path). All 12 `lib/*.test.js` suites still pass (0 failures; nothing in `lib/` changed). Clean production build, `/app` bundle +3 kB (432 kB). Deployed via `vercel --prod --yes`, confirmed READY and the newest production deployment, live at `marker-silk.vercel.app`. Confirmed the live `interview_stage_notes` table is empty post-test, exactly as found.

**Not self-tested — same standing constraint carried since Stage 60**: an actual live browser click-through. Layman spot-check for Rob: (1) move any pipeline role to Interviewing status, then check Today — confirm a black "Interview coming up for [role] — prep now →" card appears near the top, and clicking it jumps straight into a pre-selected Interview tab; (2) in the Interview tab, generate a prep pack for a real role — confirm it leads with a black TLDR card (not a number), a sticky row of section jump-links, and each likely question shows a "SAY:" script with a "Vary it" link that reveals a framework outline underneath; (3) add a note in "Notes from this process", regenerate the pack, and confirm the TLDR's "What's changed since the JD" line reflects it; (4) switch to Live mode — confirm the cue cards expand by default, tapping one reveals its answer instantly with no loading spinner, and typing a fresh question into the free-text box returns an answer in a few seconds; (5) confirm the "Move to Interviewing" nudge appears when prepping a role still at an earlier pipeline stage, and disappears once moved.

**Not touched, as instructed**: the negotiation-prep path (`mode: 'negotiate'` in `PrepTab.js`) — completely unchanged, still its own markdown result; `lib/channel-urls.js`/the Aggregator/`lib/match-engine.js` — untouched, unrelated to this stage.

**Stage 66 (prior) complete — Session AU, the verdict card + competitiveness/desirability split. Pure presentation reshape of a scored role, applied consistently across every surface scoring happens: Today's Best Opportunity, the Feed's SCORE result, Aggregator's bring-in results, and the Engine scorer's result panel. Replaces the old raw-number/factor-grid-first display with one shared component (`VerdictCard` in `app/app/shared.js`) used identically in all four places — zero new AI calls, everything computed from data `/api/analyse` already returns.

**FEATURE 1 — Verdict card.** A scored role now leads with `buildVerdict(job)`: the model's own `signalReason` (already a plain-English fit-plus-gap-plus-recommendation sentence per the analyse prompt) promoted to the top of the card, ahead of any number. A deterministic fallback (no AI) synthesises a verdict from factor scores/labels alone — "X is the strongest part of this match. Y is the notable gap, worth weighing before you commit." — for the rare case `signalReason` is missing (an older pipeline row). No new AI call either way; this is a display reorder plus an honest reuse of reasoning the scorer already produces.

**FEATURE 2 — Competitiveness vs desirability split.** `computeVerdictSplit(factors)` regroups the existing 8 AI-scored factors into two axes, both pure averages of scores already on the job: **competitiveness** (would you WIN it) = `roleSkillsMatch` + `seniorityFit` + `industryFit`; **desirability** (would you WANT it) = `officeFlexibility` + `companyCulture` + `paternityLeave` + `salaryMarket` + `careerGrowth` — every one of the 8 factors used exactly once, none double-counted, none dropped. Each axis is banded high (≥7) / mid (≥5) / low (<5), matching the app's existing score-tier colour convention (`ScoreBadge`, `FeedTab`'s `csBg`) rather than inventing new thresholds, and the 3×3 band combination maps to one honest one-line read (`SPLIT_READS`) — e.g. high desirability + low competitiveness: "A role you'd love but you're a stretch on. Apply if you're up for the challenge." Deliberately never lets a high overall score hide a low competitiveness reading underneath it.

**One component, four surfaces, no duplicated logic.** `VerdictCard` (`app/app/shared.js`) renders: identity (company/role, when given) → verdict text → the desirability/competitiveness split with its one-line read → scannable fact chips (office days, salary-vs-market note, overall score, signal) — score demoted from a leading 26-36pt number to just one fact chip among several, exactly as briefed. A `compact` prop trims sizing for list-row contexts; a `showFactors` prop (Engine scorer only) appends the full existing 8-factor `FactorBar` breakdown beneath, demoted from where it used to lead rather than removed. Wired in: **`EngineTab.js`** — replaced the old two-block score+signal card and separate factor-breakdown card with one `<VerdictCard job={result} showFactors />`, real-time chips (web search used, live salary estimate) kept as a secondary row beneath it. **`TodayDashboard.js`** — replaced the "Best opportunity today" hero's big score badge + signalReason box + top-3-factor chips with `<VerdictCard job={{...bestJob, factors: bestFactors}} />`, `buildWhyBullets`' existing strengths-only bullet list kept underneath (a different, complementary artefact — positive-only bullets vs the verdict's honest fit-and-gap framing). **`FeedTab.js`** — the SCORE button's response now also captures `factors`/`signalReason`/`officeDays` (previously discarded), and a compact `VerdictCard` renders beneath the tags row once a card is scored, replacing the bare signal chip; the small corner score badge is kept for at-a-glance scanning across a list of many cards, a distinct need from the single-role decision surface. **`AggregatorTab.js`** — each `'done'` bring-in result gets a compact `VerdictCard` beneath its one-line scan row (kept for comparing up to 10 batch-scored results at a glance).

**Deliberately out of scope**: Today's compact multi-item "Opportunities" list (5 recent roles, one line each) — turning 5 list rows into 5 mini verdict cards would violate the Stage 61 calm-front-door principle ("no columns, no cards... keep it a summary"); left as a bare score chip + status, unchanged. `lib/match-engine.js`'s deterministic feed/aggregator relevance ranking is untouched — it's invisible sort math a user never reads as a value in itself, not a "scored role" in the sense this feature means (that's always the AI-scored `factors`/`signalReason`/`score` shape from `/api/analyse`).

**Self-tested live**: a real Haiku call scored a genuine B2B-SaaS-to-consumer-retail sector-pivot scenario (JD: Senior PM, consumer retail app; candidate: 6 years B2B enterprise SaaS PM, zero consumer/retail experience) through the exact same JSON schema `/api/analyse` uses, returning `score:6, signal:maybe, signalReason:"Strong PM fundamentals and seniority match the role, but complete absence of consumer retail or e-commerce experience is a significant gap for a consumer-focused position."` — genuinely a fit-plus-gap sentence, no synthesis needed. Ran the exact `buildVerdict`/`computeVerdictSplit` logic from `shared.js` against the real factor output: verdict promoted signalReason verbatim to the top; split computed `Desirability 6.6 · Competitiveness 6` → "Middling on both counts. Neither a standout nor a write-off." (industryFit=3 pulled competitiveness down despite roleSkillsMatch=7/seniorityFit=8, an honest reflection of a genuine sector-pivot stretch); fact chips rendered `3d office · "£75k-£90k range is transparent..." · 6/10 overall · MAYBE`. Confirmed all four display sites (`EngineTab.js`, `TodayDashboard.js`, `FeedTab.js`, `AggregatorTab.js`) import and render the identical `VerdictCard` component with the identical synthesis functions — structural consistency by construction, not four hand-matched implementations. All 12 `lib/*.test.js` suites pass (0 failures, none of this touches lib/ — a pure `app/app/` presentation change). Clean production build, `/app` bundle +1 kB (429 kB), zero warnings. Deployed via `vercel --prod --yes`, confirmed READY and the newest production deployment, live at `marker-silk.vercel.app`.

**Not self-tested — same standing constraint carried since Stage 60**: an actual live browser click-through. Layman spot-check for Rob: (1) Engine tab (or Today → "Score a new role") → analyse any real job URL → confirm the result now leads with a sentence, not a number, followed by a "Desirability · Competitiveness" box with its own one-line read, then small chips (office days/salary/score/signal) below that, with the full 8-factor breakdown further down still available; (2) Feed tab → hit SCORE on any card → confirm the same verdict+split box appears beneath the card's tags, not just a bare number; (3) Aggregator → "Bring in roles to score" → paste a couple of links/JDs → confirm each scored result shows the same verdict+split beneath its one-line scan row; (4) Today tab, if you have a scored pipeline role → confirm "Best opportunity today" leads with a verdict sentence and the split box, not the old giant score number.

**Stage 65 (prior) complete — Session AT, the deterministic CV lint + CV two-phase/role-type selector. Two features built together on the same `/api/cv/generate` pipeline: mechanical, post-generation verification that the CV actually holds the structural and keyword promises the generator makes (replacing the model's own self-report, which this project has already proven worthless once), plus a role-type selector (Permanent/FTC/Contract) and an optional ask-first two-phase toggle.

**FEATURE 1 — Deterministic CV lint.** After every JD-tailored or contractor CV generation, the route now mechanically re-parses the CV it just produced and asserts three things instead of trusting the model claimed them: no leaked internal scaffolding markers (`lib/cv-lint.js`'s `SCAFFOLD_MARKER_RE`, same shape as `lib/cv-extract.js`'s own marker), standard section headings present, and role entries formatted as separate title/company/dates lines (the Workday rule) — all three built entirely on `classifyCv()`'s already-tested parser (`lib/cv-docx.js`, 44 existing test cases), zero AI calls, always runs. A fourth rule — no tables, no multi-column layout, no header/footer text — is asserted as a static architectural fact rather than checked per-generation: confirmed by a full read of `buildCvDocx()` that it only ever constructs `Paragraph`/`TextRun`, never `Table`/`Header`/`Footer`, so this can never be false regardless of input. On top of the free structural check, a cheap gated Haiku call (`runKeywordLint` in `route.js`) compares the JD's genuine requirements against the CV that was actually generated (not the original base CV) and reports matched/missing keywords plus an estimated match percentage — gated by a new `cv_lint` allowance (Free 5 / Trial 30 / Pro 30 / Max 60, sized above every tier's own `cv` cap so it never becomes the bottleneck on a legitimate generation) and skipped gracefully, never blocking the CV response, if the allowance is exhausted or the call errors. Both halves combine into one `atsCheck` object surfaced calmly in both `DirectCvPanel` and `ContractorCvPanel`: `"✓ Clean structure · ✓ 8/14 key terms present · 6 worth adding: SQL, fintech, A/B testing"`. Deliberately framed as a confidence signal ("clean parsing + strong keyword match"), never as "beat the robot that bins you" — the 75%-auto-rejection claim is a myth and appears nowhere in the copy.

**FEATURE 2 — Role-type selector + two-phase toggle.** A new ROLE TYPE row in `DirectCvPanel` (Permanent / FTC / Contract, CV generation only, not cover letters) sits above the existing DEPTH selector, untouched. Permanent and FTC set a `roleType` sent to `/api/cv/generate`, which now injects a role-type framing note into the standard/deep prompts alongside the existing track-tone note (FTC: "frame achievements to show adaptability and fast, self-sufficient delivery within a defined contract period"). Selecting Contract routes away entirely, via a new `onSelectContractor` callback threaded from `CvTab` — it switches `CvTab`'s existing `section` state to `contractor_cv`, reusing the contractor path built in Stage 62 rather than duplicating any logic; only offered when `searchMode !== 'perm'` (i.e. the Contractor CV tab actually exists for this user). Below DEPTH, an optional "Ask me a few quick questions first" checkbox (default off) implements the two-phase toggle as a plain binary, deliberately NOT the retired L1/L2/L3 graduated-question picker: when on, the first click sends `{askQuestions: true}` to the same route, which short-circuits before any generation prompt is built and returns 2-4 genuinely non-obvious clarifying questions from a cheap Haiku call (`runClarifyingQuestions`), gated by its own new `cv_questions` allowance (same Free 5 / Trial 30 / Pro 30 / Max 60 numbers as `cv_lint`) so it can't be spammed for free ahead of ever generating. The user's answers feed straight into the EXISTING `answers`/`answersSection` plumbing in `route.js` that has been present but unused since before this session — no new answer-handling code needed on the server, just a real client finally populating it. A "Skip →" link bypasses the questions and generates immediately.

**Self-tested live** (all three pieces, real API calls, no mock): structural lint against a hand-built clean CV — passes with `{ok:true}`; against the same CV with `---JD REQUIREMENTS---`/`---TAILORED CV---` scaffolding spliced in — correctly flags the leak; against a CV with inline "Job Title, Company (Dates)" formatting instead of Workday's separate-line rule — correctly flags it; against a CV missing all section headings — correctly flags it. Real Haiku keyword-match call against a real Senior Product Manager JD (payments/fintech, SQL, A/B testing, mentoring requirements) and a CV that covered stakeholder management and agile delivery but not SQL, A/B testing, fintech, or mentoring — returned `matched: 8, missing: 6 (SQL, fintech, A/B testing, mentoring junior PMs, 5+ years, data-driven decision making), matchScore: 62` — genuinely discriminating, not rubber-stamping. Real Haiku clarifying-questions call against the same JD/CV pair returned three specific, non-generic questions (which SQL/analytics example to lead with, how many PMs mentored and what they achieved, which parts of the payments experience were hands-on vs tangential) — none of them answerable from the material already given, confirming the "don't ask what's already obvious" instruction held. All 12 `lib/*.test.js` suites pass (0 failures), including the pre-existing 44-case `cv-docx.test.js` that the structural lint depends on but does not modify. Clean production build, zero warnings. Deployed via `vercel --prod --yes`, confirmed READY and the newest of 3 recent production deployments, live at `marker-silk.vercel.app`.

**Not self-tested — same standing constraint carried since Stage 60**: an actual live browser click-through. Layman spot-check steps for Rob: (1) CV tab → AI Generate → pick a role with a JD → note the new ROLE TYPE row (Permanent/FTC, plus Contract if you're in "both" or "contractor" search mode) and confirm clicking Contract switches you to the Contractor CV tab; (2) back on AI Generate, tick "Ask me a few quick questions first", click Generate — confirm 2-4 questions appear before any CV is written, answer one or two, click "Generate with answers" and confirm a CV comes back (or click "Skip" instead and confirm it generates immediately); (3) after any Tailored CV or Full rewrite generation, look for a new "ATS check:" line above the verified-stats banner — confirm it reads as a calm confidence signal, not a scary red wall, and that the "worth adding" keywords genuinely look missing from the CV when you eyeball it against the JD.

**Not touched, as instructed**: the existing DEPTH selector (quick/standard/deep) and its meaning, cover letters (no role-type/two-phase UI added there), the L1/L2/L3 legacy stats labels in `StatsTab.js` (dead but left alone), `lib/cv-docx.js`/`lib/cv-extract.js` themselves (only read from, to build the lint on top).

**Stage 64 (prior) complete — Session AS, the shared web_search allowance pool. Cost-guardrail infrastructure, invisible to the user, built ahead of the planned referrals/interview-prep-overhaul features so they draw on it from day one instead of getting their own unbounded bucket.

**The problem, found by grepping every `web_search` reference in the codebase**: exactly 5 routes make a real `web_search_20250305` tool call (`contractor/recruiters`, `perm/recruiters`, `analyse`'s JD-fetch fallback, `contractor/companies`'s live refresh, `interview-prep`) — every OTHER `web_search` hit in the codebase (`FeedTab.js`'s job-source tag, `job-feed`/`cron/wishlist-scrape`'s comments about a fallback that was deliberately removed) is not a real call. Those 5 routes were gated by 3 independent buckets (`recruiter_search` shared by the two recruiter routes, `analyse_search` shared by analyse's fallback and contractor/companies, `interview_prep` on its own) that had never been checked against each other. Max-tier worst case if a user maxed all three in one month: 20+200+30 = 250 web_search-capable calls, each up to 4 searches → up to 1,000 searches/month (~$10 in raw search fees alone, before Sonnet tokens) from one user, while every individual cap still reported "within limit".

**The fix, confirmed with Rob before building** (proposed numbers, reasoning, and the full 5-route list given first, per his explicit ask): one new `web_search` action added to `TIER_CAPS` in `lib/allowance.js` — **Free 3** (matches Free's only existing web_search grant exactly, a pure ceiling not a new grant), **Trial 30 / Pro 30** (mirrors this codebase's existing trial=pro convention for every other action), **Max 60**. Reuses the exact same `checkAllowance`/`ai_usage`/`trackAiUsage` machinery as every other action — no new table, no new counting logic, and fail-closed-on-query-error is inherited for free since `checkAllowance` itself was untouched. Each of the 5 routes now does an ADDITIONAL `checkAllowance(user.id, 'web_search')` check right before its web_search-enabled call fires (on top of, not instead of, its existing feature-specific cap), and an ADDITIONAL `trackAiUsage({..., action: 'web_search', ...})` log alongside its existing one after a successful call. Existing per-feature caps and their "X of Y left" UI messaging are completely unchanged — genuinely invisible infrastructure, the shared pool is simply the real binding ceiling in practice now. Any future web_search feature (referrals, the interview-prep overhaul) only needs to check/log against this one shared action to be cost-safe by construction; it doesn't need — and per this design, shouldn't get — a bucket of its own.

**Graceful degradation on exhaustion** (never a silent failure): each route's new check returns its existing response shape with a clear message — "You've used your web searches for this month (N/cap). Upgrade for more, or it resets on the 1st." `analyse` degrades to its deterministic score with the message in `signalReason` (same pattern it already uses for its own `analyse_search` cap); `contractor/companies` falls back to the cached list with the message attached (same pattern as its own existing cap).

**Self-tested**: `lib/usage-window.js`'s real `windowStart`/`periodFor` (unmodified) driving a simulation of `checkAllowance`'s exact decision rule against a hand-built row set — confirmed two different features (2 recruiter_search + 1 interview_prep calls, each also logging a `web_search`-tagged row) correctly sum toward the same shared cap; confirmed a third, different feature filling the pool to exactly the cap correctly flips `allowed:false`; confirmed a further call from yet another feature stays blocked once the cap is reached; confirmed a non-web_search action (`analyse`) is completely unaffected by pool exhaustion. `lib/allowance.js`'s real `web_search` numbers (3/30/30/60) grep-verified directly against the file. Full existing 11-file test suite still passes, zero regressions. Clean build from scratch — exit 0, zero warnings, `/app` bundle unchanged (server-only change, no client impact). Deployed via `vercel --prod --yes`, confirmed READY, live at `marker-silk.vercel.app`.

**Not touched, as instructed**: `analyse`/`cv`/every other non-web_search bucket, and the existing `recruiter_search`/`analyse_search`/`interview_prep` numbers themselves — all exactly as they were.

**Stage 63 (prior) complete — Session AR, UX plan step 6: the Aggregator's channel cadence now surfaces on Today, so the calm front door becomes a genuine "what to do right now" driver instead of only covering pipeline-derived actions.

**What changed, confined to `TodayDashboard.js`**: a new "Channels to check today" section reads the exact same source of truth the Aggregator tab already uses — `GET /api/aggregator/channel-click` (the `channel_checks` table) for last-checked timestamps, `isChannelOverdue`/`CHANNELS`/`CHANNEL_LABELS`/`buildChannelUrls` from `lib/channel-urls.js` for cadence math and pre-filled URLs. No second cadence system, no second URL-builder, no new table, no AI call anywhere in this — it's a table read, a table write, and string concatenation. Clicking a channel item POSTs to the identical `/api/aggregator/channel-click` endpoint Aggregator's own `recordClick` calls, upserting the same `(user_id, channel)` row, so checking from Today and checking from Aggregator are the same action — neither double-counts or resets the other, verified by reading the route (one shared upsert on `onConflict: 'user_id,channel'`, no per-page distinction exists server-side).

**Calm cap honoured**: capped to 2 visible channel items regardless of how many are due, with a quiet "+N more in Aggregator →" link for the rest — never a wall. **Role-priority honesty**: rather than always featuring `target_roles[0]` (arbitrary array order, not a real priority signal), the channel-check items rotate which target role gets featured by day (`Math.floor(now / DAY) % target_roles.length`), so every target role gets fair billing over time and the label states which role was used ("Check LinkedIn for '[role]'") rather than hiding the choice. Self-tested the rotation and the due/not-due cadence math directly against the real `lib/channel-urls.js` (not a mock) — confirmed `isChannelOverdue` correctly flags LinkedIn (1-day cadence, checked 2 days ago) and target companies (2.5-day cadence, checked 3 days ago) as due while leaving Indeed (checked 12h ago) and Adzuna (checked 1 day ago, under its 2.5-day cadence) alone; confirmed the pre-filled URLs still honour the locked design decisions (Indeed gets location+radius, LinkedIn carries no location param at all, Adzuna gets location with no radius param).

**Untouched, as instructed**: `lib/channel-urls.js`, `AggregatorTab.js`, the dedup layer (`lib/job-match.js`/`addJob`) — all only read from, never modified. The Aggregator tab itself is unchanged; power users can still go straight there, and its own cadence tracker now simply reflects whatever was last checked from either place.

**Self-tested:** all 11 `lib/*.test.js` suites pass. Clean build from scratch — exit 0, zero warnings, `/app` bundle +1 kB (428 kB, negligible — new section only, no new dependency). Deployed via `vercel --prod --yes`, confirmed READY, live at `marker-silk.vercel.app`.

**Not self-tested — same standing constraint**: an actual live click-through confirming a channel item on Today updates the Aggregator's own "last checked" display and vice versa. Checklist given to Rob directly.

**Stage 62 (prior) complete — Session AQ, two UI fixes: Contractor CV brought up to the same premium standard as AI Generate, and the Settings button made actually findable.

**(1) Contractor CV was stale — a copy-paste-only tool, not a real generator.** `ContractorCvPanel` never called the model at all: it built a prompt client-side and asked the user to paste it into their own Claude/ChatGPT, so it never got the Stage 59 scaffolding-extraction fix or the premium `.docx` layout, because those fixes only ever touched the real generate pipeline (`DirectCvPanel` → `/api/cv/generate`). Fixed by giving Contractor CV a real `mode: 'contractor'` branch in `/api/cv/generate` (same route, same `checkAllowance('cv')` gate, same `lib/cv-extract.js` scaffolding-strip, same `checkVerifiedStats`, same `buildCvDocx` output) instead of building a second pipeline — the contractor-specific framing (skills-led structure, day-rate placeholder, IR35/contract-type preference, no JD tie-in) is carried over verbatim into the new prompt, unchanged in substance. Two deliberate exceptions from the JD-tailored path, both because contractor mode has no JD: the `!cvRaw` and `!roleTitle/!jd` guards are skipped for `isContractor` (a contractor without a stored CV still gets a profile-based CV, exactly as the old tool allowed), and gap analysis is skipped (nothing to gap-check against). `ContractorCvPanel.js` rewritten to match `DirectCvPanel`'s real generate+download UI (verified-stats banner, copy-to-clipboard, "Download as Word (.docx)"), with the original prompt-building logic preserved unchanged as a secondary "prefer your own ChatGPT/Claude?" fallback — same pattern DirectCvPanel already uses for the JD-tailored path, so no existing capability is lost, just demoted to secondary.

**(2) Settings button was a tiny, low-contrast icon.** 26×26px, muted-grey-on-muted-grey gear, no label — easy to miss, and Settings is where profile edits happen (drives all scoring/feed relevance), so missing it has real cost, not just cosmetic. Changed to a black-background, cream-text pill with the gear icon plus a "Settings" label, matching the weight of the app's existing primary-CTA styling (same black/cream treatment as "Add to pipeline", "Score a role" etc.) rather than inventing a new visual language.

**Self-tested:** all 11 `lib/*.test.js` suites pass. Clean build from scratch — exit 0, zero warnings, `/app` bundle unchanged at 260 kB. Deployed via `vercel --prod --yes`, confirmed READY, live at `marker-silk.vercel.app`.

**Not self-tested — same standing constraint**: an actual live generate-and-download click-through of Contractor CV, and eyeballing the Settings button's contrast in a real browser. Rob to confirm: Contractor CV → Generate produces a real downloadable `.docx` with clean typography and no leaked `---JD REQUIREMENTS---`/`---EVIDENCE MAP---` scaffolding; the copy-paste fallback still works if opened; the Settings button is now obviously visible and tappable in the header.

**Stage 61 (prior) complete — Session AP, a product/UX architecture review (assessment only) followed by executing steps 1-4 of its incremental plan: Today becomes the calm front door. Step 5 (collapsed nav as default, retiring the Focus Mode toggle) deliberately NOT done — Rob wants to live with 1-4 first, as its own separate reviewed session.

**The review's headline finding**: the product had already tried to solve "too many tabs feels overwhelming" four separate times across earlier stages — Focus Mode (opt-in 3-tab collapse), JourneyBar (persistent 5-stage strip), GettingStartedPanel (onboarding checklist), and TodayDashboard itself (already ~80% of a daily-spine pattern) — none of which replaced the others, so they all coexisted. The plan was to consolidate the best of these into one front door rather than build a fifth mechanism. Full review (current map, the two-layer driver/toolkit model, progressive disclosure timeline, feature-by-feature placement for every built AND planned feature) given to Rob as a report; nothing built until he chose which increments to execute.

**Executed, one step at a time, each verified with its own `npm run build`:**
1. **Today is now the default landing tab** (`AppPage`'s initial `tab` state: `'Pipeline'` → `'Today'`). The old forced redirect to Discover for an empty pipeline is removed — Today's own "Build your pipeline" Next-Action nudge already points there with a clear CTA, so no jarring tab-switch on load. First-run (`FirstRunGuide`) is unchanged in content but now robustly ends on Today whether the user clicks through all 3 steps or dismisses early (new `handleFirstRunDismiss` wrapper).
2. **`TodayDashboard.js` trimmed and given a pipeline glance.** The old always-visible "Pipeline Health" 4-stat grid and the two separately-stacked "Recent Opportunities"/"Watchlist" sections are gone. In their place: a new **collapsible Pipeline Glance** — collapsed by default, one summary line ("N in progress · N interviewing · N need follow-up"), expands on tap to the same stage counts + the current Next-Action message + an "Open pipeline →" deep link. It is a pure summary — no columns, no cards, no drag-drop; the real Pipeline tab (kanban board) is completely untouched. Recent + Watchlist are merged into one capped "Opportunities" feed (5 items, sorted by recency, watchlist items tagged "watching" with their existing "Consider →" action preserved). Also added: an **empty-profile guard** — if a user reaches Today with no CV and no target roles (the latent case that could hit `/api/analyse`'s known empty-profile 500), Today now renders an honest "still setting you up" card with a link to Settings, and never reaches the scorer at all.
3. **`JourneyBar` retired as a persistent element**, not deleted — it no longer renders globally in `AppPage` (was shown above every tab except in Focus Mode). Its one piece of real value, "you're at stage X of 5," now lives as a slim line inside Today's Next Action card (`getJourneyStage`/`JOURNEY`, same shared.js logic, just relocated). `JourneyBar` itself is still fully exported from `shared.js`, untouched, reversible by restoring one render call.
4. **`GettingStartedPanel` folded into Today**, not deleted — the old behaviour was an all-or-nothing swap in `AppPage` (`activeJobs.length === 0 ? <GettingStartedPanel/> : <TodayDashboard/>`, so an empty pipeline replaced the whole dashboard). Now `TodayDashboard` always renders and includes `GettingStartedPanel` as its first section, unchanged internally — it already self-hides once its own checklist is empty (`if (steps.length === 0) return null`), so it naturally disappears as the user progresses rather than needing a separate trigger. One home for "what should I do now," not two.

Two now-dead imports removed from `page.js` in the process (`JourneyBar`, `GettingStartedPanel`, plus the already-orphaned `getJourneyStage`/`buildWhyBullets` left over from the Stage 60 split) — all four exports remain fully intact in `shared.js`.

**Self-tested:** all 11 `lib/*.test.js` suites pass (0 failures). Clean build from scratch (`rm -rf .next && npm run build`) — exit 0, zero warnings, `/app` bundle unchanged at 260 kB. Deployed via `vercel --prod --yes`, confirmed READY, live at `marker-silk.vercel.app` (`/` 200, `/app` 307 → `/auth`, matching pre-change behaviour).

**Not self-tested — outstanding, needs Rob** (same constraint as Stage 60: no login credentials, won't create a disposable test account): the actual landing/click-through experience. Checklist given directly to Rob in-session for `https://marker-silk.vercel.app`: confirm Today is the landing tab; confirm the Pipeline Glance is collapsed by default and expands to a summary only (never the board); confirm the merged Opportunities feed and the "Getting started" checklist (if the account still has one) both render inside Today; confirm the old separate Pipeline Health grid and Recent/Watchlist sections are gone; confirm no persistent journey-stage strip appears above other tabs anymore; confirm the Pipeline tab itself is completely unchanged.

**Stage 60 (prior) complete — Session AO, architecture zoom-out review followed (on Rob's go-ahead) by the smallest-safe-intervention tidy it recommended. Read-only review first (findings below), then four executed fixes, self-tested via build + full test suite, deployed to production.

**The review** (read-only, no code touched): STRUCTURE — `app/app/page.js` was 6,191 lines, ~23% of the entire non-node_modules codebase, holding all 13 tabs + 3 modals + every shared primitive/constant/hook for the whole dashboard in one file — a genuine liability, not just ugly (two real bugs below were only found because of this review, and wouldn't have been caught by the build). DEAD/ORPHANED/DUPLICATED — found `SmartNudge` and `WeekProgress` fully dead (zero references outside their own declaration, same failure class as Stage 37's `AnalyseTab`/`SearchTab`); found `app/api/contractor/recruiters/route.js` calling `checkAllowance` without importing it — a live `ReferenceError` on every call, broken since 13 Aug (commit `81a77c4`), invisible to `npm run build` because client-component JSX bugs aren't executed at build time; found `SENIORITIES` duplicated between `app/onboard/page.js` and `app/settings/page.js` and already drifted (`vp_plus` read "VP / C-Suite" in one, "VP+" in the other). FRAGILITY MAP — the dedup layer (`lib/job-match.js`) and aggregator URL-builder (`lib/channel-urls.js`) are both well-isolated with their own test files; the real fragility was structural (editing any one of 13 tabs meant editing inside one 6,191-line file), not logic risk. CONSISTENCY — 68 swallowed `catch {}` blocks (house style, not an anomaly, but the same mechanism class as the Stage 39 nonexistent-column bugs); three data-access patterns coexisting (`lib/db.js`, direct Supabase client, `fetch()` to API routes) with no written rule for which to use when. Full verdict: fine to keep adding carefully, don't do a full rebuild — smallest safe intervention was the split below.

**Executed, in order, each verified with a full `npm run build` before moving to the next:**
1. **Fixed the live bug.** `app/api/contractor/recruiters/route.js` now imports `checkAllowance` from `lib/allowance.js` (one line, matching its working sibling `perm/recruiters`).
2. **Deleted the two dead components** (`SmartNudge`, `WeekProgress`) after re-confirming zero references.
3. **Unified `SENIORITIES`/`INDUSTRIES`** into `lib/onboarding-options.js` (same pattern as the existing `ROLE_FAMILIES` fix), both pages now import from one source; picked "VP / C-Suite" as the surviving label for `vp_plus`.
4. **Split `app/app/page.js`**, one component at a time (17 total: 14 tab components + 3 modals), each extraction verified with its own build before the next. `page.js`: 6,191 → 804 lines, now holds only `AppPage` itself. New structure: `app/app/shared.js` (the ~67 constants/helpers/hooks/small UI primitives every tab needs — a pure relocation, no logic changed), `app/app/tabs/*.js` (14 files), `app/app/modals/*.js` (3 files). Extraction order respected the real dependency graph found during the review (`CvTab`→`RecruiterPanel`+`ContractorCvPanel`+`DirectCvPanel`, `FeedTab`→`WishlistTab`+`WishlistJobsTab`, `TodayDashboard`→`EngineTab`, `ContractorTab`→`RecruiterPanel`): leaf components first, composite tabs last. After every single extraction, cross-checked the new file's actual identifier usage against its import list by name (not just "build passed") — this is what caught a missing `WLB_DATA` import in `WishlistTab.js` before it ever shipped; a plain build would not have caught it, since an undefined identifier in JSX only throws at runtime for a client component, not at bundle time (the exact same silent-failure mechanism the review flagged in item 2 above).

**Self-tested:** all 11 `lib/*.test.js` suites pass (0 failures, same as before the split). Clean production build from scratch (`rm -rf .next && npm run build`) — exit 0, zero warnings, `/app` bundle size unchanged at 260 kB (confirms no logic duplication, pure relocation). Deployed via `vercel --prod --yes`, confirmed READY, aliased to `marker-silk.vercel.app`, both `/` (200) and `/app` (307 → `/auth`, correct auth-gate behaviour) verified live.

**Not self-tested — outstanding, needs Rob:** interactive browser spot-check of the split tabs. I do not have login credentials and won't create a disposable test account myself (account creation is off-limits for me regardless of context, even though this project's own history shows that exact pattern used repeatedly for self-testing). Steps for Rob to run against `https://marker-silk.vercel.app` (or `npm run dev` locally): log into `/app`; click through every tab (Today, Pipeline, Discover, Aggregator, WLB, CV, Interview, Contractor, Profile) and confirm each renders; on Discover, click between Target Companies / Live Roles / Wishlist Roles; on CV, click between AI Generate / Cover Letter / Contractor CV / Recruiters; on Contractor → Recruiters, trigger a recruiter search and confirm it returns results instead of erroring (this is the route with the live bug fixed this session); on `/settings`, confirm the Seniority chips render and the top one reads "VP / C-Suite"; same check on `/onboard` if reachable. If anything breaks, report what and where.

**Stage 59 (prior):** Session AN, three confirmed fixes from Rob's diagnostic pass: the CV scaffolding-leak bug (functional, fixed first as the most important), a full premium typographic redesign of the CV .docx (ATS-safe), and a shared prominent paste-JD CTA applied to all five entry points.

**(1) CV extraction bug — the real cause of "reads as obviously-AI".** `/api/cv/generate` was returning `text: raw` — the model's ENTIRE unsplit response, including its own working-out (`---JD REQUIREMENTS---`, `---EVIDENCE MAP---`, and on Deep effort `---ATS ANALYSIS---` before the CV and `---SIFT ASSESSMENT---` after it) — to both the on-screen preview and the `.docx` download. `buildCvDocx`'s all-caps heading detector was even bolding "JD REQUIREMENTS" and "EVIDENCE MAP" as if they were real CV section headers. New `lib/cv-extract.js` (`extractCvSections`, 22/22 tests) splits the response on its own `---SECTION---` markers and returns only the `---TAILORED CV---` content as `cv`; `reasoning`/`sift` are kept separately (available for a future "why we scored it this way" UI, never mixed back into the CV). `checkVerifiedStats` now runs against the extracted CV only, not the full raw blob — previously it could flag a number that only ever appeared in the ATS-analysis/sift-assessment scaffolding (a match score, an estimated interview probability) as a hallucinated CV stat, when it was never part of the CV.

**(2) CV .docx — premium typographic redesign, ATS constraint preserved.** `lib/cv-docx.js` rebuilt around a pure `classifyCv()` (line-array in, typed-record-array out — fully unit-testable without touching the `docx` library, see `lib/cv-docx.test.js`) and a thin rendering layer. Garamond (name, section headings) paired with Calibri (body/company/dates/bullets); soft charcoal ink, muted grey for meta text, a thin rule closing the header block and underlining each section heading; a header parser that handles real messy source text (contact lines appearing BEFORE the name, as a CV pasted from a LinkedIn/PDF export commonly produces) rather than assuming line 1 is always the name. **HARD CONSTRAINT preserved and tested**: job title / company / dates render as three separate `Paragraph`s, never merged — hierarchy comes entirely from weight/size/colour/spacing on those three still-separate lines (title bold+larger, company regular, dates small+muted), verified directly against the real generated `.docx`'s XML, not assumed.

**Two more real bugs found only by self-testing against a genuinely live AI response** (not just hand-written fixtures), both fixed with regression tests added:
- A fixed 8-line header lookahead could swallow a "Professional Summary" label AND its sentence into the header zone and silently discard them (the zone only ever extracts name/tagline/contact) — real content loss, not just a styling gap. Fixed: the header parser now stops the moment name+tagline are both found and the next line isn't contact info, rather than scanning to a fixed cap regardless.
- The model doesn't always follow "mark changed sections with `[UPDATED]` at the start" — a real generation appended it as a suffix (`"Skills [UPDATED]"`, `"...budget [UPDATED wording]"`). The old strict-prefix strip missed these; now stripped wherever they appear in a line.
- Also found live: the model doesn't always render section labels in ALL CAPS ("Experience", not "EXPERIENCE"), which the original all-caps-only heading heuristic would silently demote to plain body text. Added an exact-match (not fuzzy) known-CV-section-label vocabulary as a second path to heading detection.

**(3) Hidden paste-JD CTAs — one shared component, five entry points.** New `PasteJdCallout` (`app/app/page.js`) extracts the Stage 55 rainbow-gradient chrome (`linear-gradient(90deg, #FF6B6B, #FFD93D, #6BCB77, #4D96FF, #C77DFF)`, originally only the "Open the job page ↗" fallback button) into one reusable banner, applied to: **`EditJobModal`** (the actual worst offender — a field literally labelled "Notes" with the JD-paste hint only in its placeholder; correction from the diagnostic pass: this field lives in EditJobModal, not AddJobModal, which currently has no JD field of any kind — flagged, not built, out of the five named), **`PrepTab`** (only when no JD is already pulled onto the role), **`EngineTab`**'s primary scorer input, **`CvTab`**'s no-JD-stored fallback (also upgraded its submit button to the same gradient), and **Aggregator bring-in** (its reveal button is now the gradient CTA itself). One design language across all five instead of five different-looking, easy-to-miss fields.

**Self-tested live end to end, against a real AI-generated CV** (disposable test account with a synthetic profile, created and deleted via the Supabase Admin API): called the real `/api/cv/generate` at Deep effort (worst case — scaffolding both before AND after the CV) against a real job description. Confirmed live: the returned `text` contained the tailored CV with **zero** scaffolding markers (`JD REQUIREMENTS`/`EVIDENCE MAP`/`ATS ANALYSIS`/`SIFT ASSESSMENT` all absent), `reasoning`/`sift` correctly captured the scaffolding separately, and `flaggedMetrics` was empty. Built the real `.docx` from that real response and unzipped its actual `word/document.xml`: 32 real paragraphs, all three roles' title/company/dates confirmed as three genuinely separate `<w:p>` elements each (not merged — ATS-safe), Garamond/Calibri both present in the XML, ink/muted colours present, 5 paragraph-border rules present (1 header rule + 4 section-heading rules). Generated a genuine before/after pair from the exact same real content (old renderer recovered from git vs the new one) and sent both to Rob directly. `next build` clean; full test suite **12/12 files pass** (44 cases in `lib/cv-docx.test.js` alone, up from 0 — this file had no test coverage before this session).

**Stage 58 (prior):** Session AM, dashboard-wide duplicate / already-handled detection: a tiered, deterministic string matcher (zero AI cost, by design — explicitly the brief's own constraint) that stops the tool re-surfacing a job the user already applied to, dismissed, or is already tracking, even when it arrives under a different URL from a different source.

**Why URL-only matching doesn't work (the actual problem):** Adzuna's redirect hash changes per call, LinkedIn/Indeed/a company's own board all differ for the same real posting, and the app's own existing "already added" check (`addedLinks` in `FeedTab`) was and is purely URL-based — exactly the failure mode this fixes. Worse: dismissed jobs had **no content snapshot at all** — `/api/dismiss` only ever wrote the URL into a bare array in `profiles.hard_filters_json.dismissed`, so there was nothing to compare a re-surfaced dismissed job against even in principle.

**The matcher** (`lib/job-match.js`, 29/29 tests in `lib/job-match.test.js`, covering the brief's exact (a)/(b)/(c) acceptance cases plus edge cases): two tiers, per the brief.
- **STRONG** — same stable external/ad id, OR normalised company + title + location all match (all three present). Hard normalisation: lowercase, strip `Ltd/PLC/LLP/Group/Bank/UK/The/...` company suffixes, collapse a small set of common title-variant abbreviations (`Sr.`→senior, `Mgr`→manager, `&`→and, etc.), strip UK country-name suffixes from locations.
- **SOFT** — same company + strong fuzzy title similarity (token-Jaccard, reusing the same lightweight approach already used for role-query dedup in `lib/aggregate-role-queries.js`, no new dependency) but not exact, OR same company + exact title but location differs/missing. Never asserted as fact — always rendered as "might be the same as".
- Deliberately a separate module from the existing `lib/dedupe-jobs.js` (anonymous-row repost dedup within one ingest batch, already live and audited) rather than merged into it — different question (a specific user's own history vs an ingest batch), harder normaliser, and touching a working, low-regression-tolerance file for this wasn't worth the risk.

**Schema** (migration `013_job_dedup.sql`): `pipeline_items` gains `location` and `external_id` snapshot columns (it already snapshotted company/role independently of `jobs_cache` — `custom_company`/`custom_role` — but not these, so a match couldn't survive the source posting being deleted for those two dimensions). New `dismissed_jobs` table (company/role/location/external_id snapshot + `dismissed_at`) — **dual-written alongside** the existing `hard_filters_json.dismissed` URL array, not replacing it (that array is still read by the feed's dismissed-filter `Set`; this table is additive, avoiding touching a live read path). GRANT included from the start, verified live post-push (migration 008's finding: service_role does not bypass a missing table-level GRANT).

**Zero-AI URL peek** (`lib/job-page-scrape.js`, new — extracted unchanged from `app/api/analyse/route.js`'s existing "Strategy 2" page-fetch, which already did a schema.org JobPosting JSON-LD parse purely to build its own AI prompt): `fetchJobPage`/`extractJobPostingJsonLd`/`extractPlainText`/`extractPublishedDate` are now the one shared implementation — `analyse/route.js` calls it too, so there's no second copy silently drifting. Used by new `POST /api/aggregator/peek-job`: given a URL, check `jobs_cache` first (free DB lookup), else fetch + parse JSON-LD (free HTTP + regex, no model call anywhere in the file) — returns `{company, roleTitle, location, externalId}` or all-null if neither source yields anything, in which case the caller falls through to paid scoring as normal. **This is the mechanism that makes "flag before any paid scoring" possible for a URL-based bring-in** — there is no AI fallback here, by design, matching the brief's explicit instruction to stop and flag rather than add cost.

**Honest limitation, surfaced not hidden:** a JD-paste with no URL has nothing to peek at — company/role only become known from the paid `/api/analyse` response itself, so the pre-scoring credit-saving check cannot cover that specific case (nothing free can answer "what job is this" from unstructured pasted text without a model call, and the brief said stop and explain rather than add one). The trust benefit still lands — `scoreLine` runs the same matcher immediately after scoring using the now-known company/role and annotates the result — but that specific credit isn't saved. Flagged in-code and here, not papered over.

**Wired into three places, one shared choke point where possible:**
1. **`addJob`** (`app/app/page.js`) — the single function every entry point already funnelled through (feed cards, wishlist, balanced/contractor tabs, the single-role scorer, Aggregator's own add button, manual add) now runs the check once, here, rather than being repeated at each call site. STRONG match pauses the add with a confirm modal ("You applied to this in [month]." / "You dismissed this before." / `Already tracking this as "[status]"`. — the three exact phrasings from the brief) with an explicit "Add anyway" override; SOFT match never blocks, it rides along on the job record as a dismissable amber note (`possibleDuplicateOf`, now round-tripped through `lib/db.js`'s `jobToRow`/`rowToJob` so it survives a reload) shown on the pipeline card.
2. **Aggregator "Bring in for scoring"** (`app/app/page.js`'s `AggregatorTab`) — the explicit cost-saving requirement. For each URL line: peek for free, check the matcher; STRONG → **no `/api/analyse` call made**, row shows "Already have" + the match reason + a "Score anyway" override; SOFT → scores as normal with the note attached, non-blocking. A JD-only line scores as normal, then gets the post-score check (see limitation above).
3. **The feed itself** (`FeedTab`'s `webJobs`/`govJobs` filtering) — a STRONG match against the user's pipeline/dismissed history now filters a card out of the feed the same way the existing URL-based `addedLinks` check already did; SOFT matches stay visible (hiding an unseen card on a guess would be a real false negative, which the brief explicitly said false positives AND false negatives both matter).

All five existing `/api/dismiss` call sites updated to pass company/roleTitle/location/externalId through when the caller has them, so `dismissed_jobs` actually gets populated going forward, not left null.

**Self-tested live, end to end, with real seeded data** (disposable test account, created and deleted via the Supabase Admin API, cascade-verified clean afterwards):
- Seeded one real `pipeline_items` row (`status: applied`, Monzo Bank Ltd / Senior Product Manager / London, a real `external_id`) and one real `dismissed_jobs` row (NatWest Group / Business Analyst / Manchester) directly in production.
- Fetched them back through the **real API routes** (`GET /api/dismiss` for the dismissed record; a direct `pipeline_items` query standing in for `loadJobs()`) — not hand-typed mocks — and ran the real `lib/job-match.js` against that real fetched data for all three required acceptance cases plus a fourth: **(a) exact duplicate → STRONG** ("Same listing ID"); **(b) same job, different source/external id → STRONG** ("Same company, title and location"); **(c) genuinely different role at the same company → correctly not flagged**; **(d) the dismissed NatWest role resurfacing → STRONG**, sourced correctly from `dismissed_jobs` not `pipeline_items`. `describeDuplicateMatch` verified to produce the exact required phrasings ("You applied to this in August 2026." / "You dismissed this before.").
- **Full pre-scoring-skip chain proven against a genuinely live posting**, not a mock: seeded a `pipeline_items` row matching a real live Greenhouse posting exactly, called the real `/api/aggregator/peek-job` against that live URL (hit the `jobs_cache` path, `source: 'cache'`), fed its real response into the real matcher against the real seeded row — STRONG match, confirming `/api/analyse` would be skipped and no scoring credit spent, for genuinely live-scraped data end to end.
- `peek-job`'s graceful "nothing found" path also verified live (a non-existent URL correctly returns all-null rather than erroring).
- Re-verified `/api/analyse` itself is unchanged after the `job-page-scrape.js` extraction refactor with a real live call against the same real Greenhouse posting — identical company/role extraction to the peek endpoint, real score returned, no regression.
- `next build` clean; full test suite **10/10 files pass** (adds `lib/job-match.test.js`, 29 cases).

**Stage 57 (prior):** Session AL, the Job Aggregator tab: a new `Aggregator` tab (`app/app/page.js`) that builds pre-filled, per-role search links for the channels Requite legally can't ingest (LinkedIn, Indeed, Adzuna's public search) and sends the user out to them — link-out only, never a scrape, consistent with the file's existing HARD RULE on job links. Built on Stage 56's foundation (field-aware profiles, snapped salary bands, `radiusMiles` defaulting to 50).

**URL patterns — every one verified live against the real site before being written, not guessed** (`lib/channel-urls.js`, 15/15 tests in `lib/channel-urls.test.js`):
- **Indeed** (`uk.indeed.com/jobs?q=&l=&radius=&salaryType=£<amount>&sort=date`) — honours location and radius correctly; its own "Pay" filter writes `salaryType=£<amount>` and, tested live, accepts values outside its own 5-item quick-filter list (`salaryType=£80,000` worked even though Indeed's own dropdown only offers up to £50,000 for a nurse search), so any of Requite's `SALARY_FLOOR_OPTIONS` bands works.
- **LinkedIn** (`linkedin.com/jobs/search/?keywords=&sortBy=DD&f_TPR=r86400`) — **deliberately carries no location param.** Tested twice, logged in, with a real UK postcode and a different city: both times LinkedIn silently resolved to "London (80 km)" regardless of what was sent, ignoring/overriding the `location` param entirely. LinkedIn links are therefore national, newest-sorted (last 24h), and the UI explicitly labels them "LinkedIn (UK-wide)" — never "near you" — with an honest one-line disclosure on the tab itself.
- **Adzuna's public search page** (`adzuna.co.uk/jobs/search?q=&w=&sf=<whole £>&sb=date&sd=down`) — distinct from the ingest API this app already calls elsewhere; read its own `<select>` elements live to get the real `sf`/`sb`/`sd` param names and values rather than guess. No radius control exists in Adzuna's own search UI, so none is built here.

**Cadence tracking** — new `channel_checks` table (migration `012_channel_checks.sql`: `user_id`, `channel`, `last_checked_at`, unique per user+channel), read/written via `app/api/aggregator/channel-click/route.js` (GET/POST) using the service-role pattern from `profile/save`, **with the table GRANT included from the start** (`GRANT ... TO service_role, authenticated`) — migration 008's finding was that service_role does NOT bypass a missing table-level GRANT (a mechanism distinct from RLS), which silently broke 19 tables before being caught; verified live post-push via a direct PostgREST probe rather than assumed. Cadence: Indeed/LinkedIn daily, Adzuna + target-company career pages every 2-3 days (target-company row links out to the existing Discover tab rather than rebuilding wishlist functionality); a real click-through and an explicit dismiss (✕) both just mark the channel checked now.

**Bring in for scoring** — a bulk paste box (one link or JD per line, hard-capped at 10 lines per run client-side) that reuses the **existing** `/api/analyse` endpoint per line, in sequence — deliberately not a new scoring pathway. Draws from the same `'analyse'` allowance bucket as the single-role scorer (`app/api/aggregator/bring-in-allowance/route.js`, same read-only pattern as `cover-letter-allowance`), shows "X of Y credits left" before running, requires an explicit confirm click naming the exact cost, and stops the batch early (marking remaining lines "Skipped") if the allowance is exhausted mid-run rather than silently over-spending. Results are NOT auto-added to the pipeline — each scored result gets its own "+ Add" button, so a bad-fit result found via bulk paste doesn't land in the pipeline unreviewed. A short, honestly-sourced nudge line ("target, don't spray") sits above the box with a source link.

**Explicitly NOT built, flagged not faked:** the specialist-board-per-profession mapping (NHS Jobs, teaching-vacancy portals, trades boards) — a dashed-border "Coming soon" panel states this plainly rather than pointing a "specialist" button at a generic search link. The browser plugin is untouched, separate work.

**Stat-citation caveat, flagged honestly:** Rob's framing ("75%+ match roles convert 10-15% vs 2-5%") could not be traced to a primary source — web search surfaced mostly SEO/content-farm blog posts, none matching that exact "75%+ match" threshold. Used the closest number that traces to real underlying data instead: Wellfound's analysis of 500,000+ real job applications (~7-9% interview rate for tailored applications vs ~2-3% for generic ones), linked directly in the UI. If Rob has the original source for his figures, swap it in — this is a substitution, not a match.

**Self-tested live, end to end**, using the same reconstructed-Supabase-session-cookie technique this project has used before (Stage 39/49) since a full browser click-through needs a real login this environment can't complete unattended:
- `buildChannelUrls()` run against the real nurse profile pulled fresh from the live DB (`target_roles: ['Advanced nurse practicioner','nurse','ADHD Nurse']`, `postcode: 'BA11 4GH'`, `salary_floor: 50000`, `radiusMiles: 50`) — the exact URLs the shipped code generates were spot-checked live; Adzuna's specific 3-word title + tiny postcode + £50k floor combination genuinely returns zero current listings (confirmed not a mechanism bug: the same postcode with a plain "nurse" query returns real results, and the zero-result page correctly echoed the query back).
- Unauthenticated requests to both new API routes correctly gate (`channel-click` 401s, `bring-in-allowance` returns a safe `cap:0` default) rather than crash.
- A disposable test account (created and deleted via the Supabase Admin API, cascade-verified clean afterwards) exercised the full authenticated path: GET `channel-click` (empty state) → POST a click → GET again (persisted) → POST an invalid channel (400, correctly rejected) → `bring-in-allowance` (real tier/cap/used from a genuine `ai_usage` count).
- One real `/api/analyse` call through the same endpoint bring-in depends on, against this test account: **found and diagnosed a real 500 ("Could not parse response") on a genuinely empty profile** — pre-existing behaviour of `/api/analyse` itself (untouched this session), not something the Aggregator introduced. Confirmed the same call succeeds cleanly once the profile has real data (populated the test profile, score 7 / signal "apply" came back correctly) — and confirmed the Aggregator's per-line try/catch already surfaces exactly this kind of failure as a row-level "Failed" status rather than crashing the whole batch, so the failure mode this uncovered is already handled gracefully by the code as built.
- `next build` clean; full test suite **9/9 files pass** (adds `lib/channel-urls.test.js`, 15 cases, to the existing 8).

**Stage 56 (prior):** Session AK, foundation fixes ahead of the daily-to-do/job-aggregator build (URL-builder tab), per Rob's confirmation-pass findings. Three items, no aggregator/to-do code touched.

**(1) Salary-floor corruption — root-caused, not just repaired.** Live-queried `profiles` directly: 2 of 5 production rows had `salary_floor` = 80,000,000 and 75,000,000 — the exact £-million corruption class first found and partially fixed at Stage 44/45 (free-text £ input replaced with a fixed £k dropdown). Traced every write site in the codebase (`grep` for every `.from('profiles').update/upsert/insert` call): `app/api/profile/save/route.js:74` is the ONLY place `salary_floor` is ever written, and it has been unchanged since the repo's single squashed initial commit. Both current UI callers (onboarding, settings) already send clean dropdown £k values consistently with it — **this is not an active code bug**, it's stale data from before the Stage 44 fix (or a stray direct-API call) that Stage 45's repair never reached because it only fixed Rob's own account (`user_id 25d12e91`), not these two other rows. Confirmed via `git log -p` that the Stage 44 diagnostic itself already flagged the real gap: "there's no sanity-check ceiling on the input, so nothing stops it." Closed that gap: `app/api/profile/save/route.js` now imports `SALARY_FLOOR_OPTIONS` and snaps any `salaryFloor` value to the nearest real option server-side before writing (`normalizeSalaryFloor`), so this corruption class can't recur regardless of caller — dropdown UI, a future extension/API caller, anything. **Repaired the 2 corrupt production rows directly via the Supabase REST API** (before/after below).

**(2) Onboarding role chips still 100% marketing/tech — the unfinished half of Stage 53.** Stage 53 opened the AI-CV-parser and free-text paths to any profession but never touched the static `ROLE_FAMILIES` chip list a user picks from manually (`app/onboard/page.js:110`, pre-fix) — a nurse or electrician selecting "Healthcare/Clinical" or "Engineering (non-software)" as their field still saw only Marketing Generalist / Product Marketing / Partnerships etc. as suggested chips. Also found `app/settings/page.js` carried its OWN second, shorter, differently-labelled copy of the same list (`'BD'` instead of `'Business Development'`, etc.) — genuinely drifted from onboarding's, so a role picked in one page didn't show as selected in the other. Fixed by extracting a single shared module, `lib/onboarding-options.js`, exporting `PROFESSIONAL_FIELDS`, `SALARY_FLOOR_OPTIONS`, and a new `ROLE_FAMILIES_BY_FIELD` map (all 37 original labels preserved exactly, reorganised by field, plus new families for Healthcare/Clinical, Education/Academia, and Engineering — non-software/trades) with a `visibleRoleFamilies(fields)` helper that returns the union of chips for whichever field(s) the user picked, falling back to the full list when none is picked yet. Both `app/onboard/page.js` and `app/settings/page.js` now import from this one module and render `visibleRoleFamilies(fields)` instead of a flat list; the "custom/CV-derived role" chip-echo logic in both pages was also changed to check against the *currently visible* list rather than the master list, so a role from a field the user isn't currently showing doesn't silently vanish. Settings previously had no such echo at all — added it, matching onboarding's existing pattern, so a profile with free-text/CV-derived roles (e.g. the real nurse test profile's `'Advanced nurse practicioner'`, `'nurse'`, `'ADHD Nurse'`) stays visible and removable there too. **Verified deterministically** by importing the real shipped module and calling `visibleRoleFamilies()` directly (not a reimplementation): `['Healthcare/Clinical']` → 9 clinical roles, zero marketing; `['Engineering (non-software)']` → 9 trade roles (Electrician, Plumber, Gas Engineer, etc.), zero marketing; `['Education/Academia']` → 7 teaching roles; no field selected → falls back to the full master list (63 roles, up from the old 37 — the union across every field). **Could not get a live browser screenshot**: local dev auth requires either a real magic-link click-through or cookie injection, and the browser tool's own privacy guard blocked programmatic cookie-setting (a Supabase session cookie counts as "cookie/query string data") — correctly refused rather than worked around. `next build` clean and full test suite (8 files) green after the change.

**(3) Profile fields the future URL-builder needs.** `radiusMiles` (`hard_filters_json.radiusMiles`) was `null` on 3 of 5 live rows — not because users chose "no limit" but because the client's initial state defaulted to `null`, indistinguishable from "never touched this step." Fixed at the root: `app/onboard/page.js`'s `radiusMiles` state now defaults to `50` (matching `lib/uk-geo.js`'s existing match-time fallback), so "Anywhere" (still `null`) becomes a genuine, deliberate choice going forward rather than the same value as unset. **One-time backfill** of the 3 existing null rows to `50` (before/after below) — a data repair, not an ongoing server-side override, so a future explicit "Anywhere" click won't get silently clobbered. `target_roles` priority: confirmed no consumer anywhere in the codebase (`match-engine.js`, `scoring.js`, `aggregate-role-queries.js`) treats `target_roles[0]` specially — the array is read as an unordered set everywhere. Array order is therefore usable as a rough "most recently added" proxy at best, not a deliberate priority signal — nothing currently asks the user which role is their top choice, so `target_roles[0]` is an accident of click order, not intent. Leaving as-is per Rob's steer, flagged so the URL-builder doesn't over-trust it as "their #1 title" without knowing this.

**Salary-floor repair — before/after (live DB):**

| user_id | salary_floor before | salary_floor after |
|---|---|---|
| `53781936-7cc9-…` | 80,000,000 | 80,000 |
| `ebd1fd83-da55-…` | 75,000,000 | 75,000 |

**radiusMiles backfill — before/after (live DB):**

| user_id | radiusMiles before | radiusMiles after |
|---|---|---|
| `53781936-7cc9-…` | null | 50 |
| `7b5c16b7-a03a-…` (nurse test profile) | null | 50 |
| `5118a3c7-5f32-…` | null | 50 |

All 5 live profile rows now have a sane 5-figure `salary_floor` and a real numeric `radiusMiles`. **Not deployed to production yet this session — foundation-only per Rob's instruction; the aggregator/daily-to-do tab is still unbuilt.**

**Stage 55 (prior):** Session AJ, refining Stage 54's JD-entry UX per Rob's feedback: the link field shouldn't be hidden, and the "sites block it" line was reading as an up-front warning rather than an honest explanation only when it's actually true. `EngineTab`'s entry now shows the job-link input and the JD paste box together from the start (link first, JD box beneath — no toggle). On submit with a link and no JD already typed, the client tracks it as a "pull attempt": if `/api/analyse`'s page-fetch strategy succeeds, the JD box auto-fills with the real pulled text and a green "✓ Pulled the details in for you" confirmation shows; if it fails (falls through to the web-search fallback strategy, a thrown request, or an unparseable response — but NOT an unrelated allowance/plan-limit 429, which isn't the link's fault), the link input turns into a large rainbow-gradient "Open the job page ↗" button (opens in a new tab) with a "Try a different link" reset underneath, and only then does the calm one-line explanation appear ("Some job sites let us pull details in automatically; many block it, so pasting is the sure way."). The up-front warning copy from Stage 54 is gone entirely. **Backend change**: `/api/analyse`'s direct-page-fetch strategy (`app/api/analyse/route.js`) now returns the raw extracted page text as `extractedJd` in its JSON response — previously the client only ever got back a score, so a link-only submission that succeeded still left `job.jd` empty, breaking the "single source of truth" promise the moment someone leant purely on the link. `jd`-text and web-search-fallback strategies still return no `extractedJd` (nothing reliably raw to hand back), which is exactly the "pull failed" signal the client uses. Stage 54's `job.jd`-as-single-source-of-truth wiring (scorer ↔ CV generator ↔ cover letter ↔ interview prep) is untouched. **Self-test — live, both paths, against a real Anthropic key:** success path hit a real live Greenhouse posting (`job-boards.greenhouse.io/monzo/jobs/8143930`) and got back `extractedJd` (5,988 real characters of the actual job ad) plus a correct score/role/company — confirmed both locally and, separately, directly against the live production URL after deploy (`marker-silk.vercel.app/api/analyse`, a second live Monzo posting, 5,991-char `extractedJd`). Failure path hit a deliberately unreachable domain, confirmed the response falls through to the web-search strategy, returns HTTP 200 with no `extractedJd` and a `signalReason` of "Could not retrieve job content; paste the JD below for an accurate score" — exactly the shape the client's `pullStatus='failed'` branch expects. `next build` clean (two runs). Could not click-through the actual React UI live (same Supabase-auth/password constraint as Stage 54) — verified the state-machine logic by hand-tracing both real API responses through `analyse()`'s branches instead of guessing.

**Deployed to production 2026-09-07** — `vercel --prod --yes` from `~/dev/marker`, deployment `dpl_5f9w38kHCAaRQhAxoa5HVzihpMKj`, aliased to `https://marker-silk.vercel.app`, confirmed `readyState: READY`.

**Stage 54 (prior):** Session AI, the JD-paste UX fix. Job-page scraping is unreliable by nature (logins, JS rendering, bot-blocking), so the "Score a job" entry (`EngineTab`, `app/app/page.js`) now leads with a JD paste textarea as the primary action, with copy that's honest about what a link can and can't do ("Got a link? Add that too, but the description is what we actually need" / "we'll try to pull the details in automatically, but it doesn't always work (a lot of job sites block it)"). The URL field is demoted to a collapsed "▸ GOT A LINK? PASTE IT TOO (best-effort auto-pull)" toggle; the two "Bug 4" auto-reveal-JD-on-failure calls were removed since the JD box is no longer hidden. Interview prep (`PrepTab`) previously carried its own isolated JD textarea that asked every time, even when a JD was already stored on the job from scoring/CV/cover-letter; it now pre-fills from `job.jd` (the same field the scorer, CV generator, and cover letter already read/write) and persists any fresh paste back onto the job via `updateJob`, so `job.jd` is genuinely the single source of truth across all four features. Backend logic was already JD-first (`/api/analyse` tries `jdText` before falling back to a URL fetch then a web-search fallback), so no API changes were needed. **Self-test:** `next build` clean (two full runs, exit 0), grep-confirmed no leftover references to the retired `showJd` state var, full diff reviewed line-by-line before deploy. Could not complete a live logged-in click-through — the dev server's `/app` requires Supabase auth and entering a saved password to sign in is outside what I'll do unilaterally — so this shipped on code-level verification only, flagged honestly rather than claimed as browser-tested.

**Deployed to production 2026-09-07** — `vercel --prod --yes` from `~/dev/marker`, deployment `dpl_5sJvdR9uuDYCf7hTFmBBjdK5gJ3u`, aliased to `https://marker-silk.vercel.app`, confirmed `readyState: READY` via `vercel ls marker --yes`. Committed `b874fe2`, pushed to `origin/main`.

**Stage 53 (prior):** Session AH, the CV-personalisation root-and-branch fix. Rob tested the live product with a healthcare CV and found it still suggested Anthropic/Stripe/Figma/Monzo and pre-selected marketing role families — the product was silently built around a senior-marketing-in-tech persona at every layer: a closed `ROLE_FAMILIES`/`INDUSTRIES` list in onboarding's CV parser filtered out the model's own correct output (confirmed live: it had already said "Clinical Nursing" and the filter threw it away); the Target Companies "AI SUGGEST" auto-generation was gated by a per-BROWSER `localStorage` flag that never re-fired after a CV was replaced; `WISHLIST_SEEDS.standard` (the Anthropic/Stripe/Figma/Monzo list) was force-merged onto every non-standard track regardless of fit; the shared nightly Adzuna feed and the gov feed's title filter were both hardcoded to marketing/tech/office-track vocabulary, with the gov filter actively REJECTING 'nurse'/'doctor'/'clinical' outright. Fixed per Rob's explicit decision to go **open-vocabulary, not an expanded fixed list**: role families/industries are now derived from the CV with no filter against a canonical list (`app/api/onboard/parse-cv/route.js`); the Target Companies auto-generation now keys off a server-side CV-content signature (`lib/text-hash.js` + `hard_filters_json.wishlistCvSignature`) instead of a browser flag, regenerates when the CV changes, and every fallback firing is logged via `track('wishlist_fallback_seeds', ...)` instead of failing silently; the standard-track seed force-merge is removed; `cron/gov`'s and `feed-gov`'s profession-blanket REJECT lists are gone (feed-gov's fresh-scan now matches on the user's own `target_roles` via new `lib/role-keywords.js` when set); `cron/adzuna`'s query set is now a broadened static floor plus real distinct `target_roles` aggregated from actual user profiles (`lib/aggregate-role-queries.js`, capped, still one shared nightly batch — Cost Guardrails Rule 1 intact); `wishlist/check`'s marketing-only KW/REJECT lists are replaced with per-user keyword matching; `lib/match-engine.js` gained `healthcare`/`education`/`trades` role families for its domain-conflict guard; `BALANCED_COMPANIES` gained a sourced NHS entry (real Agenda for Change terms + Glassdoor's general NHS listing, not a fabricated stat); onboarding's founder-only `DEV SKIP` button (which also had a latent `setField` ReferenceError — the setter is `setFields`) no longer autofills a marketing persona; placeholder copy across onboarding/dashboard (interview-prep interviewer example, STAR-story examples, CV-upload no-CV form) is now field-neutral; the CV-upload step now has an explicit "No CV?" toggle instead of an implicit empty-state. **Self-tested live** against 3 synthetic CVs (nurse, secondary-school teacher, electrician) through the real `/api/onboard/parse-cv` endpoint and the real `wishlist/generate` prompt (direct Haiku call, same template) — full results in the Stage 53 log below; all three produced genuinely sector-appropriate role families and target companies, zero marketing/tech bias. Full test suite (8 files) + `next build` both green. **Not fixed, flagged honestly (banked as roadmap, not a blocker):** `lib/ats.js`'s 43-company ATS board list stays fintech/SaaS-only — live-probed several real healthcare/education companies against all 4 ATS providers (Greenhouse/Lever/Ashby/SmartRecruiters) and none exist on them; NHS/schools/trades hire through entirely different systems (NHS Jobs/TRAC, teaching-vacancy portals), so widening this list is a new ingest-source feature, not a config change. Not a launch blocker since the Adzuna feed and the AI-generated wishlist already cover these sectors — this is a coverage *improvement* (a faster/cheaper path for non-tech fields), not a gap in what users can find. **Roadmap item: sector-specific ingest sources (NHS Jobs, teaching-vacancy portals, trades boards) so non-tech fields get the same fast-path coverage the ATS boards give tech.**

**Deployed to production 2026-08-29** — `vercel --prod --yes` from `~/dev/marker` at commit `823bcd0`, deployment `dpl_3avNxrsfVaTv8rAitp7ZscZAuof8`, aliased to `https://marker-silk.vercel.app`, confirmed `readyState: READY`. **Re-verified live against production** (not local) with 3 fresh synthetic CVs via a real minted Supabase session (password-grant sign-in + reconstructed `@supabase/ssr` cookie, same technique as prior sessions) hitting the real `/api/onboard/parse-cv` and `/api/wishlist/generate` endpoints:

| CV | Role families (prod) | Target companies (prod, sample) |
|---|---|---|
| Nurse (Senior Sister, pain management) | Clinical Nursing, Ward Leadership, Pain Management Specialism | Bupa UK, Spire Healthcare, Ramsay Health Care UK, The Christie NHS FT, Nuffield Health |
| Teacher (Head of Geography) | Secondary Geography Leadership, Curriculum Design, Department Management, Educational Leadership | Teach First, Pearson UK, Ofsted, Tes, Century Tech |
| Electrician (Site Supervisor) | Electrical Supervision, Site Management, Electrical Installation | Balfour Beatty, Kier Group, Willmott Dixon, Schneider Electric UK, Octopus Energy Group |

Zero marketing/tech bias in any of the three, matching the pre-deploy self-test. Three throwaway test accounts (auth users + profile rows) created for the test were deleted immediately after via the Supabase Admin API; confirmed zero remain.
**Stage 52 (prior):** Session AG, adding cache visibility, fixing the cost model, deploying, and verifying caching against live production. **The headline finding was that nothing from Stages 47 to 51 had ever been deployed:** the newest production deployment was from 13 August, seven days stale, so every improvement existed only on GitHub. **Root cause, now confirmed via the Vercel API: the project has NO GitHub connection at all (`link: null`). It has been CLI-deploy-only since it was created on 20 May.** Pushing to `main` has never deployed anything and never will. Deployed `dpl_8BWsLwouheCusZ4AQCQQ5EWVUByV` (READY, production). **Caching then verified live: call 1 `cacheCreationTokens: 4510`, call 2 `cacheReadTokens: 4510`, and a 5-job burst returned 5/5 hits with the prefix unchanged at 4510 across completely different companies and roles, proving the Stage 47 split holds.** Across 12 successful production calls: 1 write, 11 reads, **92% hit rate**. Also fixed two real cost bugs: `estimateCost()` ignored cached tokens entirely (under-recording a cache miss by ~70%), and the Haiku rate table read 0.80/4.00 against an actual 1.00/5.00. **Historical `ai_usage` figures before Stage 52 are understated by roughly 20% from the rate error alone; past comparisons are corrected, not invalidated.**  
**Stage 51 (prior):** Session AF, reverting the Sonnet switch and growing the shared `RUBRIC` past Haiku's ~4,096-token floor instead. `analyse` is back on Haiku. `lib/scoring.js` `RUBRIC` went from 1,185 to **18,673 chars** of genuine scoring content: per-factor criteria for all 8 factors (what a 9 versus a 5 means on each, which was simply absent), a signal decision rule defining apply/maybe/dont_apply thresholds (also absent, despite the schema demanding one), 10 worked examples, 9 edge cases, and a common-errors list. **Caching now verified working on Haiku:** call 1 `cache_creation_input_tokens: 4994`, call 2 `cache_read_input_tokens: 4994`. **The HARD_FILTERS edge case is fully closed** — a user with zero filters set now measures **4,508 tokens** and caches too (proven live: 4509 written then read). All 4 test suites pass; build exit 0. **BUT the corrected cost model changes the recommendation:** Stage 50 quoted 0.91x for this option, which was wrong because it modelled only cache hits and ignored the write. Real break-even is a **93% cache hit rate**; below that this is more expensive than the small uncached rubric it replaced (2.49x on a pure miss). With the default 5-minute TTL you need roughly 15+ analyses per window to break even. **Keep this change for the scoring-quality gain, not as a cost saving.**  
...
**Last commit:** `823bcd0` (CV-personalisation fix, deployed to production 2026-08-29 as `dpl_3avNxrsfVaTv8rAitp7ZscZAuof8`)  
...
### Stage 53 — Session AH: CV-personalisation root-and-branch fix (2026-08-29)

**Diagnosis (no code changed in that pass).** Rob tested the live product with a healthcare CV (Senior Sister, clinical nursing) and got Anthropic/Stripe/Figma/Monzo target companies and marketing-adjacent role suggestions. Traced every place the app assumes a senior-marketing-in-tech user instead of reading the actual CV:

| Location | What was hardcoded | Severity |
|---|---|---|
| `app/api/onboard/parse-cv/route.js` | `ROLE_FAMILIES`/`INDUSTRIES` closed lists; the model's real output was filtered against them (`.filter(r => ROLE_FAMILIES.includes(r))`) — confirmed the model had already correctly said "Clinical Nursing" and the filter discarded it | Critical |
| `app/app/page.js` `WishlistTab` | `mkr_companies_autogen` localStorage flag gated the one-time AI auto-generation; once set in a browser it never re-fired, even after the CV was fully replaced | Critical |
| `app/app/page.js` `WISHLIST_SEEDS.standard` | Force-merged onto every non-standard track (`if (!tracks.includes('standard')) WISHLIST_SEEDS.standard.forEach(...)`) — the exact Anthropic/Stripe/Figma/Monzo list Rob saw | High |
| `app/api/cron/gov/route.js`, `app/api/feed-gov/route.js` | `TITLE_REJECT` actively excluded `'nurse'`, `'doctor'`, `'clinical'` (and finance/legal/engineer/etc) from the public-sector feed for every user regardless of profile | Critical |
| `app/api/cron/adzuna/route.js` | Shared nightly `ROLE_QUERIES` — 15 fixed marketing/tech search terms, the only source for the default Live Roles view | Critical |
| `app/api/wishlist/check/route.js` | `KW`/`REJECT` — marketing-keyword allowlist plus a hard reject on finance/legal/engineering titles, applied when checking if a wishlisted company has matching roles | High |
| `lib/ats.js` `ATS_COMPANIES` | 43 companies, entirely fintech/SaaS | High (see "not fixed" below) |
| Various placeholder copy | "Sarah Chen, VP Product", "Head of Marketing at Sky" STAR examples, founder-only `DEV SKIP` autofill (Partnerships/Fintech/Director) | Low |

**Fix, per Rob's explicit decision: open-vocabulary, not an expanded fixed list.** "The day a welder or a paramedic signs up we'd fail again and not know until they told us" — role families/industries are derived from the CV, open-ended, shown as editable suggestions, never filtered against a canonical taxonomy. Five principles applied throughout: (1) CV-derived data always wins over a fixed list; fixed lists are last-resort fallback only. (2) No silent fallbacks — every fallback firing is logged. (3) Feed queries/Target Companies/ATS boards/WLB list all derive from the user's actual sector. (4) No sector-blanket rejects. (5) Placeholder copy swept neutral.

**Changes:**
1. **`app/api/onboard/parse-cv/route.js`** — `ROLE_FAMILIES`/`INDUSTRIES` renamed to `ROLE_FAMILY_EXAMPLES`/`INDUSTRY_EXAMPLES` and repurposed as shape-only prompt examples, never a filter. Response sanitisation now trims/dedupes/caps instead of filtering against a list. Seniority IDs stay a closed enum (a structural level, not a profession). `app/onboard/page.js` step 3 reordered so CV-derived roles/industries render as prominent, removable pills above the fixed chip grid (the UI plumbing for this already existed — the bug was purely the API-side filter discarding non-canonical suggestions before they ever reached it).
2. **Target Companies regeneration** — new `lib/text-hash.js` (`hashText`, dependency-free, works client+server). `hard_filters_json.wishlistCvSignature` now records which CV a wishlist was generated for (`app/api/wishlist/save/route.js` persists it); `WishlistTab`'s mount effect compares it against the current CV's signature and regenerates on mismatch, replacing the `localStorage` flag entirely. Every fallback (`seedFallback()`) logs via `track('wishlist_fallback_seeds', {reason, tracks})`. The standard-track force-merge is deleted (both occurrences — mount effect and the generation-failure path). `wishlist/generate`'s prompt gained an explicit instruction to reflect the person's real sector when it isn't tech/fintech.
3. **Feed queries** — new `lib/aggregate-role-queries.js`: `cron/adzuna`'s query set is now `BASE_ROLE_QUERIES` (broadened from 15 to 24 entries — added nursing/teaching/electrical/accountancy/legal/social-work/allied-health) plus real distinct `target_roles` aggregated from actual user profiles (deduped against the base, capped at 20 extra) — still one shared nightly batch, budget-gated exactly as before (Cost Guardrails Rule 1 untouched; combined nightly cron volume rises from ~42 to a bounded ~76, well under the 220 daily cron ceiling). `cron/gov`'s `GOV_QUERIES` widened (+5 healthcare/education entries); its `TITLE_REJECT` profession list removed, replaced with a 6-term junior-noise-only list; `TITLE_MUST` extended with cross-sector senior terms (consultant/principal/matron/sister/headteacher). `feed-gov`'s per-user fresh-scan now matches on the user's own `target_roles` via new `lib/role-keywords.js` (`titleMatchesRoleKeywords`) when set, falling back to the old generic seniority-word filter only when a profile has no target roles at all. `wishlist/check` now derives match keywords from the signed-in user's own `target_roles`/`cvKeywords` (fetched server-side, already authenticated) instead of the fixed marketing KW/REJECT lists.
4. **`lib/match-engine.js`** — added `healthcare`/`education`/`trades` buckets to the `ROLE_FAMILIES` domain-conflict map (purely additive; existing 28/28 tests unchanged) so cross-domain word-overlap false positives (e.g. a pharma sales title sharing "nurse") get the same family-conflict guard other sectors already had.
5. **Onboarding UI** — CV-upload step now has an explicit "No CV?" toggle button instead of an implicit empty-state unfold. Founder-only `DEV SKIP` no longer autofills Partnerships/Fintech/Director (also silently fixed a latent bug: it called `setField(...)`, which doesn't exist — the real setter is `setFields` — so the button threw on click). Placeholder copy swept: interview-prep interviewer example, STAR-story examples, CV-upload no-CV form fields/placeholders.
6. **`BALANCED_COMPANIES`** — added one sourced NHS entry (Glassdoor's general "NHS" listing: 3.3/5 WLB from 14,689 reviews; real Agenda for Change national terms: 8 weeks full pay + 18 weeks half pay maternity leave, day-one flexible-working right) rather than fabricate trust-specific stats. Added `Healthcare` to `BALANCED_SECTORS`.

**Self-test (live, real Anthropic calls, cost-guardrail-compliant — cheap Haiku, no per-user-click scaling):**

| CV | Role families suggested | Target companies (sample) |
|---|---|---|
| Nurse (Senior Sister, NHS pain management) | Clinical Nursing, Pain Management Specialism, Nurse Leadership | Bupa, Spire Healthcare, Ramsay Health Care UK, Cygnet Health Care, Nottinghamshire Healthcare NHS FT |
| Secondary school teacher (Head of Geography) | Secondary Geography Education, Department Leadership, Curriculum Design | Pearson Education, OCR, AQA, Ark Schools, Teach First |
| Electrician (Site Electrician, commercial construction) | Electrical Trades, Site Supervision, Commercial Electrical Installation | Laing O'Rourke, Kier Group, Skanska UK, Morgan Sindall, Caple Electrical |

Zero marketing/tech bias in any of the three. Role-family test ran through the real `/api/onboard/parse-cv` endpoint end-to-end; Target Companies test ran the identical prompt template from `wishlist/generate` directly against live Haiku (bypassing only the session-auth layer, not the logic) since minting a real Supabase session for a throwaway test account was judged not worth the added fragility for this pass. `lib/aggregate-role-queries.js` verified against live production data (confirmed the diagnostic session's real nurse test profile — `target_roles: ["Advanced nurse practicioner","nurse","ADHD Nurse"]` — is picked up and correctly deduped against the broadened static floor). Full 8-file test suite: all 8 files report 100% pass (`ai-context` 25/25, `freshness` 20/20, `loop-guard` 17/17, `match-engine` 28/28 — unchanged despite the new role-family entries, `uk-eligibility` 26/26, `scoring` and `usage-window` both "ALL PASS"), zero regressions. `next build`: exit 0, 110 routes. No test data left in the live DB (all self-tests used synthetic CVs sent directly to the model, not saved profiles).

**Not fixed, flagged for Rob:** `lib/ats.js`'s ATS board list stays fintech/SaaS-only. Live-probed Cera, Huma, Teach First, Nuffield Health, Bupa, Babylon Health, Zava, and Thirty Madison against all 4 supported ATS providers (Greenhouse/Lever/Ashby/SmartRecruiters) — zero hits. Real NHS trusts, schools, and trade bodies hire through entirely different systems (NHS Jobs/TRAC, teaching-vacancy portals, trade-body listings) that this ATS abstraction doesn't cover — widening real coverage here is a new ingest-source feature (an NHS Jobs integration, a teaching-vacancies integration, etc.), not a config change to the existing company list. The Adzuna-based feed and wishlist/generate's AI search are the mechanisms that do cover these sectors today.

**Not deployed** — per instruction, implementation is committed locally only; `vercel --prod` was deliberately not run this session.

### Stage 52 — Session AG: cache visibility, corrected cost model, first deploy in 7 days, live verification (2026-08-20)

**The finding that reframed everything: none of Stages 47 to 51 had ever been deployed.** Rob asked me to verify caching on live production. Before spending anything, `vercel ls marker` showed the newest production deployment was **13 August, seven days old**, predating every commit in this arc. Production was running `ce62112` (Stage 46), which has **both** cache failure modes at once: the per-job `${company}` / `${roleTitle}` still inside the cached block (the Stage 47 bug) **and** the old ~1,000-token rubric, far under Haiku's ~4,096 floor. So the honest answer to "is caching firing in production" was **no, and it could not have been.**

**Why deploys were not happening (Rob's question 5), answered definitively.** Not an inference from the deployment list. Queried the Vercel API directly:
```
GET /v9/projects/prj_isZvOS4FtY8HOYOoyE7xZPDcPpfJ  ->  "link": null
```
**The project has no connected Git repository. It has been CLI-only since it was created on 20 May 2026.** `vercel project inspect` shows no Git section, and every deployment in the list is attributed to Rob's username rather than a commit author. **Pushing to `main` has never triggered a deploy and never will.** Every previous stage that ended with "committed and pushed" left production untouched. Connecting the GitHub repo in the Vercel dashboard is the fix, and it cannot be done from the CLI.

**Visibility added (item 1).** `app/api/analyse/route.js` now returns `cacheCreationTokens` and `cacheReadTokens` on both success paths, mirroring the `cacheReadTokens` convention already used by `lib/score-jobs-batch.js` and `pipeline/tidy-up`. Without this, no external test could ever confirm caching on this route: the response carried no usage, nothing logged it, and `ai_usage` did not store it.

**Two genuine cost bugs fixed (item 2).**
1. `estimateCost()` billed only `input_tokens` and `output_tokens`. **Cached tokens are excluded from `input_tokens` but are still billed**, so once Stage 51 shipped, a cache miss of 4,994 tokens at 1.25x was being recorded as zero. Now takes `cacheWriteTokens` at **1.25x** and `cacheReadTokens` at **0.10x** the model input rate. Measured impact on a cache miss: the old calculation recorded **£0.002130** where the true cost is **£0.007061**, an under-report of **70%**.
2. The rate table listed Haiku 4.5 at `input: 0.80, output: 4.00` against an actual **$1.00 / $5.00**. Corrected. **Historical `ai_usage` rows written before this stage are understated by roughly 20% from this alone. Past comparisons are corrected rather than invalidated, since the error was uniform across every Haiku row.**

Note the `ai_usage` table has **no columns for cache tokens** (`id, user_id, account_id, model, action, input_tokens, output_tokens, cost_estimate_gbp, created_at`), so the cache counts are folded into `cost_estimate_gbp` only. Storing them as queryable columns would need a migration. Also flagged and NOT changed: the table lists `claude-opus-4-8` at 15.00/75.00 against an actual 5.00/25.00. It is dead config, since `lib/anthropic.js` `MODELS` only exposes haiku and sonnet.

**Deployed (item 3).** `vercel --prod --yes` from `~/dev/marker`. `dpl_8BWsLwouheCusZ4AQCQQ5EWVUByV`, `readyState: READY`, target production, exit 0. First production deploy since 13 August.

**LIVE VERIFICATION (item 4).** Two identical calls to `https://marker-silk.vercel.app/api/analyse`, 12 seconds apart:
```
call 1:  cacheCreationTokens=4510   cacheReadTokens=0
call 2:  cacheCreationTokens=0      cacheReadTokens=4510
```
**Caching is firing correctly in production.** The 4,510-token prefix matches the 4,508 measured locally in Stage 51 for the no-profile case almost exactly, confirming the deployed prompt is the Stage 51 rubric.

Five-call burst, five different companies and roles, four seconds apart:
```
HIT  read=4510   Head of Growth / Starling Bank
HIT  read=4510   Product Marketing Lead / Sony
HIT  read=4510   Digital Transformation Manager / NatWest
HIT  read=4510   Senior Technical Sales Executive / Oracle   -> signal dont_apply
HIT  read=4510   Interim Partnerships Director / Deliveroo
```
**5/5 hits, and the prefix is byte-identical at 4510 across completely different jobs.** This is the direct proof that the Stage 47 split works: per-job `company` and `roleTitle` live in `SYSTEM_VOLATILE`, after the breakpoint, so varying them does not invalidate the cache. Under the pre-Stage-47 code every one of those would have been a miss.

**Honest note on the first burst attempt:** the first call of the first burst returned no fields at all. It was **not** a cache miss, it was a failed response, almost certainly a cold start on a function deployed 90 seconds earlier. Re-running the identical payload succeeded immediately. The clean warm-run figures are the ones quoted above; both runs are reported rather than only the flattering one.

**Overall hit rate across the whole session's production traffic:** 13 calls attempted, 12 successful, 1 cold-start failure. Of the 12 successful, **1 wrote the cache and 11 read it: a 92% hit rate.** Worth noting that lands almost exactly on the **93% break-even** computed in Stage 51, so on this usage pattern the grown rubric is roughly cost neutral while delivering the better scoring. Real user traffic is unlikely to be this bursty, so treat 92% as an upper bound, not a forecast.

**Behavioural confirmation the Stage 51 rubric is live:** the Oracle listing (quota carrying, five days on site) returned `signal: dont_apply`. That is the new `SIGNAL_RULE` firing on the skills-3-or-below condition. The old rubric had no signal thresholds at all.

**NOT verified:** the `estimateCost` fix is confirmed by arithmetic, not by a production `ai_usage` row. All test calls were unauthenticated, and `trackAiUsage` no-ops without a `userId`, so nothing was written to the table. It will exercise on the first real signed-in analysis.

**NOT done — carried forward:**
- **Connect the GitHub repo in the Vercel dashboard** so pushes deploy. This is the single highest-value item here: without it every future "committed and pushed" is again a no-op in production. Cannot be done from the CLI.
- Decide whether to add `cache_creation_tokens` / `cache_read_tokens` columns to `ai_usage` so cache behaviour is queryable rather than only visible in a response body.
- Fix or remove the `claude-opus-4-8` rate (15.00/75.00 against an actual 5.00/25.00).
- Everything from Stage 51 (whether the grown rubric earns its keep at real hit rates) and Stage 49 (delete `~/Desktop/marker-OLD-icloud-DELETE-ME`; other repos still on iCloud).
...
Rob asked to see the Stage 45 top-10 result with his own eyes before trusting it. Two of the ten — a Business Analyst role and a "Lead Software Engineer (Python & Django)" role — had `roleFit` correctly scored at 2 ("doesn't match") but still made the top 10 at 6.6/10 overall, because roleFit is only 25% of the weighted average and their seniority/location/comp/freshness scores were strong enough to buy back in. Rob's instruction: fix this as a principle, not a one-off cap — role relevance should GATE entry to the feed; the other six dimensions may only rank roles that already passed the gate against each other, never lift an irrelevant one into view.

**Where the threshold was drawn, and why.** `scoreRoleFit`'s buckets are 10 (jaccard ≥0.40), 8 (≥0.25), 6 (≥0.12, "some skill match"), 4 (≥0.04, "limited overlap"), 2 (<0.04, "doesn't match"), plus 3 for an explicit functional-family conflict. Rob specifically flagged bucket 4 as ambiguous — allow or exclude? Reasoning: the 4-12% overlap band represents, in practice, one or two incidentally shared words between otherwise-unrelated titles (jaccard is shared/union of significant tokens, and a title pair needs real vocabulary overlap to clear even 12%) — the same character of false signal as the "lead"/"engineering" bug fixed in Stage 45, not a genuine partial match. So the gate excludes buckets 2, 3, and 4 (`ROLE_FIT_GATE_MIN = 6`), keeps 6/8/10. Documented inline in `lib/match-engine.js` (both a file-header "GATING PRINCIPLE" block and a comment on the constant itself) so the reasoning survives the next person who touches this file, not just this session's log.

**Never gates on missing data.** `scoreRoleFit` now returns `assessed: false` for its two "we don't know" cases (no job title to compare; no target roles or CV keywords set on the profile at all) — both already returned the neutral score 5, which is below the gate threshold and would have wrongly excluded every job for a profile that simply hasn't set target roles yet. The gate checks `roleFit.assessed && roleFit.score < 6`, so `assessed: false` never triggers exclusion on its own. Verified live: an empty-`target_roles` profile scores every job at roleFit 5/`assessed:false`/`excluded:false`.

**Implementation — one place, not five.** Added `excluded`/`excludeReason` to `scoreMatch`'s return value (`lib/match-engine.js`), folding in the pre-existing location/seniority hard-mismatch checks (previously replicated ad-hoc across routes) alongside the new role-relevance gate — a single computation every caller inherits. Updated all 5 routes that call `scoreMatch` (`feed-cache`, `feed-web`, `feed-gov`, `job-feed`, `contractor/roles`) to filter on `!relevance.excluded` instead of their own inline dimension checks, so the rule can't drift out of sync route-by-route the way the original relevance-ranking gap (Stage 45 Critical 1) did.

**Self-test — live, against production:**
- Replayed the exact two leaked jobs through the real `scoreMatch`: both now return `excluded: true` with the correct reason.
- Confirmed genuinely relevant roles (Interim Programme Manager, Head of Customer & Revenue Strategy, Lead Product Manager) are untouched (`excluded: false`).
- Confirmed the no-target-roles edge case never gets excluded on missing data alone.
- `node lib/match-engine.test.js` — all 28 cases still pass.
- `npx next build` clean; deployed to `marker-silk.vercel.app`.
- Live `GET /api/feed-cache` for `robert@oxocreatives.com` (real minted session), fresh top 10 — every entry now roleFit ≥ 6, zero leakage:

  | # | Score | Role | roleFit |
  |---|---|---|---|
  | 1 | 7.5 | Programme Manager — Pearson Whiffin IT & Digital | 10 |
  | 2 | 7.3 | Interim Capital Programme Manager — Hays | 8 |
  | 3 | 7.1 | Interim Programme Manager — Adecco (London) | 8 |
  | 4 | 7.1 | Interim Programme Manager — Adecco (Farringdon) | 8 |
  | 5 | 7.0 | Major Projects Programme Delivery Lead — Sheer Jobs Ltd | 8 |
  | 6 | 6.9 | Technical Programme Manager — Michael Page | 10 |
  | 7 | 6.8 | Head of Customer & Revenue Strategy — DFT Operator | 8 |
  | 8 | 6.7 | Lead Product Manager - Central Government — Spinwell Global | 6 |
  | 9 | 6.6 | Interim Head of Finance \| Rapid Growth FinTech Scale-Up — Harmonic Group Ltd | 6 |
  | 10 | 6.6 | Digital Platform Product Manager — Lloyd Recruitment - Epsom | 8 |

- **Coverage check (the thing Rob asked to see, not just trust):** ran the real `scoreMatch` against the full live `jobs_cache` table (3,235 rows, not just feed-cache's 500-row recent window) with this profile. **349 roles pass the gate (≈11% of the cache).** Breakdown of what's excluded: 2,808 role mismatches, 78 location mismatches, 0 seniority mismatches. 349 is a healthy working set (well above the ~5-8 live roles the product's own onboarding copy suggests as a realistic pipeline size) — **not** a coverage problem requiring feed-widening right now. Recorded per Rob's own instruction: if this number had been thin, that would have been the next problem to solve instead of loosening the gate; it isn't, so no further action here.

### Stage 45 — Session Z: fix pass on every Stage 44 finding + Adzuna dedup, 2 new critical bugs found by self-testing (2026-08-13)

Rob's framing: fix all 12 numbered items from the Stage 44 diagnostic, then re-confirm feed relevance live, self-test payment and CV generation with real calls, no time cap. A mid-session message added a 13th item: audit Adzuna dedup across every ingestion path. Every fix below was self-tested against the live production DB/API before being reported done, per standing instruction.

**CRITICAL 1 — feed relevance ranking.** The architecture Rob asked for (score once nightly, rank per-user deterministically at read time, never re-score per user) turned out to **already exist** in `feed-web`/`feed-gov`/`job-feed`/`contractor/roles` — all four already read the nightly Haiku baseline (`match_score`, written by `cron/score-cache`) and rank with the zero-AI `lib/match-engine.js`. The one gap: `feed-cache`, the actual route the Live Roles tab loads on every page view, never did this — it only excluded hard location/seniority mismatches and left everything else in raw `cached_at` order. Rewrote `feed-cache` to match the established pattern: run `scoreMatch(profile, row)` (plain JS, zero API cost) over the fetched page, filter hard mismatches as before, sort by relevance descending, return `relevanceScore`/`relevanceDimensions` per job. Confirmed live against `robert@oxocreatives.com`'s real profile and real `jobs_cache`: the *first* live test still put "Tech Lead - Software Engineering" and "Lead IAM Technical Architect" in the top 5 (roleFit 6-8) — a **second bug**, not fixed by the ranking wiring alone: `scoreRoleFit`'s jaccard word-overlap was matching on the bare word "lead" (present in the target "Programme Lead" and in almost every seniority-flavoured title regardless of function) and `familiesConflict`'s safety net never fired because the `engineering` family list only had the word "engineer", which `matchesKeyword`'s word-boundary regex never matches inside "engineering". Fixed both in `lib/match-engine.js`: added `'lead'` to `LOW_SIGNAL_WORDS` (verified it doesn't harm real matches — "Interim Programme Manager" still scores 8 via "programme"), and added `'engineering'`/`'architect'` to the `engineering` family. Re-tested live after redeploy: top 10 for the marketing/partnerships profile became Programme Manager, Interim Capital Programme Manager, Interim Programme Manager (×2), Product Design Lead, Major Projects Programme Delivery Lead, Technical Programme Manager, Head of Customer & Revenue Strategy, Lead Product Manager, Senior Technical Business Analyst — zero HR, warehouse, construction, or pure software engineering. All 28 `match-engine.test.js` cases still pass.

**CRITICAL 2 — upgrade flow.** Rewrote `/pricing` (`app/pricing/page.js`) as a client component: Pro/Max buttons call the same `/api/stripe/checkout` Settings already used, logged-in users go straight to Stripe; logged-out users are sent to `/auth?next=%2Fpricing%3Fcheckout%3D<plan>` (reusing `/auth`'s existing, already-working `?next=` round-trip through both password and magic-link sign-in) and the pricing page auto-fires checkout the moment it detects a returning, now-authenticated user with a queued plan — never dumped on the dashboard. Confirmed the new logic is in the deployed bundle. **Self-test surfaced two real Stripe-side blockers**, not code bugs: (1) `app/api/stripe/checkout/route.js` had **zero error handling** — any Stripe failure (stale customer id, invalid price) threw unhandled, returning an empty-body 500. Added a full try/catch, a stale-customer self-heal (`stripe.customers.retrieve` before reuse, falls back to creating a fresh one), and a guard against placeholder price ids. (2) With real errors now surfacing: **both** `PLANS.pro.priceId` and `PLANS.max.priceId` in `lib/stripe.js` are invalid in the connected Stripe account — Max was already known-placeholder (`price_TODO_max_monthly`), but live-testing Pro returned `"No such price: 'price_1TZbsgIzu43fVGB0nQ7Lp8Wz'"` from Stripe itself, meaning **neither plan's checkout has actually worked**, not just Max. This needs Rob's Stripe dashboard, not code — creating live Price objects is a real billing decision outside what I'll do unilaterally. **ACTION FOR ROB: create real monthly+annual Price objects for both Pro and Max in the Stripe dashboard and paste the 4 price ids into `lib/stripe.js`.** Once that's done the flow is code-complete and will work end-to-end; the error handling now means a bad id fails with a clear message instead of a blank 500.

**CRITICAL 3 — CV generation.** Fixed the reported bug: `DirectCvPanel` (`app/app/page.js`) checked `selectedJob?.jdRaw`, which was never set anywhere; changed every reference to `selectedJob?.jd`, matching the property name used everywhere else (pipeline card, `lib/db.js`). Added a real paste-JD block when no JD is stored: a large textarea, a link out to the job posting when available, and a single "Save JD & generate →" action that persists the JD onto the pipeline item and generates in the same click — no more dead-end message. **Self-testing this via a real live `POST /api/cv/generate` call surfaced a second, independent, more serious bug**: the response came back `200` with `type:'cv'` but `text` was completely empty. Traced it live: Claude Sonnet 5 engages extended thinking automatically on a sufficiently complex prompt (verified by replaying the exact prompt directly against the Anthropic API — `content: ['thinking','text']`, 682 thinking tokens) **even with no `thinking` parameter requested anywhere in the code**. `cv/generate`'s main call read `msg.content[0]?.text`, which is the thinking block, not the text, on exactly this call shape — so the flagship CV generator has been producing empty output on every real `standard`/`deep` call. Grepped the whole codebase for the same fragile pattern and found 4 more instances (`onboard/parse-cv`, `cv/generate`'s gap-analysis sub-call, `cv/cover-letter`, `wishlist/generate`); every other Anthropic call site already used the robust `.filter(c => c.type === 'text').map(c => c.text).join('')` pattern (established precedent in `analyse`/`interview-prep`/etc), so this was a real, bounded bug class, not systemic. Fixed all 5. Re-tested live after redeploy: a real `standard`-effort CV generated in full (5,532 characters), evidence-tracing to the candidate's actual NatWest history, `flaggedMetrics: []` (no invented stats). Cleaned up the `ai_usage` rows my test calls created (the account's genuine monthly cap is untouched).

**HIGH 4 — geo-filtering.** `postcode`/`radiusMiles` were read by nothing in `lib/match-engine.js` — confirmed via full-file grep before touching anything. Built `lib/uk-geo.js`: a static ~60-entry UK city/town coordinate table plus a postcode-outward-code → city map (covers common areas including Epsom/KT for Rob's own upcoming move) and a haversine distance function — zero external API calls, consistent with the codebase's existing pragmatic patterns (the `ukCities`/`UK_SIGNALS` lists). Wired into `scoreLocationFit`: for any non-remote role, resolve both sides' location, and if they're outside the profile's radius, that's now a hard mismatch (score 1) *in addition to* the existing office-days logic — never replacing it, so nothing is double-filtered. Unresolvable locations (unknown town names) fall through to the pre-existing neutral behaviour unchanged. Self-tested: London profile scores Manchester/Edinburgh as hard mismatches (~163/332 miles, excluded), Reading (~40mi) and London itself unaffected, fully-remote roles unaffected by distance at all.

**HIGH 5 — sort/filter UI.** Added to the Live Roles toolbar in `app/app/page.js`: a sort control (Best match / Newest / Salary — client-side re-sort of the already-fetched, already-relevance-sorted page, no re-fetch), a minimum-relevance filter (Any / 6+ / 8+, using the new `relevanceScore` field), and an "Include out-of-area roles" toggle that re-fetches `feed-cache` with `?broaden=1` (the existing param, previously unused by the frontend). Kept the existing visual design and the existing posted-within control untouched.

**HIGH 6 — invisible button.** `color: 'var(--marker-black)'` was hardcoded regardless of state on the "Add to pipeline" button; fixed to `isAdded ? 'var(--marker-black)' : 'var(--marker-cream)'` matching the already-correct background logic. Found and fixed **two** instances (Live Roles feed card and the contractor-roles feed card — same copy-pasted bug in both).

**MEDIUM 7/8 — CV mechanism consolidation + signposting.** Rob's decision: AI Generate primary, one copy-paste fallback, remove the redundant third. Removed the standalone "Tailor CV" tab (`CvGeneratorFlow`, ~230 lines, was one of the two copy-paste tools) entirely; `DirectCvPanel`/"AI Generate" is now the default section for every non-contractor-only user. Kept `ContractorCvPanel`/"Contractor CV" as its own tab — it's a genuinely different tool (untargeted mailshot CV, not tied to any one role), not a duplicate of the role-specific pair that was actually causing the confusion. Added ONE copy-paste fallback inside `DirectCvPanel` itself: a "Prefer to use your own ChatGPT/Claude? Copy the prompt instead →" toggle that reveals a clearly-numbered 3-step block (1. Copy this prompt → 2. Paste into Claude/ChatGPT → 3. "Come back to this tab when it's done" in a highlighted lime callout) — satisfies Medium 8's signposting ask directly. Re-wired the pipeline card's "Tailor CV" button (`prefill`) through to `DirectCvPanel` so the pre-select-a-role convenience isn't lost now that `CvGeneratorFlow` is gone.

**PROFILE FIXES 9-12:**
- **9 — salary_floor.** Replaced the free-text £ input with a fixed dropdown (£50k/60k/70k/80k/90k/100k/120k/150k + "No minimum") in both `app/onboard/page.js` and `app/settings/page.js`, storing a clean integer — the exact bug class (85 typed where "85" meant thousands, stored as £85,000,000) can no longer happen. The CV-derived salary hint now snaps to the nearest fixed option rather than pre-filling an orphan value. **Repaired Rob's own broken value directly in the live DB**: `salary_floor` was `85000000`, set to `80000` (nearest fixed option to the implied £85k).
- **10 — professional field multi-select.** Converted `field` (single string) to `fields` (array) in both onboard and settings, using the existing Chip-toggle pattern already used for benefits/industries. Found and fixed 7 read sites across the codebase (`app/app/page.js`, 3 contractor routes, `perm/recruiters`, `wishlist/generate`, `lib/scoring.js`) that assumed a plain string — all now array-safe and handle legacy single-string profiles (like this account's existing `"Marketing"`) gracefully via `Array.isArray` checks, no migration needed.
- **11 — sales-quota explainer.** Added "Roles where part of your pay depends on hitting sales targets, e.g. commission or a quota-based bonus." under the toggle in both onboard and settings.
- **12 — the inert `archived` flag.** **Decision: removed, not wired up.** It was written by a cron (`cron/archive-inactive`, scheduled nightly in `vercel.json`) that did nothing but flag 30-day-inactive profiles, and read by zero code anywhere — confirmed via full grep. There's no existing feature (reactivation flow, admin filter, win-back email) that would give it purpose, and inventing one wasn't in scope for a fix session. Deleted the route and its `vercel.json` cron entry; the field may still exist as harmless residue on old profile rows.

**ADZUNA DEDUP (mid-turn addition):**
1. **Audit: external_id is clean.** Checked every ingestion path (`cron/adzuna`, `cron/gov`, `cron/contract`, and the `feed-web`/`feed-gov`/`contractor/roles` fresh-scan paths) — all correctly derive `external_id` from Adzuna's stable numeric ad id (`adzuna-${job.id}` / `gov-${job.id}`), never from the volatile `redirect_url`. Rob's leading hypothesis was reasonable but not what's happening here.
2. **Quantified the real cause: content-level duplicates.** Pulled the full `jobs_cache` table (3,342 rows) and grouped by normalised company+role+location: **100 duplicate groups, 107 redundant rows (~3.2%)**. 49 are genuine Adzuna reposts (the same real ad re-crawled under a new numeric id — market noise, not a Marker bug); 29 are the identical underlying Adzuna ad pulled independently by both `cron/adzuna` and `cron/gov` (confirmed via matching numeric ad ids under different `adzuna-`/`gov-` prefixes — `cron/gov` genuinely queries Adzuna's own API with public-sector-flavoured search terms, it isn't a separate government board); 15 greenhouse-only; 7 gov-only. **Deleted all 107 rows after confirming with Rob** (kept the highest-baseline-score, most-recently-verified row per group).
3. **Cross-source dedup at display time.** Built `lib/dedupe-jobs.js` (normalise company+role+location, keep the best-scored/most-recent row per group) and wired it into both `feed-cache` (all 3 sources combined — where cross-source duplicates were actually visible) and `feed-web` (Adzuna-only, catches pure reposts). Runs before ranking so a duplicate never displaces a genuinely different role.
4. **Pipeline-level exclusion, fixed to use the stable id.** The "already added" check matched on `job.link`/`job.url` — Adzuna's `redirect_url` carries a session-specific hash that changes on every API call, so a job added today could reappear in the feed tomorrow because the freshly-refreshed row's link no longer matches what's stored on the pipeline item. Added `job_cache_id` end-to-end: `lib/db.js`'s `jobToRow`/`rowToJob` now read/write it, `addToPipeline` (all relevant call sites) passes the real cache row id when adding from a feed card, and the "already added" checks now match on `job_cache_id` first, falling back to link matching only for older/custom-added rows that predate this.

**SELF-TESTS — all live, against production DB/API, per standing instruction:**
- Feed relevance: `GET /api/feed-cache` called with a real minted session for `robert@oxocreatives.com` (password grant + reconstructed `@supabase/ssr` cookie — the Chrome extension was unresponsive this session, so browser-level testing wasn't available; this is the equivalent live-HTTP verification). Top 10 confirmed genuinely relevant after the roleFit fix (see Critical 1).
- Payment: `POST /api/stripe/checkout` called live; confirmed the code path is correct and now surfaces the real Stripe blocker (invalid price ids) instead of an opaque 500 — see Critical 2.
- CV generation: `POST /api/cv/generate` called live with a real JD; confirmed a full, evidence-backed CV comes back after the content-block fix — see Critical 3. Test-generated `ai_usage` rows deleted afterward so the account's real allowance is untouched.
- `for f in lib/*.test.js; do node "$f"; done` — all 8 suites, 183 tests, pass (including `match-engine.test.js` after the roleFit changes).
- `npx next build` — clean, twice (once before the roleFit/content-block fixes, once after).
- Deployed to production 3 times this session as fixes were found via live testing; final deploy aliased to `marker-silk.vercel.app`.

**Repo cleanup:** deleted `JOURNEY-AUDIT 2.md` (a stale, pre-fix duplicate of the already-tracked `JOURNEY-AUDIT.md` — diffed to confirm). Kept `REQUITE-MARKETING-COPY-v2.md` and added it to git — it's the real reference doc that guided the Stage 12 marketing pass, not scratch material. Added `supabase/.temp/` (the Supabase CLI's local cache) to `.gitignore`.

**Remaining, flagged for Rob:**
- Stripe: both Pro and Max price ids in `lib/stripe.js` are invalid in the connected account — needs real Price objects created in the Stripe dashboard (see Critical 2).
- MARKER-COST-GUARDRAILS.md doesn't yet mention that Sonnet 5 can engage extended thinking unprompted on complex prompts, which affects both output-parsing (fixed this session) and cost (thinking tokens bill as output tokens) — worth a line in the guardrails doc next time it's touched.
- No pure "Marketing"/"Partnerships"-titled roles reached the top 10 in the live relevance test despite being 2 of the 5 target_roles — plausibly just reflects what's currently in the cache (Programme/Product roles may simply be more numerous right now) rather than a residual bug, but not independently verified; worth a glance if it recurs.

### Stage 44 — Session Y: diagnostic-only pass on 8 live-product findings + credit/dormancy re-check (2026-08-12)

Rob's framing: a month since the last session, tested the live product on his own old account (`robert@oxocreatives.com`) and got poor results across the feed, revenue flow, and CV tools. Diagnosis only, no fixes applied. Leading theory to confirm or rule out first: the account's profile data is stale/malformed.

**Preliminary checks (before the profile):**
- **Anthropic credit — RESOLVED, Stage 43's finding is stale.** Live `POST /v1/messages` with the `.env.local` key returns 200 for both `claude-haiku-4-5-20251001` and `claude-sonnet-5` with real generated text. `vercel env pull --environment=production` confirmed `ANTHROPIC_API_KEY` (and 5 other critical vars) is byte-identical between `.env.local` and Vercel prod. Prod `ai_usage` has real rows from today (`analyse` + `analyse_search`, 2026-08-12 13:53-13:54 UTC) — Rob's own test session, using the real model. Conclusion: none of the poor results below are the credit-outage fallback path; they are the real model/real code doing this.
- **Dormancy sweep (WAKING-UP-CHECKLIST.md) — clean.** Supabase awake (`jobs_cache` query 200). Newest cache row `2026-08-12T05:16Z` (~14h old at check time) — crons are running. `jobs_cache` total 3256 rows, 3255 Fresh / 1 Expired. Latest Vercel deploy live, homepage 200. `.env.local` vs Vercel prod identical on the 6 vars that matter. Dormancy does not explain any of the items below.

**Profile check (leading theory) — profile is NOT empty, stale, or off-topic. Ruled out as the primary cause, but found two real defects on this account.** Pulled `profiles` directly for `user_id 25d12e91-dd3e-4fbc-a65d-e56540c745eb`: `target_roles` = Product Management, Programme Lead, Digital Strategy, Growth, Partnerships; `industries` = Fintech, Gaming, Media, SaaS, Martech, Retail Tech, Public Sector, Charity, Consumer Goods; `postcode` = London; `seniority` = senior_manager; full CV text present in `hard_filters_json.cvRaw` (Meta/Sony/NatWest/King career history). This is a complete, correctly on-theme marketing/product/partnerships profile — not the cause of irrelevant categories in the feed. Two genuine defects found on this account: (1) **`salary_floor: 85000000`** (£85 million) — `profile/save/route.js:74` multiplies the input field by 1000 (`parseInt(p.salaryFloor) * 1000`), and both onboarding and settings label the input "in £k" (placeholder "e.g. 80/90", suffix "k / year"), so this value could only result from someone typing the full figure (e.g. "85000") into a field expecting "85". Data-entry error on this old account, not reproducible from current UI behaviour — but there's no sanity-check ceiling on the input, so nothing stops it. (2) **`hard_filters_json.archived: true`** — set by `cron/archive-inactive` (flags any profile after 30 days with no sign-in). Confirmed via full grep this flag is written but **never read** by any feed/matching route — inert metadata, not a cause of anything below.

**DIAGNOSIS TABLE**

| # | Item | Root cause | Data vs code | Scope | Severity |
|---|---|---|---|---|---|
| 1 | Feed shows HR/finance/construction/warehouse, barely any marketing | `app/api/feed-cache/route.js` pulls the 500 most-recently-cached rows from the shared `jobs_cache` (source in greenhouse/gov/adzuna) and applies only a coarse pre-filter that excludes hard **location/seniority** mismatches via `scoreMatch` (score===1). It **never filters or sorts by `roleFit`/`target_roles`/industries** at all — the 8-factor score (which does include role fit) is only computed on-demand when a user clicks "SCORE" on one card, never applied to the list. Compounding: the nightly `cron/adzuna` ingest runs 15 fixed, non-personalised keyword queries shared by every user (`ROLE_QUERIES` in `app/api/cron/adzuna/route.js`), and Adzuna's fuzzy free-text match on generic terms like "operations manager"/"programme manager" naturally pulls in warehouse ops, construction PM, insurance-adjacent titles. | **Code** | Everyone | **Critical** — this is the literal "pre-filtered listings" promise in the UI copy (`app/app/page.js:2456`), and it fails open by construction, not by a bug in a conditional — there was never a role-relevance filter to begin with. Recommended fix (not applied): apply `scoreRoleFit` (or a cheaper role-family match) as an actual filter/sort step in `feed-cache`, not just as an on-demand per-job score. |
| 2 | Pipeline had Lead Product Designer, Insurance Pricing Analyst, Solution Consulting director | Pulled all 4 `pipeline_items` rows directly: every one is a manually custom-added entry (`custom_company`/`custom_role` fields, no `job_cache_id` link) dated 2026-05-21/22 — the week this account was created. Old dev/test data added by hand, not something the live matching/feed flow auto-added. | **Data** (this account only) | Just this account | **Low** — clear the old test rows before further testing; not a bug. |
| 3 | No London roles despite a London profile | Grepped `lib/match-engine.js` in full: `profile.postcode` and `hard_filters_json.radiusMiles` are **never read anywhere** in the scoring engine — only mentioned in a doc-comment. What's named `scoreLocationFit`/"locationFit" is actually an office-days/remote-hybrid preference scorer; it has nothing to do with geography. So there's no active bug excluding London — there's simply **no geographic targeting at all**, so the feed is effectively UK-wide and London representation in any given 500-row window is down to chance. | **Code** (a collected, promised field is silently unused) | Everyone | **High** — users set a postcode + commute radius at onboarding and it does nothing. |
| 4 | 433 results, no sort, no filter UI | Confirmed in `app/app/page.js`: `filteredWeb`/`filteredGov` apply only a free-text search box and the "posted within" date window (lines 2433-2446) — no sort control exists, and (per item 1) no automatic relevance filtering happens server-side either. Never built beyond the search box, not a broken control. | **Code** (gap, not a broken feature) | Everyone | **High** — same root cause family as item 1. |
| 5 | Trial ended → View plans → forced re-login → dumped back to dashboard, no payment | Traced end-to-end. Dashboard's trial-ended banner (`app/app/page.js:5393`) links to `/pricing`. **All three plan buttons on `/pricing` hardcode `href: '/auth'`** (`app/pricing/page.js`, Free/Pro/Max all identical, no auth-state check, no plan id carried through). The real, working checkout call (`POST /api/stripe/checkout`) exists only in `app/settings/page.js` and is never linked from `/pricing` or the trial-ended banner. So an already-logged-in user is sent to the login page again, and after auth's redirect lands back on `/app` with no plan selected and no checkout ever initiated — exactly the loop Rob described. | **Code** | Everyone | **Critical** — this is the payment path; nobody can currently upgrade via the prompted route. Recommended fix (not applied): point `/pricing`'s CTAs (and the trial-ended banner) at the same `/api/stripe/checkout` call Settings uses, or route logged-in users straight to `/settings` instead of `/auth`. |
| 6 | "Tailored CV" → "No JD stored for this role, paste it first" but no paste field | Found a property-name mismatch. `DirectCvPanel`'s JD check and `generate()` call read `selectedJob.jdRaw` (`app/app/page.js` lines 1986, 1994-1995, 2051, 2071) — but `jdRaw` **is never set anywhere in the codebase**. Every other place a job's JD is stored or read — the pipeline card's own paste field (`jd: notes` at line 702), `negotiation-prep`'s call, every job-object initializer — uses the property name `jd`. So `DirectCvPanel` can never see a JD that was correctly pasted via the pipeline card; its own error message points the user to a fix that can never satisfy its own check, and the panel offers no paste field of its own. | **Code** (property-name drift) | Everyone | **Critical** — fully disables the in-app "AI Generate"/"Cover Letter" flow for every role, independent of any data condition. One-line-class fix once decided (`jdRaw` → `jd`, or vice versa, consistently). |
| 7 | Two CV mechanisms side by side | Confirmed, and there are actually **three**, all under the "CV tools" tab (`app/app/page.js` ~2170-2210): "Contractor CV" (`ContractorCvPanel` — copy-paste prompt for Claude/ChatGPT, JD-independent, mailshot-style), "Tailor CV" (`CvGeneratorFlow` — *also* a copy-paste prompt, JD-independent, built from role/company/CV text only — `buildCvPrompt` never touches `jd`/`jdRaw`), and "AI Generate"/"Cover Letter" (`DirectCvPanel` — the real in-app API-driven generator, gated by the broken check in item 6). These read as intentionally different tools (different copy explaining each, different purposes: bring-your-own-AI vs in-app paid generation) rather than leftover duplication — but the UI doesn't explain the distinction, and clicking "Tailor CV" on a pipeline card (`tailorCv()`, line 5217-5220) lands on the copy-paste flow by default, not the API one, which is a likely source of the "does nothing" confusion layered on top of item 6's real bug. | **Code/product** (IA confusion, not itself broken) | Everyone | **Medium** — needs a product/copy decision on how to present the three paths, not a code fix. |
| 8 | "Add to pipeline" button invisible (black on black) | Confirmed at `app/app/page.js:2422`: `color: 'var(--marker-black)'` is hardcoded regardless of state, while `background` is `var(--marker-black)` until the job is added (then `var(--marker-lime)`). So the un-added state renders black text on a black background. | **Code** (one-line CSS) | Everyone | **High** — this is the single action that moves a job from feed to pipeline. |

**Self-test / verification method:** every root cause above was confirmed against either a live API call, a direct Supabase REST query against the production database (service-role key), or a direct code read of the exact function/line responsible — not inferred from symptoms alone. No fixes were applied this session per Rob's instruction; recommended fixes are noted per item for a future session.
