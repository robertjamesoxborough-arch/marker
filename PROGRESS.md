...
**Stage:** 35 complete — Session P, claims audit. Went through the Trust Panel line by line (the highest-risk page, per Rob's own framing) and cross-checked every specific claim against the real code, plus spot-checks on the homepage and pricing. Found 8 real findings, several of them significant: the G4 "auto-adds to your Watchlist, no manual step" claim is false (a separate "Add to pipeline" click is always required); the G2 "daily cron re-verifies every role" claim overstates what the cron actually does (pure date-math recomputation from an existing timestamp, no live check at all — only the user-triggered "Still open?" button does a real check); the pricing section states "3 analyses/day on free tier" when the real cap is 30/month; "Role sourcing: Adzuna, Gov.uk" omits the 4 ATS providers and wishlist-scrape entirely and wrongly implies Gov.uk is a separate licensed source when cron/gov just queries Adzuna again with different search terms. Full table with exact wording, code evidence, and recommendations in the Stage 35 log entry below — **no copy was changed**, per Rob's explicit instruction; this is a findings report awaiting his decisions. Audit was NOT exhaustive across every page given the time cap — Trust Panel got full line-by-line treatment as instructed; homepage/pricing got targeted spot-checks; onboarding, Terms, Privacy, and in-app tooltips/empty-states were not yet covered and are flagged as remaining scope.  
**Last commit:** (pending — PROGRESS.md and memory only this session, no code changed)  
