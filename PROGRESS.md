...
**Stage:** 34 complete — Session O, legal hardening for job data aggregation ahead of Rob's lawyer review. Built `lib/robots.js` (robots.txt respect, cached, self-tested against real ATS-hosted sites including a genuine Disallow rule) and wired it into `cron/wishlist-scrape`, the only cron that fetches arbitrary third-party HTML; added a polite, honest `RequiteBot` user-agent naming a contact email and a real delay between requests. Audited every job-link anchor in the app (11 total) — all already link out correctly with `target="_blank"`/`rel="noopener"` and there's no in-app application flow — documented as a hard, non-removable rule. Added transparent source labelling ("via Greenhouse"/"via Adzuna"/"from {company} careers page") to job cards, threading `track_tags` through to the client for the first time. Reused the previously-unused `admin_feature_flags` table for a real per-source kill switch, wired into all 5 ingest crons and a working admin UI (replacing an old "coming soon" stub). Audited and trimmed stored job-description text to the genuine minimum scoring needs (300 chars, confirmed never displayed to users). Added `public/robots.txt` and a job-data-sourcing clause to the Terms page. Caught and fixed a real near-miss along the way: a same-line comment that swallowed a closing brace, and the discovery that `node --check` gives a false pass for exactly this bug class on any file with a top-level `import` — the Vercel build is the only fully authoritative gate for this codebase.  
**Last commit:** feat: Session O — legal hardening for job data aggregation before commercial launch  
