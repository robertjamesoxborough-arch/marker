...
**Stage:** 36 complete — Session Q, the fix pass for every false claim Session P found, plus new findings from Rob's own live-site review. Pricing decision made final: candidate-pays is the moat (Requite is paid by the candidate, not the employer, unlike Jack & Jill/hackajob), so the employer-commission marketing claims are cut everywhere they appeared as an established, live feature (PricingSection headline/footer, Trust Panel employer card + CTA kicker) and replaced with honest candidate-pays copy; the employer-side code, routes, and tables are untouched and parked, not deleted. G2 (freshness) was rebuilt for real: `cron/freshness` now does genuine HTTP HEAD checks (via `safeFetch`, `isAllowedByRobots`, `REQUITE_USER_AGENT`, a bounded 40-link/run batch prioritised by staleness, and a 1s polite delay per request, mirroring `cron/wishlist-scrape`'s established pattern) and genuinely updates `last_verified_at`; the old pure date-math sweep still runs for the rest of the table (and for `employer_roles`, which has no external link field at all — date-math there is structurally correct, not a shortcut). G4 (auto-add) was fixed in copy, not code: `EngineTab`'s `analyse()` already auto-adds to the Watchlist for URL-based analyses (a prior fix, confirmed still working); the Feed tab's "SCORE" button does not, and still needs a separate "Add to pipeline" click — the Trust Panel now describes that real distinction instead of the blanket "no manual step" claim. Network Meter, Memory Card, and Balanced Roles were all resolved as copy fixes (building real per-field query filtering, inline career-history editing, or live company-data sourcing was judged clearly more work than making the copy accurate) — full reasoning in the Stage 36 log below. Also fixed: free-tier cap copy (3/day → 30/month) everywhere it appeared, including live in-app in Settings and MemoryCard, not just the Trust Panel; scoring-dimension counts reconciled (Employer shortlist scores 6→7 matching `match-engine.js`'s `WEIGHTS`; homepage "six things you care about" → the real 8-factor `/api/analyse` schema); Role sourcing copy now names Adzuna, all 4 ATS providers, and wishlisted-company career-page checks, and no longer implies Gov.uk is a separate licensed source. From Rob's own site review: the three homepage stat cards (74%/3x/82%) had zero citation anywhere in the codebase, so the whole section was cut (this also removed the "Marker"-watermarked images, since they were the same three assets); the "MOST CHOSEN" Pro badge was removed (zero paying users exist); the duplicate hero headline was fixed (bottom CTA now reads "Stop wasting evenings on roles that aren't right." instead of repeating the H1); a full "Marker" grep across every user-facing string found and fixed the cookie banner, `/cookies`, and 21 Notes-article CTA lines in `lib/articles.js`, plus 4 stale `'Marker/1.0'` outbound User-Agent headers now using `REQUITE_USER_AGENT`. Onboarding, Privacy, and in-app tooltips/empty-states were swept for the same class of claim (grep-based, not full manual read given the time cap) — clean, no further false claims found there. Full before/after detail, self-test evidence, and the reasoning behind every build-vs-copy call in the Stage 36 log entry below.  
